# hrcs_coordbizfield_t_hrcs_coordbizfield 系列 — 操作清单（合并去重）
**涵盖实体数**: 2  **去重后操作数**: 42
---
## 1. 继承树
以下展示目标实体的继承关系（缩进表示父子关系）：
```bos_basetpl (基础资料模板)    └── bos_basegrouptpl (分组基础资料模板)        └── hbp_bd_grouptpl_all (HR分组基础资料全页面模板)            └── hrcs_coordbizfield (业务协作字段)                └── hrcs_coordbizfieldlist (选择字段)```
---
## 2. 操作总览
...
| 4 | `b599405400001aac` | save | 保存 | save | 6 | 4 | 1 | 2 |
| 5 | `b599405400001bac` | delete | 删除 | delete | 3 | 3 | 1 | 2 |
...
> 根定义: `bos_basetpl` | 涉及实体 (2): `hrcs_coordbizfield`, `hrcs_coordbizfieldlist`
**插件清单**：
