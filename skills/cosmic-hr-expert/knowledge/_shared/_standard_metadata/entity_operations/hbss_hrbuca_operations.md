# hbss_hrbuca — HR业务管理视图 操作清单
**继承链**: `bos_basetpl` → `hbp_bd_tpl_all` → `hbp_bd_tpl_dlg` → `hbss_hrbuca`  **操作数**: 43
---
## 操作总览
...
| 42 | `0S=ILTA+FQ40` | donothing | 保存 | doupdate | `hbss_hrbuca` | 3 | 1 |
| 43 | `0Z424ZRNU5TH` | donothing | 空操作 | donothing | `hbss_hrbuca` | 0 | 0 |
---
## 操作详情（仅展示有插件或校验器的操作）
---
...
| 4 | ConditionValidation | `0+=DGI9FKA8/` | 合法性校验 | `hbss_hrbuca` |
...
|---|--------|---------|------------|------|
| 1 | — | `hbp_bd_tpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp |
| 2 | — | `hbss_hrbuca` | `kd.hr.hbss.opplugin.web.CheckIsSysPreSetOp` |  |
...
| 4 | ConditionValidation | `/XIYC/0WRRCM` | 合法性校验 | `hbss_hrbuca` |
...
---
### 42. 保存（donothing / Key: doupdate / oid: 0S=ILTA+FQ40）
> 根定义: `hbss_hrbuca`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `hbss_hrbuca` | `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp |
| 2 | 1 | `hbss_hrbuca` | `kd.hr.hbss.opplugin.web.HRBUCAUpdateOp` |  |
| 3 | 2 | `hbss_hrbuca` | `kd.hr.hbss.opplugin.web.HRBUUpdateOp` |  |

**校验器**：
| # | 类型 | Id | 说明 | 来源 |
|---|-----|----|------|------|
| 1 | MustInputValidation | `0WTS7OL1GJ0V` | 字段值合规性校验 | `hbss_hrbuca` |
