# hrcs_commonvariable — 常用变量 操作清单
**继承链**: `bos_basetpl` → `hbp_bd_tpl_all` → `hbp_bd_tpl_dlg` → `hrcs_commonvariable`  **操作数**: 43
---
## 操作总览
...
| 43 | `3NJ625BSZN7N` | donothing | 取值业务对象 | tplvariableconfig | `hrcs_commonvariable` | 0 | 0 |
---
## 操作详情（仅展示有插件或校验器的操作）
---
...
| 7 | 6 | `hrcs_commonvariable` | `kd.hr.hrcs.opplugin.web.CommonVariableSaveOp` | 保存插件 |
...
| 5 | GrpfieldsuniqueValidation | `varmappingscene.id` |  | `hrcs_commonvariable` |
...
| 4 | 3 | `hrcs_commonvariable` | `kd.hr.hrcs.opplugin.web.CommonVariableDeleteOp` | 删除插件 |
