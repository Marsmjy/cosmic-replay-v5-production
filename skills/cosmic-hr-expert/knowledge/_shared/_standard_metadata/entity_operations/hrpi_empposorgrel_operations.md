# hrpi_empposorgrel — 任职经历 操作清单
**继承链**: `hbp_bd_originalmintpl` → `hbp_bd_timelinemintpl` → `hrpi_assigntimelinetpl` → `hrpi_empposorgrel`  **操作数**: 23
---
## 操作总览
...
| 5 | `/X9FTE5ABBM6` | save | 保存 | save | `hbp_bd_originalmintpl` | 9 | 4 |
...
| 15 | `4RIR/ALATGNY` | delete | 删除 | delete | `hbp_bd_timelinemintpl` | 7 | 0 |
...
| 21 | `4WSFBV5X7=UQ` | exportlist | 导出数据（按导入模板） | exportlist | `hrpi_empposorgrel` | 0 | 0 |
| 22 | `4WT86C1R0NDK` | exportdetails | 查看导出结果 | exportdetails | `hrpi_empposorgrel` | 0 | 0 |
| 23 | `4XLNV0VM9/Y6` | close | 关闭 | close | `hrpi_empposorgrel` | 0 | 0 |
---
## 操作详情（仅展示有插件或校验器的操作）
---
...
| 6 | 5 | `hrpi_empposorgrel` | `kd.bos.business.plugin.CodeRuleOp` | kd.bos.business.plugin.CodeRuleOp |
| 7 | 6 | `hrpi_assigntimelinetpl` | `kd.hrmp.hrpi.opplugin.web.common.EmployeeCommonStandardMustInputOp` | 标品字段必填Op |
| 8 | 6 | `hrpi_empposorgrel` | `kd.hrmp.hrpi.opplugin.web.assignment.attach.EmpPosOrgRelSaveOp` | 任职经历保存插件 |
...
| 3 | GrpfieldsuniqueValidation | `55KCGVWPCC4B` | 任职编号不能重复 | `hrpi_empposorgrel` |
| 4 | DynamicValidation | `53BSB1UN7QZA` | 【岗位】字段对应的职位和【职位】字段的值是否一致校验 | `hrpi_empposorgrel` |
...
| 5 | 3 | `hrpi_empposorgrel` | `kd.bos.coderule.CodeRuleDeleteOp` | kd.bos.coderule.CodeRuleDeleteOp |
| 6 | 4 | `hrpi_assigntimelinetpl` | `kd.hrmp.hrpi.opplugin.web.template.IgnoreReferenceDeleteOp` | 附表删除忽略基础资料引用OP |
| 7 | 4 | `hrpi_empposorgrel` | `kd.hrmp.hrpi.opplugin.web.assignment.attach.EmpPosOrgRelDeleteOp` | 逻辑删除后置处理时间轴 |
