# hrpi_assignmentmag — 组织分配管理主体 操作清单
**继承链**: `hbp_bd_originalmintpl` → `hbp_bd_timelinemintpl` → `hrpi_assigntimelinetpl` → `hrpi_assignmentmag`  **操作数**: 20
---
## 操作总览
...
| 3 | 2 | `hrpi_assignmentmag` | `kd.hrmp.hrpi.opplugin.web.assignment.attach.AssignmentMagSaveOp` | 工作分配主体保存逻辑 |
| 4 | 3 | `hrpi_assigntimelinetpl` | `kd.hrmp.hrpi.opplugin.web.template.AssignmentTplCommonSaveOp` | 组织分配通用保存 |
| 5 | 4 | `hrpi_assigntimelinetpl` | `kd.hrmp.hrpi.opplugin.web.chgrecord.ChgRecordSaveOp` | 操作成功后更新变动事务记录 |
...
| 6 | 5 | `hrpi_assignmentmag` | `kd.hrmp.hrpi.opplugin.web.assignment.attach.AssignmentMagDeleteOp` | 组织分配管理主体删除OP |
