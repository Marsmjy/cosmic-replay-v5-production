# hrpi_terminationinfo — 离职信息 操作清单
**继承链**: `hbp_bd_originalmintpl` → `hbp_bd_timelinemintpl` → `hrpi_assigntimelinetpl` → `hrpi_terminationinfo`  **操作数**: 21
---
## 操作总览
...
| 5 | `/X9FTE5ABBM6` | save | 保存 | save | `hbp_bd_originalmintpl` | 9 | 2 |
...
| 15 | `4RIR/ALATGNY` | delete | 删除 | delete | `hbp_bd_timelinemintpl` | 6 | 0 |
...
| 21 | `5N9T2GRREJP6` | donothing | 修订离职日期 | revisedepartdate | `hrpi_terminationinfo` | 1 | 0 |
---
## 操作详情（仅展示有插件或校验器的操作）
---
...
| 5 | 4 | `hrpi_terminationinfo` | `kd.hrmp.hrpi.opplugin.web.assignment.attach.TerminationInfoSaveOp` | 开始日期和结束日期赋值逻辑 |
| 6 | 5 | `hrpi_assigntimelinetpl` | `kd.hrmp.hrpi.opplugin.web.assignment.EmpStageHandleOpPlugin` | 雇佣阶段关联处理 |
| 7 | 5 | `hrpi_terminationinfo` | `kd.hrmp.hrpi.opplugin.web.assignment.attach.TerminationInfoEmpStageOp` | 离职信息保存雇佣阶段处理 |
...
| 5 | 3 | `hrpi_terminationinfo` | `kd.hrmp.hrpi.opplugin.web.assignment.attach.TerminationInfoEmpStageOp` | 离职删除雇佣阶段处理 |
| 6 | 4 | `hrpi_assigntimelinetpl` | `kd.hrmp.hrpi.opplugin.web.template.IgnoreReferenceDeleteOp` | 附表删除忽略基础资料引用OP |
...
---
### 21. 修订离职日期（donothing / Key: revisedepartdate / oid: 5N9T2GRREJP6）
> 根定义: `hrpi_terminationinfo`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `hrpi_terminationinfo` | `kd.hrmp.hrpi.opplugin.web.assignment.attach.TerminationReviseDepartDateOp` | 离职日期修订OP插件 |
