# hrcs_essyncschemecfig — ES同步方案配置 操作清单
**继承链**: `bos_basetpl` → `hbp_bd_tpl_all` → `hrcs_essyncschemecfig`  **操作数**: 50
---
## 操作总览
...
| 43 | `2MV3JKWV=+N4` | donothing | 设置同步计划 | donothing_setsyncschedule | `hrcs_essyncschemecfig` | 0 | 2 |
| 44 | `2MV3QCBKVNTA` | donothing | 手动执行同步 | donothing_manualsync | `hrcs_essyncschemecfig` | 0 | 2 |
| 45 | `2MV63=IFFFYB` | donothing | 查看同步记录 | donothing_viewrecord | `hrcs_essyncschemecfig` | 0 | 0 |
| 46 | `2O+MA2E+B=7A` | donothing | 设置同步计划(表单页) | donothing_schedule | `hrcs_essyncschemecfig` | 0 | 0 |
| 47 | `2O+MGMEF7CB7` | donothing | 手动执行同步(表单页) | donothing_sync | `hrcs_essyncschemecfig` | 0 | 0 |
| 48 | `2OECFXCPOHG4` | save | 保存并配置同步计划 | save_setsyncscheme | `hrcs_essyncschemecfig` | 0 | 2 |
| 49 | `2OEDEX3KE8S1` | save | 保存并同步 | save_manualsync | `hrcs_essyncschemecfig` | 0 | 2 |
| 50 | `2R2H3QGB8+UJ` | donothing | ES环境配置 | donothing_esconfig | `hrcs_essyncschemecfig` | 0 | 0 |
---
## 操作详情（仅展示有插件或校验器的操作）
---
...
| 6 | 4 | `hrcs_essyncschemecfig` | `kd.hr.hrcs.opplugin.web.es.EsSyncSchemeConfigSaveOp` | EsSyncSchemeConfigSaveOp |
...
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | 1 | `hrcs_essyncschemecfig` | `kd.hr.hrcs.opplugin.web.es.EsSyncSchemeConfigDisableOp` | 禁用 |
...
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | 1 | `hrcs_essyncschemecfig` | `kd.hr.hrcs.opplugin.web.es.EsSyncSchemeConfigEnableOp` | EsSyncSchemeConfigEnableOp |
| 2 | — | `hbp_bd_tpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp |
...
---
### 43. 设置同步计划（donothing / Key: donothing_setsyncschedule / oid: 2MV3JKWV=+N4）
> 根定义: `hrcs_essyncschemecfig`
**校验器**：
| # | 类型 | Id | 说明 | 来源 |
|---|-----|----|------|------|
| 1 | ConditionValidation | `2NCBDNV9P1=Y` | 合法性校验 | `hrcs_essyncschemecfig` |
| 2 | ConditionValidation | `2NQ6VPAL7BYN` | 合法性校验 | `hrcs_essyncschemecfig` |

---
### 44. 手动执行同步（donothing / Key: donothing_manualsync / oid: 2MV3QCBKVNTA）
> 根定义: `hrcs_essyncschemecfig`
**校验器**：
| # | 类型 | Id | 说明 | 来源 |
|---|-----|----|------|------|
| 1 | ConditionValidation | `2NCBAW40YQZS` | 合法性校验 | `hrcs_essyncschemecfig` |
| 2 | ConditionValidation | `2NQ7/EK57Z67` | 合法性校验 | `hrcs_essyncschemecfig` |

---
### 48. 保存并配置同步计划（save / Key: save_setsyncscheme / oid: 2OECFXCPOHG4）
> 根定义: `hrcs_essyncschemecfig`
**校验器**：
| # | 类型 | Id | 说明 | 来源 |
|---|-----|----|------|------|
| 1 | ConditionValidation | `2OECD2/EL/C3` | 合法性校验 | `hrcs_essyncschemecfig` |
| 2 | ConditionValidation | `2OECF4L5MLU0` | 合法性校验 | `hrcs_essyncschemecfig` |

---
### 49. 保存并同步（save / Key: save_manualsync / oid: 2OEDEX3KE8S1）
> 根定义: `hrcs_essyncschemecfig`
**校验器**：
| # | 类型 | Id | 说明 | 来源 |
|---|-----|----|------|------|
| 1 | ConditionValidation | `2OEDCG+KXPO8` | 合法性校验 | `hrcs_essyncschemecfig` |
| 2 | ConditionValidation | `2OEDEBMA3VMQ` | 合法性校验 | `hrcs_essyncschemecfig` |
