# hbss_taxunit — 纳税单位 操作清单
**继承链**: `bos_basetpl` → `hbp_bd_tpl_all` → `hbp_bd_tpl_dlg` → `hbss_taxunit`  **操作数**: 47
---
## 操作总览
...
| 43 | `0DACD6GQT/MR` | donothing | 空操作 | individuation | `hbss_taxunit` | 0 | 0 |
| 44 | `1EU59CDCU06+` | moveentryup | 上移一行 | moveentryup | `hbss_taxunit` | 0 | 0 |
| 45 | `1EU5CBN6TU8D` | moveentrydown | 下移一行 | moveentrydown | `hbss_taxunit` | 0 | 0 |
| 46 | `3M3YGTWHL=OA` | donothing | 修订 | revise | `hbss_taxunit` | 0 | 0 |
| 47 | `3M71T1RUFAOX` | donothing | 修订保存 | revise_save | `hbss_taxunit` | 1 | 0 |
---
## 操作详情（仅展示有插件或校验器的操作）
---
...
| 7 | 5 | `hbss_taxunit` | `kd.hr.hbss.opplugin.web.taxunit.TaxUnitSaveOp` | 纳税单位保存插件 |
| 8 | 6 | `hbss_taxunit` | `kd.hr.hbss.opplugin.web.taxunit.TaxUnitLawEntityLoginOp` | 纳税单位法律实体注册插件 |
...
| 5 | GrpfieldsuniqueValidation | `0/ZQLLX5NKUG` | 纳税人识别号唯一性校验 | `hbss_taxunit` |
| 6 | MustInputValidation | `6073a58000003eac` | 必录校验 | `hbss_taxunit` |
| 7 | GrpfieldsuniqueValidation | `00I7OPB3IIFR` | 名称唯一性校验 | `hbss_taxunit` |
...
| 4 | 3 | `hbss_taxunit` | `kd.hr.hbss.opplugin.web.taxunit.TaxUnitLawEntityLoginOp` | 纳税单位法律实体注册插件 |

**校验器**：
| # | 类型 | Id | 说明 | 来源 |
|---|-----|----|------|------|
| 1 | ConditionValidation | `1cc0054f000016ac` | 合法性校验 | `bos_basetpl` |
| 2 | CustValidation | `2+RE4J37K857` | HR基础资料删除校验 | `hbp_bd_tpl_all` |
| 3 | ConditionValidation | `29I6GCUX8RY6` | 数据已经禁用，不能删除。 | `hbss_taxunit` |
...
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | 1 | `hbss_taxunit` | `kd.hr.hbss.opplugin.web.taxunit.TaxUnitUnAuditOp` |  |
...
| 4 | ConditionValidation | `/U2/165YZ4K5` | 合法性校验 | `hbss_taxunit` |
...
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | 1 | `hbss_taxunit` | `kd.hr.hbss.opplugin.web.taxunit.TaxUnitEnableOp` | 纳税单位启用插件 |
| 2 | — | `hbp_bd_tpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp |
...
| 6 | 5 | `hbss_taxunit` | `kd.hr.hbss.opplugin.web.taxunit.TaxUnitSaveOp` | kd.hr.hbss.opplugin.web.taxunit.TaxUnitSaveOp |
...
| 5 | GrpfieldsuniqueValidation | `424b895300015bac` | 模版上的编码的唯一性校验 | `hbss_taxunit` |
| 6 | GrpfieldsuniqueValidation | `62a4cf9800016fac` | 模版上的名称唯一性校验 | `hbss_taxunit` |
| 7 | MustInputValidation | `1=ACAJLBT12S` | 必录校验 | `hbss_taxunit` |
...
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `hbss_taxunit` | `kd.bd.assistant.plugin.basedata.BaseDataSavePlugin` |  |

---
### 47. 修订保存（donothing / Key: revise_save / oid: 3M71T1RUFAOX）
> 根定义: `hbss_taxunit`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `hbss_taxunit` | `kd.hr.hbss.opplugin.web.taxunit.TaxUnitReviseOp` | kd.hr.hbss.opplugin.web.taxunit.TaxUnitReviseOp |
