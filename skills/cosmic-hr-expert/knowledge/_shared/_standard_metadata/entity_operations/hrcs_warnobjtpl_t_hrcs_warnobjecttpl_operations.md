# hrcs_warnobjtpl_t_hrcs_warnobjecttpl 系列 — 操作清单（合并去重）
**涵盖实体数**: 2  **去重后操作数**: 45
---
## 1. 继承树
以下展示目标实体的继承关系（缩进表示父子关系）：
```bos_basetpl (基础资料模板)    └── hbp_bd_tpl_all (HR基础资料全页面模板)        └── hrcs_warnobjtpl (预警对象模板(废弃))            └── hrcs_warnobjtpl_listout (预警对象模板布局(废弃))```
---
## 2. 操作总览
...
| 43 | `43C7DR//BP14` | donothing | 下一步 | nextstep | 0 | 2 | 1 | 2 |
| 44 | `43C7GOKZ6K34` | donothing | 上一步 | laststep | 0 | 0 | 1 | 2 |
| 45 | `44Z571XVVFPO` | donothing | 数据预览 | previewdata | 0 | 0 | 1 | 2 |
---
## 3. 操作详情（仅展示有插件或校验器的操作）
---
### 4. 保存（save / Key: save / oid: b599405400001aac）
> 根定义: `bos_basetpl` | 涉及实体 (2): `hrcs_warnobjtpl`, `hrcs_warnobjtpl_listout`
**插件清单**：
...
| 7 | 6 | `hrcs_warnobjtpl` | `kd.hr.hrcs.opplugin.web.earlywarn.objtpl.WarnObjTplOp` | kd.hr.hrcs.opplugin.web.earlywarn.objtpl.WarnObjTplOp |
| 8 | 7 | `hrcs_warnobjtpl` | `kd.hr.hrcs.opplugin.web.earlywarn.objtpl.WarnObjTplAdConditionOp` | 预警对象模板高级条件op |
...
| 4 | 3 | `hrcs_warnobjtpl` | `kd.hr.hrcs.opplugin.web.earlywarn.objtpl.WarnObjTplOp` | 预警对象模板op插件 |
...
---
### 43. 下一步（donothing / Key: nextstep / oid: 43C7DR//BP14）
> 根定义: `hrcs_warnobjtpl` | 涉及实体 (2): `hrcs_warnobjtpl`, `hrcs_warnobjtpl_listout`
**校验器**：
| # | 类型 | Id | 说明 | 来源 |
|---|-----|----|------|------|
| 1 | GrpfieldsuniqueValidation | `44AA9QCHEK1R` | 编码唯一性校验 | `hrcs_warnobjtpl` |
| 2 | GrpfieldsuniqueValidation | `44AAIWUQFQ4M` | 名称唯一性校验 | `hrcs_warnobjtpl` |
