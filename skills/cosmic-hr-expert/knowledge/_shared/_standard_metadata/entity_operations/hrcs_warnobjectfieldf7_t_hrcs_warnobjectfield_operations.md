# hrcs_warnobjectfieldf7_t_hrcs_warnobjectfield 系列 — 操作清单（合并去重）
**涵盖实体数**: 3  **去重后操作数**: 11
---
## 1. 继承树
以下展示目标实体的继承关系（缩进表示父子关系）：
```hbp_bd_originalmintpl (HR原生基础资料最小模板)    └── hbp_queryfieldtpl (查询字段模板)        └── hrcs_warnscenequeryfield (预警场景查询字段)            ├── hrcs_selectqueryfieldf7 (已选查询字段F7)            └── hrcs_warnobjectfieldf7 (预警场景字段F7)```
---
## 2. 操作总览
