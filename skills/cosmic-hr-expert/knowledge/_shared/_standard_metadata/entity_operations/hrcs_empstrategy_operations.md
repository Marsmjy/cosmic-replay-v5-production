# hrcs_empstrategy — 行政组织-员工管理关系策略设置 操作清单
**继承链**: `bos_basetpl` → `hbp_bd_tpl_all` → `hrcs_managestrategy` → `hrcs_empstrategy_tpl` → `hrcs_empstrategy`  **操作数**: 46
---
## 操作总览
...
| 42 | `0O68451YUI/5` | newentry | 新增分录 | newentry | `hrcs_managestrategy` | 0 | 0 |
| 43 | `0O685=2+GRU0` | deleteentry | 删除分录 | deleteentry | `hrcs_managestrategy` | 0 | 0 |
| 44 | `11KYANWQUWN9` | donothing | 变更 | change | `hrcs_empstrategy` | 0 | 0 |
| 45 | `1ZN/S8TSQ6U0` | donothing | 初始化员工管理关系策略 | initempstrategy | `hrcs_empstrategy` | 0 | 0 |
| 46 | `26/ICDA7R+YR` | donothing | 查看变更记录 | barviewchange | `hrcs_empstrategy` | 0 | 0 |
---
## 操作详情（仅展示有插件或校验器的操作）
---
...
| 5 | 4 | `hrcs_managestrategy` | `kd.hr.hrcs.opplugin.web.strategy.ManageStrategyLogOp` | kd.hr.hrcs.opplugin.web.strategy.ManageStrategyLogOp |
| 6 | 4 | `hrcs_empstrategy` | `kd.hr.hrcs.opplugin.web.EmpManageStrategySaveOp` | kd.hr.hrcs.opplugin.web.EmpManageStrategySaveOp |
