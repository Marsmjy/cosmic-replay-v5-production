# hrcs_tplvariableconfig_t_hrcs_tplvariableconfig 系列 — 操作清单（合并去重）
**涵盖实体数**: 2  **去重后操作数**: 53
---
## 1. 继承树
以下展示目标实体的继承关系（缩进表示父子关系）：
```bos_basetpl (基础资料模板)    └── hbp_bd_tpl_all (HR基础资料全页面模板)        ├── hrcs_addtplvariableconfig (添加子业务对象)        └── hrcs_tplvariableconfig (取值对象配置)```
---
## 2. 操作总览
...
| 43 | `3V72UCHBWE/9` | newentry | 新增 | newentry | 0 | 0 | 1 | 1 |
| 44 | `3V72XH11JV/Q` | deleteentry | 删除 | deleteentry | 0 | 0 | 1 | 1 |
| 45 | `3V72ZBK2IURY` | moveentryup | 上移 | moveentryup | 0 | 0 | 1 | 1 |
| 46 | `3V73+O66BODS` | moveentrydown | 下移 | moveentrydown | 0 | 0 | 1 | 1 |
| 47 | `3V77+K=66W1+` | donothing | 添加子实体 | addentity | 0 | 0 | 1 | 1 |
| 48 | `3V771=K=IJC1` | newentry | 新增分录 | newentry | 0 | 0 | 1 | 1 |
| 49 | `3V773BNI4TDQ` | deleteentry | 删除分录 | deleteentry | 0 | 0 | 1 | 1 |
| 50 | `3V775FX1KN5I` | moveentryup | 上移一行 | moveentryup | 0 | 0 | 1 | 1 |
| 51 | `3V777BWN1NUE` | moveentrydown | 下移一行 | moveentrydown | 0 | 0 | 1 | 1 |
| 52 | `3V77D7UDISH3` | donothing | 删除 | deleteentity | 1 | 0 | 1 | 1 |
| 53 | `3V77FSDAFLK0` | donothing | 取消 | cancel | 0 | 0 | 1 | 1 |
---
## 3. 操作详情（仅展示有插件或校验器的操作）
---
### 4. 保存（save / Key: save / oid: b599405400001aac）
> 根定义: `bos_basetpl` | 涉及实体 (2): `hrcs_addtplvariableconfig`, `hrcs_tplvariableconfig`
**插件清单**：
...
| 7 | 6 | `hrcs_addtplvariableconfig` | `kd.hr.hrcs.opplugin.web.TplVariableConfigSaveOpPlugin` | kd.hr.hrcs.opplugin.web.TplVariableConfigSaveOpPlugin |
...
---
### 52. 删除（donothing / Key: deleteentity / oid: 3V77D7UDISH3）
> 根定义: `hrcs_tplvariableconfig` | 涉及实体 (1): `hrcs_tplvariableconfig`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `hrcs_tplvariableconfig` | `kd.hr.hrcs.opplugin.web.TplVariableConfigDeleteOpPlugin` | kd.hr.hrcs.opplugin.web.TplVariableConfigDeleteOpPlugin |
