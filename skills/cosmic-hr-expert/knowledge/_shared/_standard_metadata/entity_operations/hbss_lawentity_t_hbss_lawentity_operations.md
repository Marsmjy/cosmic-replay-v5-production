# hbss_lawentity_t_hbss_lawentity 系列 — 操作清单（合并去重）
**涵盖实体数**: 2  **去重后操作数**: 50
---
## 1. 继承树
以下展示目标实体的继承关系（缩进表示父子关系）：
```bos_basetpl (基础资料模板)    └── bos_baseorgtpl (基础资料带组织模板)        └── hbp_bd_orgtpl_all (HR带组织基础资料全页面模板)            └── hbss_lawentity (法律实体)                └── hbss_lawentitychg (法律实体变更)```
---
## 2. 操作总览
...
| 4 | `b599405400001aac` | save | 保存 | save | 9 | 6 | 1 | 2 |
| 5 | `b599405400001bac` | delete | 删除 | delete | 4 | 3 | 1 | 2 |
| 6 | `b599405400001cac` | audit | 审核 | audit | 3 | 3 | 1 | 2 |
...
| 12 | `f381c03f00002cac` | submit | 提交 | submit | 6 | 5 | 1 | 2 |
...
| 33 | `2PVU5DLAO/49` | donothing | 名称历史查询 | namehistoryview | 0 | 0 | 1 | 2 |
| 34 | `QQJ9VPEIWE2` | assign | 分配 | assign | 0 | 0 | 1 | 2 |
| 35 | `QQJATA099TH` | unassign | 取消分配 | unassign | 0 | 0 | 1 | 2 |
| 36 | `/IRN4Y=FI53J` | donothing | 变更控制策略 | bdctrlchange | 0 | 0 | 1 | 2 |
| 37 | `0DACD6GQT/MR` | donothing | 空操作 | individuation | 0 | 0 | 1 | 2 |
| 38 | `329L6QTC2T7I` | donothing | 分配 | assign_new | 0 | 0 | 1 | 2 |
| 39 | `3A=7K395P2/8` | donothing | 自动分配管理 | auto_assign | 0 | 0 | 1 | 2 |
| 40 | `3ACT23RWB6N/` | donothing | Excel导入分配关系 | tbl_assign_import | 0 | 0 | 1 | 2 |
| 41 | `21A5G1REGO=D` | donothing | 查看全部日志 | logview | 0 | 0 | 1 | 2 |
| 42 | `2CM=+M12M1OM` | donothing | 查看日志 | viewonelog | 0 | 0 | 1 | 2 |
| 43 | `36NMATCD+40R` | importdata_hr | 导入数据 | importdata_hr | 0 | 0 | 1 | 2 |
| 44 | `36NMCH7J7K17` | show_import_record_hr | 查看导入记录 | show_import_record_hr | 0 | 0 | 1 | 2 |
| 45 | `36NMEU7ZJ64A` | export_from_list_hr | 按列表导出 | export_from_list_hr | 0 | 0 | 1 | 2 |
| 46 | `36NMFZKH+JK4` | export_from_impttpl_hr | 按导入模板导出 | export_from_impttpl_hr | 0 | 0 | 1 | 2 |
| 47 | `36NMHG0I3HKP` | export_from_expttpl_hr | 按导出模板导出 | export_from_expttpl_hr | 0 | 0 | 1 | 2 |
| 48 | `36NMIYKCRQOF` | show_export_record_hr | 查看导出记录 | show_export_record_hr | 0 | 0 | 1 | 2 |
| 49 | `2PE6DN1=TW4U` | donothing | 变更 | lawchange | 0 | 0 | 1 | 2 |
| 50 | `2PLKW21X625N` | save | 保存 | editsave | 2 | 5 | 1 | 1 |
---
## 3. 操作详情（仅展示有插件或校验器的操作）
---
### 4. 保存（save / Key: save / oid: b599405400001aac）
> 根定义: `bos_basetpl` | 涉及实体 (2): `hbss_lawentity`, `hbss_lawentitychg`
**插件清单**：
...
| 7 | 6 | `hbss_lawentity` | `kd.hr.hbss.opplugin.web.lawentity.LawEntityChgOp` | 法律实体变更OP插件 |
| 8 | 7 | `hbp_bd_orgtpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp` | kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp |
| 9 | 8 | `hbss_lawentity` | `kd.hr.hbss.opplugin.web.lawentity.LawEntitySaveOp` | 法律实体保存OP |
...
| 4 | ConditionValidation | `2PF21E57SQZ4` | 变更保存时变更类型为空校验 | `hbss_lawentity` |
| 5 | GrpfieldsuniqueValidation | `2PF24N0SB53M` | 名称唯一性校验 | `hbss_lawentity` |
| 6 | GrpfieldsuniqueValidation | `2PF2659CK0DL` | 编码唯一性校验 | `hbss_lawentity` |
...
| 2 | 2 | `hbss_lawentity` | `kd.hr.hbss.opplugin.web.lawentity.LawEntityAuditOp` | kd.hr.hbss.opplugin.web.lawentity.LawEntityAuditOp |
...
---
### 50. 保存（save / Key: editsave / oid: 2PLKW21X625N）
> 根定义: `hbss_lawentitychg` | 涉及实体 (1): `hbss_lawentitychg`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `hbss_lawentitychg` | `kd.hr.hbss.opplugin.web.lawentity.LawEntityChgOp` | 法律实体变更保存OP插件 |
| 2 | 1 | `hbss_lawentitychg` | `kd.hr.hbss.opplugin.web.lawentity.LawEntityLogSaveOp` | 法律实体变更日志保存op |

**校验器**：
| # | 类型 | Id | 说明 | 来源 |
|---|-----|----|------|------|
| 1 | MustInputValidation | `2PLMY97URSH=` | 字段值合规性校验 | `hbss_lawentitychg` |
| 2 | GrpfieldsuniqueValidation | `2PLN05WL99UI` | 创建组织和编码组合字段唯一性校验 | `hbss_lawentitychg` |
| 3 | ConditionValidation | `2PLN5=SYF35K` | 变更保存时变更类型为空校验 | `hbss_lawentitychg` |
| 4 | GrpfieldsuniqueValidation | `2PLN7=/KU40V` | 名称唯一性校验 | `hbss_lawentitychg` |
| 5 | GrpfieldsuniqueValidation | `2PLN8S0W=8DF` | 编码唯一性校验 | `hbss_lawentitychg` |
