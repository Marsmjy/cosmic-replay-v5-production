# hrpi_appointremoverel — 任免经历 操作清单
**继承链**: `hbp_bd_originalmintpl` → `hbp_bd_timelinemintpl` → `hrpi_assigntimelinetpl` → `hrpi_appointremoverel`  **操作数**: 21
---
## 操作总览
...
| 5 | `/X9FTE5ABBM6` | save | 保存 | save | `hbp_bd_originalmintpl` | 8 | 5 |
...
| 21 | `5/E65IF1Z+FT` | close | 关闭 | close | `hrpi_appointremoverel` | 0 | 0 |
---
## 操作详情（仅展示有插件或校验器的操作）
---
...
| 6 | 5 | `hrpi_appointremoverel` | `kd.hrmp.hrpi.opplugin.web.assignment.attach.AppointRemoveRelSaveOp` | 保存插件 |
...
| 3 | DynamicValidation | `58Z7DWXAEWYM` | 字段选择的数据是岗位时，岗位不在行政组织下，提示：行政组织“%1$s”下不存在岗位“%2$s” | `hrpi_appointremoverel` |
| 4 | DynamicValidation | `597MVVSUABF9` | 【岗位】字段对应的职位和【职位】字段的值是否一致校验 | `hrpi_appointremoverel` |
| 5 | DynamicValidation | `597N0IY9SILH` | 字段选择的数据是标准岗位时，需要校验是否在标准岗位的适用组织范围内 | `hrpi_appointremoverel` |
