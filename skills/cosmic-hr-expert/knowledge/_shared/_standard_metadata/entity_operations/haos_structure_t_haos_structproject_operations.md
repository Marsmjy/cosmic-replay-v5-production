# haos_structure_t_haos_structproject 系列 — 操作清单（合并去重）
**涵盖实体数**: 2  **去重后操作数**: 53
---
## 1. 继承树
以下展示目标实体的继承关系（缩进表示父子关系）：
```bos_basetpl (基础资料模板)    └── hbp_bd_tpl_all (HR基础资料全页面模板)        ├── haos_structproject (矩阵组织设置)        └── haos_structure (矩阵组织维护)```
---
## 2. 操作总览
...
| 4 | `b599405400001aac` | save | 保存 | save | 7 | 5 | 2 | 2 |
| 5 | `b599405400001bac` | delete | 删除 | delete | 4 | 3 | 2 | 2 |
| 6 | `b599405400001cac` | audit | 审核 | audit | 1 | 3 | 1 | 2 |
| 7 | `b599405400001dac` | unaudit | 反审核 | unaudit | 1 | 3 | 1 | 2 |
| 8 | `b599405400001eac` | disable | 禁用 | disable | 2 | 3 | 2 | 2 |
| 9 | `b599405400001fac` | enable | 启用 | enable | 2 | 1 | 2 | 2 |
...
| 43 | `3BSF+V0IJXAJ` | donothing | 调整架构 | maintainframework | 1 | 0 | 2 | 2 |
| 44 | `3C5+=UONNYOM` | newentry | 新增分录 | newentry | 0 | 0 | 1 | 2 |
| 45 | `3C5+NPNO5KYC` | deleteentry | 删除分录 | deleteentry | 0 | 0 | 1 | 2 |
| 46 | `3C5/+9HY/OU5` | previousentry | 上一分录 | previousentry | 0 | 0 | 1 | 2 |
| 47 | `3C5/=KT5B6SK` | nextentry | 下一分录 | nextentry | 0 | 0 | 1 | 2 |
| 48 | `3CBB3CWDMMN7` | donothing | 新增_带组织 | addnew_org | 0 | 0 | 1 | 1 |
| 49 | `3CZWRF4ZXO4P` | donothing | 维护架构 | maintain_struct | 1 | 0 | 1 | 1 |
| 50 | `3FW3OLM04QC9` | donothing | 空操作 | donothing | 0 | 0 | 1 | 1 |
| 51 | `3FW5JMZTAIF2` | donothing | 删除 | delete_project | 1 | 2 | 1 | 1 |
| 52 | `3JMMGSBMFGV/` | donothing | 编辑架构-网络互斥用 | edit_struct | 0 | 0 | 1 | 1 |
| 53 | `3J=+0ZFN7ZPR` | donothing | 空操作 | vectorap | 0 | 0 | 1 | 1 |
---
## 3. 操作详情（仅展示有插件或校验器的操作）
---
### 4. 保存（save / Key: save / oid: b599405400001aac）
> 根定义: `bos_basetpl` | 涉及实体 (2): `haos_structproject`, `haos_structure` | **2 个插件变体**

#### 变体 1（1 个实体）
> 实体: `haos_structproject`
**插件清单**：
...
| 7 | 6 | `haos_structproject` | `kd.hr.haos.opplugin.web.otherframework.StructProjectSaveOp` | kd.hr.haos.opplugin.web.otherframework.StructProjectSaveOp |
...
| 5 | GrpfieldsuniqueValidation | `3FF3TK7F54AR` | 组合字段唯一性校验 | `haos_structproject` |
...
| 4 | 3 | `haos_structproject` | `kd.hr.haos.opplugin.web.otherframework.StructProjectDeleteOP` | kd.hr.haos.opplugin.web.otherframework.StructProjectDeleteOP |
...
> 根定义: `bos_basetpl` | 涉及实体 (2): `haos_structproject`, `haos_structure`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
...
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | 1 | `haos_structproject` | `kd.hr.haos.opplugin.web.otherframework.StructProjectDisableOp` | kd.hr.haos.opplugin.web.otherframework.StructProjectDisableOp |
...
| 3 | ConditionValidation | `3MXYV64E0QH9` | 预置数据不能禁用 | `haos_structproject` |
...
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | 1 | `haos_structproject` | `kd.hr.haos.opplugin.web.otherframework.StructProjectEnableOp` | kd.hr.haos.opplugin.web.otherframework.StructProjectEnableOp |
| 2 | — | `hbp_bd_tpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp |
...
---
### 43. 调整架构（donothing / Key: maintainframework / oid: 3BSF+V0IJXAJ）
> 根定义: `haos_structproject` | 涉及实体 (2): `haos_structproject`, `haos_structure` | **2 个插件变体**
...
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `haos_structproject` | `kd.hr.haos.opplugin.web.structproject.StructOrgPermSaveOp` | kd.hr.haos.opplugin.web.structproject.StructOrgPermSaveOp |


#### 变体 2（1 个实体）
> 实体: `haos_structure`
---
### 49. 维护架构（donothing / Key: maintain_struct / oid: 3CZWRF4ZXO4P）
> 根定义: `haos_structproject` | 涉及实体 (1): `haos_structproject`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `haos_structproject` | `kd.hr.haos.opplugin.web.structproject.StructOrgPermSaveOp` | kd.hr.haos.opplugin.web.structproject.StructOrgPermSaveOp |
...
---
### 51. 删除（donothing / Key: delete_project / oid: 3FW5JMZTAIF2）
> 根定义: `haos_structproject` | 涉及实体 (1): `haos_structproject`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `haos_structproject` | `kd.hr.haos.opplugin.web.otherframework.StructProjectDeleteOP` | kd.hr.haos.opplugin.web.otherframework.StructProjectDeleteOP |

**校验器**：
| # | 类型 | Id | 说明 | 来源 |
|---|-----|----|------|------|
| 1 | ConditionValidation | `3FXCNR3UKOCG` | 数据已经禁用，不能删除。 | `haos_structproject` |
| 2 | ConditionValidation | `3FXCQ0DYMI6V` | 数据已经被启用，不能删除。 | `haos_structproject` |
