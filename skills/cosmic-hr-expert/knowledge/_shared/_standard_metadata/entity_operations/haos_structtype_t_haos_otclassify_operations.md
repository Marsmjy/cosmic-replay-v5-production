# haos_structtype_t_haos_otclassify 系列 — 操作清单（合并去重）
**涵盖实体数**: 2  **去重后操作数**: 46
---
## 1. 继承树
以下展示目标实体的继承关系（缩进表示父子关系）：
```bos_basetpl (基础资料模板)    └── hbp_bd_tpl_all (HR基础资料全页面模板)        └── hbp_bd_tpl_dlg (HR基础资料对话框模板)            └── haos_otclassify (组织分类)                └── haos_structtype (架构类型)```
---
## 2. 操作总览
...
| 4 | `b599405400001aac` | save | 保存 | save | 7 | 4 | 2 | 2 |
...
| 8 | `b599405400001eac` | disable | 禁用 | disable | 2 | 1 | 2 | 2 |
...
| 43 | `5CYRZLHIK53E` | donothing | 变更名称 | chgname | 0 | 0 | 1 | 1 |
| 44 | `5CYSS73VL1TP` | save | 保存 | update | 1 | 3 | 1 | 1 |
| 45 | `5E/+Z3CN9LE7` | donothing | 删除 | deleteallrel | 1 | 0 | 1 | 1 |
| 46 | `5ENH=0RY=X43` | save | 变更 | changesave | 1 | 0 | 1 | 1 |
---
## 3. 操作详情（仅展示有插件或校验器的操作）
---
### 4. 保存（save / Key: save / oid: b599405400001aac）
> 根定义: `bos_basetpl` | 涉及实体 (2): `haos_otclassify`, `haos_structtype` | **2 个插件变体**
...
| 5 | 4 | `hbp_bd_tpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseDataEnableOp` | 控制使用状态 |
| 6 | 5 | `hbp_bd_tpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp` | kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp |
| 7 | 6 | `haos_structtype` | `kd.hr.haos.opplugin.web.otherstruct.structtype.StructTypeSaveOp` | kd.hr.haos.opplugin.web.otherstruct.StructTypeSaveOp |
...
> 根定义: `bos_basetpl` | 涉及实体 (2): `haos_otclassify`, `haos_structtype`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
...
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | 1 | `haos_structtype` | `kd.hr.haos.opplugin.web.otherstruct.structtype.StructTypeDisableOp` | 架构类型禁用 |
...
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | 1 | `haos_structtype` | `kd.hr.haos.opplugin.web.otherstruct.structtype.StructTypeEnableOp` | 架构类型启用 |
| 2 | — | `hbp_bd_tpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp |
...
---
### 44. 保存（save / Key: update / oid: 5CYSS73VL1TP）
> 根定义: `haos_structtype` | 涉及实体 (1): `haos_structtype`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `haos_structtype` | `kd.hr.haos.opplugin.web.otherstruct.structtype.StructTypeSaveOp` | 架构类型保存插件 |

**校验器**：
| # | 类型 | Id | 说明 | 来源 |
|---|-----|----|------|------|
| 1 | MustInputValidation | `5CYSIZFR+DXA` | 字段值合规性校验 | `haos_structtype` |
| 2 | GrpfieldsuniqueValidation | `5CYSNLVZO+4Y` | 编码唯一性校验 | `haos_structtype` |
| 3 | GrpfieldsuniqueValidation | `5CYSPT22A/HF` | 名称唯一性校验 | `haos_structtype` |

---
### 45. 删除（donothing / Key: deleteallrel / oid: 5E/+Z3CN9LE7）
> 根定义: `haos_structtype` | 涉及实体 (1): `haos_structtype`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `haos_structtype` | `kd.hr.haos.opplugin.web.otherstruct.structtype.StructTypeDeleteDonothingOp` | 架构类型删除根组织，组织架构，矩阵组织，架构类型配置，元数据，菜单 |

---
### 46. 变更（save / Key: changesave / oid: 5ENH=0RY=X43）
> 根定义: `haos_structtype` | 涉及实体 (1): `haos_structtype`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `haos_structtype` | `kd.hr.haos.opplugin.web.otherstruct.structtype.StructTypeChgNameOp` | 变更名称修改菜单和元数据 |
