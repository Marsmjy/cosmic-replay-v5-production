# hrpi_perserlen — 服务年限 操作清单
**继承链**: `hbp_bd_originalmintpl` → `hbp_bd_timelinemintpl` → `hrpi_assigntimelinetpl` → `hrpi_perserlen`  **操作数**: 21
---
## 操作总览
...
| 21 | `23/AQ3FTXY0B` | close | 退出 | close | `hrpi_perserlen` | 0 | 0 |
---
## 操作详情（仅展示有插件或校验器的操作）
---
...
| 3 | 2 | `hrpi_assigntimelinetpl` | `kd.hrmp.hrpi.opplugin.web.template.TimeLineCommonSaveOp` | 时间轴通用操作 |
| 4 | 3 | `hrpi_assigntimelinetpl` | `kd.hrmp.hrpi.opplugin.web.template.AssignmentTplCommonSaveOp` | 组织分配通用保存 |
| 5 | 3 | `hrpi_perserlen` | `kd.hrmp.hrpi.opplugin.web.assignment.attach.PerSerLenSaveOp` | 服务年限司龄工龄的计算 |
| 6 | 4 | `hrpi_assigntimelinetpl` | `kd.hrmp.hrpi.opplugin.web.chgrecord.ChgRecordSaveOp` | 操作成功后更新变动事务记录 |
| 7 | 5 | `hrpi_assigntimelinetpl` | `kd.hrmp.hrpi.opplugin.web.assignment.EmpStageHandleOpPlugin` | 雇佣阶段关联处理 |
