# hrcs_prompt — 提示语配置 操作清单
**继承链**: `bos_basetpl` → `hbp_bd_tpl_all` → `hrcs_prompt`  **操作数**: 51
---
## 操作总览
...
| 43 | `1H7=1YCHHI51` | close | 关闭 | close | `hrcs_prompt` | 0 | 0 |
| 44 | `1HE/ZV=UHGQH` | donothing | 选择变量 | selectfield | `hrcs_prompt` | 0 | 0 |
| 45 | `1HV7OOQCH5N3` | donothing | 查看规则 | viewruledetail | `hrcs_prompt` | 0 | 0 |
| 46 | `1MKJWKZ=NAMK` | donothing | 关联提示语 | opentips | `hrcs_prompt` | 0 | 0 |
| 47 | `1MUYEFR1ANMX` | exportlist | 引出提示语及内容 | export_prompt | `hrcs_prompt` | 0 | 0 |
| 48 | `2+6/3BTI9/CR` | donothing | 空操作 | donothing | `hrcs_prompt` | 0 | 0 |
| 49 | `2+60XV3C3CHP` | donothing | 自定义变量 | customvar | `hrcs_prompt` | 0 | 0 |
| 50 | `2+616SBGRIFL` | donothing | 自定义弹窗 | customwindow | `hrcs_prompt` | 0 | 0 |
| 51 | `3W63/79I1I0G` | save | 导入保存 | importsave | `hrcs_prompt` | 8 | 3 |
---
## 操作详情（仅展示有插件或校验器的操作）
---
...
| 3 | 1 | `hrcs_prompt` | `kd.hr.hrcs.opplugin.web.PromptSaveOp` |  |
| 4 | 2 | `hbp_bd_tpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseDataStatusOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataStatusOp |
| 5 | 3 | `hbp_bd_tpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp |
| 6 | 4 | `hbp_bd_tpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseDataEnableOp` | 控制使用状态 |
...
| 3 | 1 | `hrcs_prompt` | `kd.hr.hrcs.opplugin.web.PromptDeleteOp` |  |
| 4 | 2 | `hbp_bd_tpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp |
...
---
### 51. 导入保存（save / Key: importsave / oid: 3W63/79I1I0G）
> 根定义: `hrcs_prompt`
**插件清单**：
| # | RowKey | 来源层级 | 插件类全路径 | 说明 |
|---|--------|---------|------------|------|
| 1 | — | `hrcs_prompt` | `kd.bos.business.plugin.CodeRuleOp` | kd.bos.business.plugin.CodeRuleOp |
| 2 | 1 | `hrcs_prompt` | `kd.bos.base.bdversion.BdVersionSaveServicePlugin` | kd.bos.base.bdversion.BdVersionSaveServicePlugin |
| 3 | 2 | `hrcs_prompt` | `kd.hr.hbp.opplugin.web.config.HRBaseDataStatusOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataStatusOp |
| 4 | 3 | `hrcs_prompt` | `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp |
| 5 | 4 | `hrcs_prompt` | `kd.hr.hbp.opplugin.web.config.HRBaseDataEnableOp` | kd.hr.hbp.opplugin.web.config.HRBaseDataEnableOp |
| 6 | 5 | `hrcs_prompt` | `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp` | kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp |
| 7 | 6 | `hrcs_prompt` | `kd.hr.hrcs.opplugin.web.PromptBatchImportSaveOp` | kd.hr.hrcs.opplugin.web.PromptBatchImportSaveOp |
| 8 | 7 | `hrcs_prompt` | `kd.hr.hrcs.opplugin.web.PromptSaveOp` | kd.hr.hrcs.opplugin.web.PromptSaveOp |

**校验器**：
| # | 类型 | Id | 说明 | 来源 |
|---|-----|----|------|------|
| 1 | MustInputValidation | `3W62NM6=WW9J` | 字段值合规性校验 | `hrcs_prompt` |
| 2 | GrpfieldsuniqueValidation | `3W62P/1J6AIJ` | 编码唯一性校验 | `hrcs_prompt` |
| 3 | GrpfieldsuniqueValidation | `3W62QGS/93M3` | 名称唯一性校验 | `hrcs_prompt` |
