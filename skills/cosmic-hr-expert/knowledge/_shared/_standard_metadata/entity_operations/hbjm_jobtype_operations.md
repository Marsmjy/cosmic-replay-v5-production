# hbjm_jobtype — 职位类别 操作清单
**继承链**: `bos_basetpl` → `bos_baseorgtpl` → `hbp_bd_orgtpl_all` → `hbp_bd_orgtpl_dlg` → `hbjm_bd_orgtpl_dlg` → `hbjm_jobtype`  **操作数**: 49
---
## 操作总览
...
| 4 | `b599405400001aac` | save | 保存 | save | `bos_basetpl` | 10 | 6 |
...
| 7 | 5 | `hbjm_bd_orgtpl_dlg` | `kd.hrmp.hbjm.opplugin.web.HBJMDataBaseOp` | 使用状态和初始化状态校验 |
| 8 | 6 | `hbp_bd_orgtpl_all` | `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp` | kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp |
| 9 | 6 | `hbjm_jobtype` | `kd.hrmp.hbjm.opplugin.web.HbjmBaseNewDataOp` | hbjm基础新增op |
| 10 | 7 | `hbjm_jobtype` | `kd.hrmp.hbjm.opplugin.web.HbjmBaseChangeOp` | hbjm基础变更op |
...
| 4 | 4 | `hbjm_jobtype` | `kd.hrmp.hbjm.opplugin.web.DeleteIgnorInitMetaOp` | 排除引用 |
...
| 4 | ConditionValidation | `/NNA=SJXYN9E` | 合法性校验 | `hbjm_jobtype` |
