...
## 实体: haos_changescene（行政组织变动场景） [BaseEntity]

- **数据库表**: `t_haos_changescene`  
...
| orgchangetype | 变动类型 | BasedataField | forgchangetypeid | ✓ | haos_orgchangetype |
| otclassify | 组织团队分类 | BasedataField | fotclassifyid |  | haos_otclassify |
| changereason | 变动原因 | MulBasedataField | t_haos_cschangereason（子表） |  |  |
| changeoperat | 变动操作 | MulBasedataField | t_haos_cschangeoperat（子表） |  |  |
