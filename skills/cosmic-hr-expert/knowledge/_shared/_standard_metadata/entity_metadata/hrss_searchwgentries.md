...
## 实体: hrss_searchwgentries（搜索权重等级分录） [BaseEntity]

- **数据库表**: `t_hrss_searchwgentry`  
...
| name | 权重等级 | MuliLangTextField | fname |  |  |
...
| scenceid | 关联场景权重等级id | BigIntField | fid |  |  |
| index | 排序值 | IntegerField | fseq |  |  |
| weightvalue | 计分数值 | DecimalField | fweightvalue |  |  |
