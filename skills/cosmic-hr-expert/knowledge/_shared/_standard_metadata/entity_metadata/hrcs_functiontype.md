...
## 实体: hrcs_functiontype（公式函数分类） [BaseEntity]

- **数据库表**: `t_hrcs_functiontype`  
...
| businessfield | 适用HR领域 | MulBasedataField | t_hrcs_functiontypehrbf（子表） |  |  |
| appscope | 适用应用范围 | MulBasedataField | t_hrcs_fttypeappscope（子表） |  |  |
| rgfscopetype | 适用范围按钮组 | RadioGroupField | fscopetype |  |  |
| general | HR通用 | RadioField | — |  |  |
| customize | 指定范围 | RadioField | — |  |  |
