...
## 实体: hrcs_strategy（基础资料_策略类型） [BaseEntity]

- **数据库表**: `t_hbss_strategy`  
...
| name | 策略名称 | MuliLangTextField | fname |  |  |
...
| bussinessfield | 业务类型 | BasedataField | fbussinessfieldid | ✓ | hrcs_bussinesstype |
| classname | 处理类 | TextField | fhandleclass |  |  |
| strategytype | 策略类别 | TextField | fstrategytype |  |  |
| isupgrade | 是否升级 | CheckBoxField | fisupgrade |  |  |
