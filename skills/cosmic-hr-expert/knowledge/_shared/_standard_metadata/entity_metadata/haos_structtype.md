...
## 实体: haos_structtype（架构类型） [BaseEntity]

- **数据库表**: `t_haos_otclassify`  
...
| isstandardperm | 是否使用标准团队权限 | CheckBoxField | fisstandardperm |  |  |
| iscustom | 是否自定义 | ComboField | fiscustom |  |  |
| effdt | 生效日期 | DateField | feffdt | ✓ |  |
| org | 创建组织 | OrgField | forgid | ✓ | bos_org |
| metanumsuffix | 元数据编号后缀 | TextField | fmetanumsuffix |  |  |
