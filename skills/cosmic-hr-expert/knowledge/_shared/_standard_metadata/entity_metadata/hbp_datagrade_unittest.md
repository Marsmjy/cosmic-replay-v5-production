...
## 实体: hbp_datagrade_unittest（数据分级单元测试元数据） [BaseEntity]

- **数据库表**: `t_hbp_datagrade_unittest`  
...
| matchingmethod | 匹配方法 | ComboField | fmatchingmethod |  |  |
| scalerule | 数据精度尾差处理 | ComboField | fscalerule |  |  |
| datagradeparam | 数据分级表 | TextField | fdatagradeparam |  |  |
| businessfield | 业务领域(业务需要自行添加默认值) | BasedataField | fbusinessfield |  | hbss_hrbusinessfield |
