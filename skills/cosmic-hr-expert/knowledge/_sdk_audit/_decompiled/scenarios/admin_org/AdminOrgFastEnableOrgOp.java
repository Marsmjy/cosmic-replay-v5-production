...
public class AdminOrgFastEnableOrgOp
extends HRDataBaseOp {
    public void beginOperationTransaction(BeginOperationTransactionArgs args) {
        ...
        OperationResult operationResult = new AdminOrgDetailHelper().executeOperate("confirmchange", adminOrgs, "enableorg", operateOption);
        ...
        new AdminOrgDetailHelper().adminOrgCoopHandle("enableorg", args.getDataEntities());
    }

    public void afterExecuteOperationTransaction(AfterOperationArgs args) {
        new AdminChangeMsgService().handleChangeMsg();
        Date now = HRDateTimeUtils.getNowDate();
        AdminOrgUnitSyncService.getInstance().disableOrEnableOrgChgUnit((DynamicObject[])Arrays.stream(args.getDataEntities()).filter(dyn -> !HRDateTimeUtils.dayAfter((Date)dyn.getDate("bsed"), (Date)now)).toArray(DynamicObject[]::new), "enable");
        ...
        OrgStrategyService.getInstance().enableOrgStrategy(adminOrgIdList);
    }
    ...
    private void copyBizField(DynamicObject adminOrg, DynamicObject oldAdminOrg) {
        ...
        adminOrg.set("disabledate", null);
        adminOrg.set("disabler", null);
        adminOrg.set("enable", (Object)"1");
        if (adminOrg.getBoolean("tobedisableflag")) {
            adminOrg.set("tobedisabledate", adminOrg.get("bsed"));
        } else {
