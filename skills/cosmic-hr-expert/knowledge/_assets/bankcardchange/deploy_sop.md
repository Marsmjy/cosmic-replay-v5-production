# 银行卡变更申请(移动端) (bankcardchange) · 部署 SOP
...
python scripts/assemble_asset.py \
    --asset bankcardchange \
    --isv-flag bjss \
    --biz-app bjss_hsas_ext \
    --output D:/myproject/bankcardchange-bjss/
```
...
# 如已有同名 → 必须合并·不能覆盖
diff <现场 ext>.dym D:/myproject/bankcardchange-bjss/datamodel/.../*.dym
```
...
```bash
cd D:/myproject/bankcardchange-bjss
gradle build
...
| 项 | 内容 |
|---|---|
| 新增数据表 | bjss_bankcardchange 主表 + 语言表 |
