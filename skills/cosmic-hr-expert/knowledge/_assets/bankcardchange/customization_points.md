# 银行卡变更申请(移动端) (bankcardchange) · 扩展点（Customization Points）
...
|---|---|---|---|---|
| EP-01 | 改 AppflgConstant 行为 | AppflgConstant.java | ⭐⭐ | (按需) |
| EP-02 | 改 ChangeBankCardBillPlugin 行为 | ChangeBankCardBillPlugin.java | ⭐⭐ | (按需) |
| EP-03 | 改 ChangeBankCardOperationPlugin 行为 | ChangeBankCardOperationPlugin.java | ⭐⭐ | (按需) |
| EP-04 | 改 AcctModifyBankByPayDetailOperatePlugin 行为 | AcctModifyBankByPayDetailOperatePlugin.java | ⭐⭐ | (按需) |

---

## EP-01 · 改 AppflgConstant 行为
...
AppflgConstant 继承 独立·扩展时按 SDK 白名单 @Override 重写
...
## EP-02 · 改 ChangeBankCardBillPlugin 行为
...
`ChangeBankCardBillPlugin.java`

### 改造方案

ChangeBankCardBillPlugin 继承 AbstractMobBillPlugIn·扩展时按 SDK 白名单 @Override 重写
...
## EP-03 · 改 ChangeBankCardOperationPlugin 行为
...
`ChangeBankCardOperationPlugin.java`

### 改造方案

ChangeBankCardOperationPlugin 继承 AbstractOperationServicePlugIn·扩展时按 SDK 白名单 @Override 重写
...
## EP-04 · 改 AcctModifyBankByPayDetailOperatePlugin 行为
...
`AcctModifyBankByPayDetailOperatePlugin.java`

### 改造方案

AcctModifyBankByPayDetailOperatePlugin 继承 AbstractOperationServicePlugIn·扩展时按 SDK 白名单 @Override 重写
