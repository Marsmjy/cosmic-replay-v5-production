# 社保代缴单 (sit_socialsecurity) · 部署 SOP
...
|---|---|---|
| 苍穹版本 | v1.5+ | 后台 → 系统信息 |
| 所属云 / 应用 | payroll / sit 已部署 | 应用管理 → 检查 |
| ISV 开发商标识 | 客户已选定（如 bjss） | 与客户确认 |
| ISV 应用编码 | 客户已分配（如 bjss_sit_ext） | 苍穹 → 应用管理 |
...
python scripts/assemble_asset.py \
    --asset sit_socialsecurity \
    --isv-flag bjss \
    --biz-app bjss_sit_ext \
    --output D:/myproject/sit_socialsecurity-bjss/
```
...
# 如已有同名 → 必须合并·不能覆盖
diff <现场 ext>.dym D:/myproject/sit_socialsecurity-bjss/datamodel/.../*.dym
```
...
```bash
cd D:/myproject/sit_socialsecurity-bjss
gradle build

# 上传 jar 到客户苍穹环境
cp build/libs/bjss_sit_ext-1.0.jar /客户/cosmic/lib/
...
| 项 | 内容 |
|---|---|
| 新增数据表 | bjss_sit_socialsecurity 主表 + 语言表 |
| 修改标品表 | 走 ext 扩展元数据·**不直接 alter table** |
| 新增 jar | bjss_sit_ext-1.0.jar |
| 新增菜单 | (按需手动挂) |
