# 计算前校验在途单据 (beforecomputationcheck) · 扩展点（Customization Points）

> 3 个标准扩展点·按客户需求定制·**不改资产骨架**
...
| EP-02 | 改 CalPayPushPlugin 行为 | CalPayPushPlugin.java | ⭐⭐ | (按需) |
| EP-03 | 改 HsasCalpayRolltListPlugin 行为 | HsasCalpayRolltListPlugin.java | ⭐⭐ | (按需) |
...
`CalPayPushPlugin.java`

### 改造方案

CalPayPushPlugin 继承 AbstractFormPlugin·扩展时按 SDK 白名单 @Override 重写
...
## EP-03 · 改 HsasCalpayRolltListPlugin 行为
...
`HsasCalpayRolltListPlugin.java`

### 改造方案

HsasCalpayRolltListPlugin 继承 AbstractListPlugin·扩展时按 SDK 白名单 @Override 重写
