# 组织信息变更审核(变更前后信息) (adminorgbill_extension) · 扩展点（Customization Points）

> 4 个标准扩展点·按客户需求定制·**不改资产骨架**
...
|---|---|---|---|---|
| EP-01 | 改 AdminOrgBatchDetailDealSourceInfoFormPlugin 行为 | AdminOrgBatchDetailDealSourceInfoFormPlugin.java | ⭐⭐ | (按需) |
| EP-02 | 改 AdminOrgShowMobBillPlugin 行为 | AdminOrgShowMobBillPlugin.java | ⭐⭐ | (按需) |
| EP-03 | 改 OrgBatchBillSubmitDealSourceInfoOp 行为 | OrgBatchBillSubmitDealSourceInfoOp.java | ⭐⭐ | (按需) |
| EP-04 | 改 OrgBatchBillInfoHelper 行为 | OrgBatchBillInfoHelper.java | ⭐⭐ | (按需) |

---

## EP-01 · 改 AdminOrgBatchDetailDealSourceInfoFormPlugin 行为
...
`AdminOrgBatchDetailDealSourceInfoFormPlugin.java`

### 改造方案

AdminOrgBatchDetailDealSourceInfoFormPlugin 继承 AbstractFormPlugin·扩展时按 SDK 白名单 @Override 重写
...
## EP-02 · 改 AdminOrgShowMobBillPlugin 行为
...
`AdminOrgShowMobBillPlugin.java`

### 改造方案

AdminOrgShowMobBillPlugin 继承 AbstractMobBillPlugIn·扩展时按 SDK 白名单 @Override 重写
...
## EP-03 · 改 OrgBatchBillSubmitDealSourceInfoOp 行为
...
`OrgBatchBillSubmitDealSourceInfoOp.java`

### 改造方案

OrgBatchBillSubmitDealSourceInfoOp 继承 HRCoreBaseBillOp·扩展时按 SDK 白名单 @Override 重写
...
## EP-04 · 改 OrgBatchBillInfoHelper 行为
...
`OrgBatchBillInfoHelper.java`

### 改造方案

OrgBatchBillInfoHelper 继承 独立·扩展时按 SDK 白名单 @Override 重写
