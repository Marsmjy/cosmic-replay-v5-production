# 信息采集校验配置工具 (home_check_rule) · 扩展点（Customization Points）

> 2 个标准扩展点·按客户需求定制·**不改资产骨架**
...
|---|---|---|---|---|
| EP-01 | 改 CollectInfoRuleMobFormPlugin 行为 | CollectInfoRuleMobFormPlugin.java | ⭐⭐ | (按需) |
| EP-02 | 改 CollectInfoRuleUtil 行为 | CollectInfoRuleUtil.java | ⭐⭐ | (按需) |

---

## EP-01 · 改 CollectInfoRuleMobFormPlugin 行为
...
`CollectInfoRuleMobFormPlugin.java`

### 改造方案

CollectInfoRuleMobFormPlugin 继承 AbstractFormPlugin·扩展时按 SDK 白名单 @Override 重写
...
## EP-02 · 改 CollectInfoRuleUtil 行为
...
`CollectInfoRuleUtil.java`

### 改造方案

CollectInfoRuleUtil 继承 独立·扩展时按 SDK 白名单 @Override 重写
