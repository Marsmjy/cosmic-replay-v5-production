...
python scripts/assemble_asset.py \
    ...
    --output D:/myproject/swc_bonus_new-bjss/
```
...
# 如已有同名 → 必须合并·不能覆盖
diff <现场 ext>.dym D:/myproject/swc_bonus_new-bjss/datamodel/.../*.dym
```
...
```bash
cd D:/myproject/swc_bonus_new-bjss
gradle build
...
| 项 | 内容 |
|---|---|
| 新增数据表 | bjss_swc_bonus_new 主表 + 语言表 |
