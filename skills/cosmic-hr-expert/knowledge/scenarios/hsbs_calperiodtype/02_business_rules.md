...
## ✅ verified · 主表 `hsbs_calperiodtype` 共 56 个 opKey
...
| 7 | `CalPeriodSaveOp` | enabled=True |
...
| 5 | `CalPeriodSaveOp` | enabled=True |
| 6 | `HRBaseDataEnableOp` | HR 基础资料启用态管理 · 控制 enable 字段 |
...
| 3 | `CalPeriodSaveOp` | enabled=True |
...
- `auto_assign` · `tbl_assign_import` · `logview` · `viewonelog` · `batchgenerate` · `newentry` · `deleteentry` · `donothing_editentry` · `setentryenable` · `donothing_maintenance`
