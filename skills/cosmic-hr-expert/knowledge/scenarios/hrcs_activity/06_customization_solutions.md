...
在开发平台给本 form（`hrcs_activity`）加 ISV 字段：
...
标品 `ActivityDeleteOp` 在 `onAddValidators` 注入了：
...
@SdkPlugin
public class MyActivityIsvOp extends HRDataBaseOp {
    @Override
...
```java
@SdkPlugin
public class MyActivityAfterOp extends HRDataBaseOp {
    @Override
...
标品 `ActivityEditPlugin` 已实现：
- `registerListener` (L41)
- `afterBindData` (L47)
- `propertyChanged` (L68)
- `beforeF7Select` (L116)
...
```java
@SdkPlugin
public class MyActivityIsvFormPlugin extends HRDataBaseEdit {
    @Override
...
本场景反编译类调用的 Service / Helper（4 个 · 仅前 10）：

- `ActivityDeleteValidator`
- `ActivityDisableValidator`
- `ActivitySaveValidator`
- `HRBaseServiceHelper`
