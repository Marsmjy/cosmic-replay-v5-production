...
## ✅ verified · 主表 `wtis_punchcarddata` 共 17 个 opKey
...
| order | className | 角色 |
|:---:|---|---|
| 1 | `PunchCardDataOp` | enabled=True |
...
#### `manualsyn` · 手动同步（opType=donothing）

**执行链（1 个 plugin · 按 order 排）**：

| order | className | 角色 |
|:---:|---|---|
| 1 | `PunchCardDataSyncOp` | enabled=True |
...
#### `autosysn` · 同步原始卡（opType=donothing）

**执行链（1 个 plugin · 按 order 排）**：

| order | className | 角色 |
|:---:|---|---|
| 1 | `PunchCardAutoSynOp` | enabled=True |
...
### 其他 opKey（14 个无独立规则·复用主链或反编译为空）

- `importdata_hr` · `show_import_record_hr` · `export_from_list_hr` · `export_from_impttpl_hr` · `export_from_expttpl_hr` · `show_export_record_hr` · `first` · `previous` · `next` · `last` · `refresh` · `close`
- `exportlistbyselectfields` · `exportdetails`
