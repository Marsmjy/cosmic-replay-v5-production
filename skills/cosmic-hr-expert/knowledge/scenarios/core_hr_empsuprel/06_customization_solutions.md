# core_hr_empsuprel · 定制方案（ISV 扩展真实模板）
...
在开发平台给本 form（`hrpi_empsuprel`）加 ISV 字段：
...
```java
@SdkPlugin
public class MyEmpsuprelAfterOp extends HRDataBaseOp {
    @Override
...
```java
@SdkPlugin
public class MyEmpsuprelIsvFormPlugin extends HRDataBaseEdit {
    @Override
...
本场景反编译类调用的 Service / Helper（13 个 · 仅前 10）：
