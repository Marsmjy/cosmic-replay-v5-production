...
- `EmpSupRelExpireValidator`

**调用的 Service / Helper**（1 个）：`EmpSupRelExpireValidator`

**实现的生命周期方法**：`onAddValidators`(L46), `beginOperationTransaction`(L51)
...
**触发的 Validator**（save/delete 前置校验）：
- `EmpSupRelEmpposorelDateValidator`
- `EmpSupRelEmpposorelValidator`
- `EmpSupRelReportTypeValidator`
- `EmpSupRelSuperiorValidator`

**调用的 Service / Helper**（4 个）：`EmpSupRelEmpposorelDateValidator`, `EmpSupRelEmpposorelValidator`, `EmpSupRelReportTypeValidator`, `EmpSupRelSuperiorValidator`

**实现的生命周期方法**：`onAddValidators`(L63), `beginOperationTransaction`(L71)
...
- 数据源：`_sdk_audit/_decompiled/scenarios/core_hr_empsuprel/` 的 13 个反编译类 + `_shared/_standard_metadata/entity_metadata/hrpi_empsuprel.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.hrpi.opplugin.web.assignment.attach.EmpSupRelSaveOp -->

## chgaction 实证补充（EmpSupRelSaveOp 跨类追踪聚合）
...
| `KDDateUtils_0` | %s不是有效的当前时区时间格式 |
| `PersonServiceImpl_5` | 请传入有效校验值。 |
| `PersonRoleServiceImpl_6` | 查询上级行政组织异常。 |

### 调用的核心 Service（Top 10）
- `EmpSupRelDomainServiceImpl.class`
- `EmpSupRelDomainServiceImpl.querySuperiorByOrg`
- `EmpSupRelDomainServiceImpl.lambda`
- `EmpSupRelDomainServiceImpl.querySuperiors`
- `EmpSupRelDomainServiceImpl.removeDupDepEmpId`
- `ChargeDomainServiceImpl.getMaxDate`
- `ChargeDomainServiceImpl.getMinDate`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.hrpi.opplugin.web.assignment.attach.EmpSupRelExpireOp -->

## chgaction 实证补充（EmpSupRelExpireOp 跨类追踪聚合）
