...
主表 `wtp_overworkrule` 标品接入 **51** 个插件。
...
- `OtRuleOp` ← `kd.wtc.wtp.opplugin.web.overtime.OtRuleOp`
...
- order **9**: `HisModelOPCommonPlugin` · 时态模型通用 · his_bsed/his_bsled/effectdate 的维护
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisModelOPCommonPlugin`
- order **10**: `OtRuleOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.overtime.OtRuleOp`
