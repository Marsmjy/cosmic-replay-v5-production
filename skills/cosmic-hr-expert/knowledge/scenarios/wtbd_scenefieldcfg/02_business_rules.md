...
## ✅ verified · 主表 `wtbd_scenefieldcfg` 共 41 个 opKey
...
| 5 | `TreeBaseDataSavePlugin` | enabled=True |
| 6 | `BaseDataSavePlugin` | enabled=True |
| 7 | `HRBaseDataStatusOp` | HR 基础资料单据状态（draft/audit/disable） |
| 8 | `HRBaseDataLogOp` | HR 基础资料操作日志 |
| 9 | `HRBaseDataEnableOp` | HR 基础资料启用态管理 · 控制 enable 字段 |
| 10 | `SceneFieldCfgMustInputOp` | enabled=False |
| 11 | `HRBaseOriginalOp` | HR 基础资料原始值记录（对比变更前后） |
| 12 | `RetrievalItemOp` | enabled=True |
...
| order | className | 角色 |
|:---:|---|---|
| 1 | `BaseDataSubmitPlugin` | enabled=True |
| 2 | `CodeRuleOp` | 编码规则（未启用时跳过）· onAddValidators 注册 numberValidator |
| 3 | `BdVersionSaveServicePlugin` | 基础资料版本管理 · 主表 name + 版本子表 name 的写历史 |
| 4 | `BaseTreeOp` | enabled=True |
| 5 | `BaseTreeLongNumberOp` | enabled=True |
| 6 | `HRBaseDataLogOp` | HR 基础资料操作日志 |
| 7 | `HRBaseDataEnableOp` | HR 基础资料启用态管理 · 控制 enable 字段 |
| 8 | `HRBaseOriginalOp` | HR 基础资料原始值记录（对比变更前后） |
| 9 | `SceneFieldCfgMustInputOp` | enabled=True |
...
| 2 | `BaseTreeDeleteValidate` | enabled=False |
| 3 | `BaseDataTreeDeletePlugin` | enabled=True |
| 4 | `CodeRuleDeleteOp` | enabled=True |
| 5 | `HRBaseDataStatusOp` | HR 基础资料单据状态（draft/audit/disable） |
| 6 | `HRBaseDataLogOp` | HR 基础资料操作日志 |
| 7 | `RetrievalItemOp` | enabled=True |
...
| 2 | `BaseDataEnablePlugin` | enabled=True |
...
### 其他 opKey（31 个无独立规则·复用主链或反编译为空）

- `new` · `modify` · `view` · `close` · `returndata` · `copy` · `refresh` · `option` · `submitandnew` · `first` · `previous` · `next`
- `last` · `importtemplatelist` · `mobtoolbarselect` · `mobtoolbarcancel` · `namehistory` · `namehistoryview` · `assign` · `unassign` · `bdctrlchange` · `individuation` · `assign_new` · `auto_assign`
- `tbl_assign_import` · `logview` · `viewonelog` · `newentry` · `deleteentry` · `moveentryup` · `moveentrydown`
