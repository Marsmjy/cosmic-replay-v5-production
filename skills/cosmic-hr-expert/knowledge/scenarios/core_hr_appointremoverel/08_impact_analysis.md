# 变更影响面 · core_hr_appointremoverel
...
- ❌ 禁继承 HisModelOPCommonPlugin（@SdkInternal · 平台内部类 · ISV 不允许继承）（出现于 opKey=`importdata_hr`）
- ❌ 禁继承 HisUniqueValidateOp（@SdkInternal · 平台内部类）（出现于 opKey=`importdata_hr`）
- ❌ 禁继承 HisModelListCommonPlugin（@SdkInternal · 历史模型列表内部类）（出现于 opKey=`importdata_hr`）
- ❌ 禁继承 AbsOrgBaseOp（HR 通用推荐 HRDataBaseOp · 见 cosmic_hr_sdk_whitelist_audit.md）（出现于 opKey=`importdata_hr`）
