...
**历史版本流转**（HisModel）：1 个 opKey

- `newhisversion` 新增数据版本 → 写 `t_*_h` 历史表 + 关闭旧版
- `revise` 修订 → 不增版本，直接覆盖当前版
- `versionchangecompare` 版本对比

## ✅ verified · OP 类执行链（反编译实证）

反编译共发现 **10** 个 OP 类，按命名推断生命周期：

| OP 类 | 推断阶段 |
|---|---|
| `AssignmentTplCommonSaveOp` | 保存 |
| `ChgRecordSaveOp` | 保存 |
| `EmployeeCommonStandardMustInputOp` | 通用 |
| `IgnoreReferenceDeleteOp` | 删除 |
| `TerminationInfoEmpStageOp` | 通用 |
| `TerminationInfoSaveOp` | 保存 |
| `TerminationReviseDepartDateOp` | 通用 |
| `TimeLineCommonSaveOp` | 保存 |
| `TimelineLogOp` | 通用 |
| `TimelineTplOp` | 通用 |
