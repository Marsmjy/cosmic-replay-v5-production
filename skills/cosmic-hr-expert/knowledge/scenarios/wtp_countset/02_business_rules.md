...
## ✅ verified · 主表 `wtp_countset` 共 57 个 opKey
...
| 6 | `SuppleCountLimitOp` | enabled=True |
...
| 5 | `SuppleCountLimitOp` | enabled=True |
...
#### `submitandaudit` · 提交并生效（opType=submit）
...
| 3 | `SuppleCountLimitOp` | enabled=True |
...
- `auto_assign` · `tbl_assign_import` · `logview` · `viewonelog` · `selmodify` · `deleteentry` · `addentry` · `editentry` · `newentry` · `moveentryup` · `moveentrydown`
