...
**调用的 Service / Helper**（3 个）：`HRBaseServiceHelper`, `SceneServiceHelper`, `SceneValidator`

**实现的生命周期方法**：`onAddValidators`(L67), `beginOperationTransaction`(L71)
...
- **继承**：`HRDataBaseEdit`
- **生命周期方法**：`registerListener`(L148), `afterBindData`(L89), `beforeDoOperation`(L112)
- **读字段**（15）：`id`, `inputdateformat`, `inputdynprop`, `inputissyspreset`, `inputmultiple`, `inputname`, `inputnumber`, `inputobject`

### `SceneListPlugin`

- **FQN**：`kd.hr.brm.formplugin.web.SceneListPlugin`
- **继承**：`HRF7TreeListPlugin`
- **生命周期方法**：`registerListener`(L48), `beforeDoOperation`(L64)
...
- 数据源：`_sdk_audit/_decompiled/scenarios/brm_scene/` 的 3 个反编译类 + `_shared/_standard_metadata/entity_metadata/brm_scene.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hr.brm.formplugin.web.SceneConfigPlugin -->

## chgaction 实证补充（SceneConfigPlugin 跨类追踪聚合）
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hr.brm.formplugin.web.SceneListPlugin -->

## chgaction 实证补充（SceneListPlugin 跨类追踪聚合）
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hr.brm.opplugin.web.SceneConfigOp -->

## chgaction 实证补充（SceneConfigOp 跨类追踪聚合）
...
| 实体 | 操作 | 入口类 | 来源链 |
|---|---|---|---|
| `brm_scene_his` | deleteByFilter | HRBaseServiceHelper | <self> (depth=0) |
