...
在开发平台给本 form（`brm_scene`）加 ISV 字段：
...
标品 `SceneConfigOp` 在 `onAddValidators` 注入了：
- `SceneValidator`
...
@SdkPlugin
public class MySceneIsvOp extends HRDataBaseOp {
    @Override
...
```java
@SdkPlugin
public class MySceneAfterOp extends HRDataBaseOp {
    @Override
...
标品 `SceneConfigPlugin` 已实现：
- `registerListener` (L148)
- `afterBindData` (L89)
- `beforeDoOperation` (L112)

ISV **并列挂**（PR-001 · 不要继承标品 Edit）：

```java
@SdkPlugin
public class MySceneIsvFormPlugin extends HRDataBaseEdit {
    @Override
