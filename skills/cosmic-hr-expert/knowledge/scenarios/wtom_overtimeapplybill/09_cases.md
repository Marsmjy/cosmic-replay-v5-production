# ISV 实战 case · wtom_overtimeapplybill
...
### Case 1：扩展 `HRPermCommonEdit` (opKey=`save`)

**业务背景**：标品 `HRPermCommonEdit` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hr.hbp.formplugin.web` 包
2. 写 ISV 子类继承 `HRPermCommonEdit`：
```java
...
### Case 4：扩展 `HRCodeRuleOp` (opKey=`submiteffect`)

**业务背景**：标品 `HRCodeRuleOp` 处理 `submiteffect` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hr.hbp.opplugin.web` 包
2. 写 ISV 子类继承 `HRCodeRuleOp`：
```java
...
4. 单测：触发 opKey `submiteffect` · 验证 ISV 逻辑 + super 行为都跑
...
### opKey `export_from_impttpl_hr` · 导出数据（按导入模板）
...
### opKey `show_export_record_hr` · 查看导出结果
...
### opKey `export_from_expttpl_hr` · 导出数据（按导出模板）
