# core_hr_transfer · 定制方案（ISV 扩展真实模板）

> **场景**：调配管理 - 调动（hdm · transferapply 等 10 单据）（2 个子实体）
> **生成时间**：2026-04-30

## CS-01 · 给子实体加 ISV 自定义字段

**适用**：`hdm_transferapply`, `hdm_transferbatch`（任一子实体）
...
**真发证据 / 反编译示例（如有）**：
- `TransferAfterEffectOp.java` (5521 bytes)
- `TransferEffectOp.java` (21441 bytes)
- `TransferSaveOp.java` (4907 bytes)
