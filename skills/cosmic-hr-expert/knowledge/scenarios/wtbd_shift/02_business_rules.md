...
## ✅ verified · 主表 `wtbd_shift` 共 73 个 opKey
...
| 10 | `ShiftHRImportSaveOp` | enabled=True |
...
### 其他 opKey（62 个无独立规则·复用主链或反编译为空）

- `importdata_hr` · `show_import_record_hr` · `export_from_list_hr` · `export_from_impttpl_hr` · `export_from_expttpl_hr` · `show_export_record_hr` · `modify` · `view` · `close` · `returndata` · `copy` · `refresh`
...
- `tbl_assign_import` · `logview` · `viewonelog` · `hisversioninfo` · `change` · `hiscopy` · `orgpermchange` · `newhisversion` · `revise` · `reviserecord` · `versionchangecompare` · `showallversion`
- `newentry` · `deleteentry` · `tablnew` · `taledit` · `edit_save` · `submithis` · `new` · `genshift` · `newentry1` · `deleteentry1` · `newentry2` · `deleteentry2`
