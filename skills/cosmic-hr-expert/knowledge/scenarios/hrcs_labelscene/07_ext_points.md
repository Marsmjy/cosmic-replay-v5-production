...
主表 `hrcs_labelscene` 标品接入 **19** 个插件。
...
- `LabelSceneFormPlugin` ← `kd.hr.hrcs.formplugin.web.label.LabelSceneFormPlugin`
...
- `LabelSceneTreeListPlugin` ← `kd.hr.hrcs.formplugin.web.label.LabelSceneTreeListPlugin`
...
- `LabelSceneEditOp` ← `kd.hr.hrcs.opplugin.web.label.LabelSceneEditOp`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.hrcs.formplugin.web.label.LabelSceneFormPlugin -->

## ISV 扩展指引（基于 LabelSceneFormPlugin 真实证）

> FQN: `kd.hr.hrcs.formplugin.web.label.LabelSceneFormPlugin`
> 反编译产物: `_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.label.LabelSceneFormPlugin/`

### 类型与继承
- 插件类型：**FORM_PLUGIN**
- 父类: `kd.bos.form.plugin.AbstractFormPlugin`
- ISV 可继承: ✅ 推荐继承本父类
- 推荐挂载方法: `registerListener`, `afterDoOperation`, `preOpenForm`, `afterBindData`, `propertyChanged`, `beforeBindData`, `beforeDoOperation`, `beforeF7Select`, `closedCallBack`
...
**QUERY_BUILDER** · LabelSceneFormPlugin L290
```java
 288           ListShowParameter fsp = ShowFormHelper.createShowListForm((String)"hrcs_label", (boolean)true, (int)0, (boolean)true);
 289           fsp.setCloseCallBack(new CloseCallBack((IFormPlugin)this, "hrcs_label"));
 290 >         QFilter filter = new QFilter("entryentityrange.labelobject", "=", labelObject.getPkValue());
 291           filter.and(new QFilter("status", "=", (Object)"C"));
 292           List selectedIds = this.getModel().getEntryEntity(LABEL_ENTRY).stream().map(dy -> dy.getLong(LABEL_ID)).collect(Collectors.toList());
```

**READ_VIA_HELPER** · LabelSceneFormPlugin L159
```java
 157                   continue;
 158               }
 159 >             DynamicObject[] labalPolicyDyns = LabelPolicyServiceHelper.getPolicyDyns((Long)this.getModel().getDataEntity().getDynamicObject("labelobject").getLong("id"), (Long)((DynamicObject)labelEntry.get(idx)).getLong(LABEL_ID));
 160               if (((DynamicObject)labelEntry.get(idx)).getBoolean(KEY_ENTRYENTITY_BIZLABEL)) {
 161                   if (labalPolicyDyns == null || labalPolicyDyns.length == 0) {
```

详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.label.LabelSceneFormPlugin/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.label.LabelSceneFormPlugin/extension_guide.md)
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.hrcs.formplugin.web.label.LabelSceneTreeListPlugin -->

## ISV 扩展指引（基于 LabelSceneTreeListPlugin 真实证）

> FQN: `kd.hr.hrcs.formplugin.web.label.LabelSceneTreeListPlugin`
> 反编译产物: `_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.label.LabelSceneTreeListPlugin/`

### 类型与继承
- 插件类型：**LIST_PLUGIN**
- 父类: `kd.hr.hbp.formplugin.web.template.HRF7TreeListPlugin`
- ISV 可继承: ⚠️ 标品 @SdkInternal·改继承基类
- 推荐挂载方法: `registerListener`, `setFilter`, `beforeItemClick`, `closedCallBack`
...
**QUERY_BUILDER** · LabelSceneTreeListPlugin L105
```java
 103               if (null != focusedIdObject) {
 104                   String focusedId = (String)focusedIdObject;
 105 >                 QFilter[] filters = new QFilter[]{new QFilter("id", "=", (Object)focusedId)};
 106                   if (bosAppServiceHelper.isExists(filters)) {
 107                       fsp.setCustomParam("biz_app_id", (Object)focusedId);
```

详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.label.LabelSceneTreeListPlugin/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.label.LabelSceneTreeListPlugin/extension_guide.md)
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.hrcs.opplugin.web.label.LabelSceneEditOp -->

## ISV 扩展指引（基于 LabelSceneEditOp 真实证）

> FQN: `kd.hr.hrcs.opplugin.web.label.LabelSceneEditOp`
> 反编译产物: `_sdk_audit/_decompiled_deep/kd.hr.hrcs.opplugin.web.label.LabelSceneEditOp/`
...
详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.hrcs.opplugin.web.label.LabelSceneEditOp/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.hrcs.opplugin.web.label.LabelSceneEditOp/extension_guide.md)
