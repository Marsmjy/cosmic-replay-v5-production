# hrcs_promptrule · 定制方案（ISV 扩展真实模板）

> **form**：`hrcs_promptrule`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`hrcs_promptrule`）加 ISV 字段：
...
标品 `PromptRuleBatchImportSaveOp` 在 `onAddValidators` 注入了：
...
@SdkPlugin
public class MyPromptruleIsvOp extends HRDataBaseOp {
    @Override
...
```java
@SdkPlugin
public class MyPromptruleAfterOp extends HRDataBaseOp {
    @Override
...
标品 `PromptRuleEditPlugin` 已实现：
- `registerListener` (L125)
- `propertyChanged` (L270)
- `beforeF7Select` (L154)
- `beforeDoOperation` (L295)
- `click` (L136)
- `itemClick` (L372)
...
```java
@SdkPlugin
public class MyPromptruleIsvFormPlugin extends HRDataBaseEdit {
    @Override
...
本场景反编译类调用的 Service / Helper（2 个 · 仅前 10）：
