...
## ✅ verified · 主表 `wtbd_specialdate` 共 43 个 opKey
...
| 7 | `WTCNumberCheckOp` | enabled=True |
| 8 | `HRBaseOriginalOp` | HR 基础资料原始值记录（对比变更前后） |
| 9 | `SpecialDateSaveOp` | enabled=True |
...
- `tbl_assign_import` · `newentry` · `deleteentry` · `moveentryup` · `moveentrydown` · `orgpermchange` · `tbl_clearcache` · `tblassign_search` · `tblunassign_search` · `tblassign_searchorg`
