...
> **聚合场景**：杂项底表聚合（用户关联/离职类型/再雇佣 3 实体）（3 个子实体）
> **生成时间**：2026-04-30
...
**标品公共字段**：`creator`, `createtime`, `modifier`, `modifytime`, `enable`
...
- `creator` (创建人) → `bos_user` · `CreaterField`
- `modifier` (修改人) → `bos_user` · `ModifierField`
- `group` (离职类型分组) → `bos_assistantdata_detail` · `GroupField`
...
### `hrpi_reemploymentrel` · ⚠️ 标品 entity_metadata md 不存在
