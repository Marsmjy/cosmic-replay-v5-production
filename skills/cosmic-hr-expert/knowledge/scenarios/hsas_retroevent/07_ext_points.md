...
- `RetroEventEdit` ← `kd.swc.hsas.formplugin.web.cal.retro.RetroEventEdit`

### ListPlugin（2）
- `HRCertCheckList` ← `kd.hr.hbp.formplugin.web.cert.HRCertCheckList`
- `RetroEventListPlugin` ← `kd.swc.hsas.formplugin.web.cal.retro.RetroEventListPlugin`

### OpPlugin（1）
- `RetroEventAbandonOp` ← `kd.swc.hsas.opplugin.web.cal.RetroEventAbandonOp`
...
## ✅ verified · opKey 扩展切入点详解（1 个有执行链的 opKey）

ISV 想在某个 opKey 加扩展逻辑·按下表选切入位置：

### opKey `donothing_abandon` · 废弃

**执行链**（1 个 plugin 按 order 顺序跑）：

- order **1**: `RetroEventAbandonOp` · enabled=True
  - 完整 FQN: `kd.swc.hsas.opplugin.web.cal.RetroEventAbandonOp`
