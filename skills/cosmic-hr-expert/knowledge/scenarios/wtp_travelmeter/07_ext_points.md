...
主表 `wtp_travelmeter` 标品接入 **55** 个插件。
...
- ...（共 30，详见 `_auto_plugin_registry.md`）
...
- `TripConfigMustInputOp` ← `kd.wtc.wtp.opplugin.web.tp.TripConfigMustInputOp`
...
- order **9**: `HisModelOPCommonPlugin` · 时态模型通用 · his_bsed/his_bsled/effectdate 的维护
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisModelOPCommonPlugin`
- order **10**: `TripConfigMustInputOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.tp.TripConfigMustInputOp`
...
- order **8**: `HRBaseOriginalOp` · HR 基础资料原始值记录（对比变更前后）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp`
- order **9**: `TripConfigMustInputOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.tp.TripConfigMustInputOp`
...
- order **2**: `HisUniqueValidateOp` · 时态模型唯一性校验（同一 boid 同一时间段唯一）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisUniqueValidateOp`
- order **3**: `TripConfigMustInputOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.tp.TripConfigMustInputOp`
