...
**必填字段（1）**：
- `postcategory` (任职类型分类) · `BasedataField` → ref hbss_postcategory

**引用基础资料（4）**：
- `creator` (创建人) → `bos_user` · `CreaterField`
- `modifier` (修改人) → `bos_user` · `ModifierField`
- `disabler` (禁用人) → `bos_user` · `UserField`
- `postcategory` (任职类型分类) → `hbss_postcategory` · `BasedataField`
...
**必填字段（1）**：
- `poststatecls` (任职状态分类) · `BasedataField` → ref hbss_poststatecls

**引用基础资料（4）**：
- `creator` (创建人) → `bos_user` · `CreaterField`
- `modifier` (修改人) → `bos_user` · `ModifierField`
- `disabler` (禁用人) → `bos_user` · `UserField`
- `poststatecls` (任职状态分类) → `hbss_poststatecls` · `BasedataField`

**标品公共字段**：`number`, `name`, `status`, `creator`, `modifier`, `enable`, `createtime`, `modifytime`, `masterid`, `issyspreset`

> ⚠️ 这些字段由 HRBaseDataTplEdit 标品维护 · ISV **不可改值**（除特殊场景）

### `hbss_poststatecls` · 任职状态分类
...
- `modifier` (修改人) → `bos_user` · `ModifierField`
- `disabler` (禁用人) → `bos_user` · `UserField`
- `parent` (上级单位) → `hbss_costcenter` · `BasedataField`
...
### `hbss_bussinessfield` · 业务类型

**字段总数**：21

**必填字段（1）**：
- `controlfuntype` (HR职能) · `BasedataField` → ref bos_org_biz

**引用基础资料（5）**：
- `creator` (创建人) → `bos_user` · `CreaterField`
- `modifier` (修改人) → `bos_user` · `ModifierField`
- `group` (业务领域) → `hbss_hrbusinessfield` · `GroupField`
- `disabler` (禁用人) → `bos_user` · `UserField`
- `controlfuntype` (HR职能) → `bos_org_biz` · `BasedataField`
...
### `hbss_industrytype` · ⚠️ 标品 entity_metadata md 不存在
...
### `hbss_cadrecategory` · 干部类别

**字段总数**：20

**必填字段（1）**：
- `categorygrade` (干部类别级别码) · `IntegerField`
...
### `hbss_projecttype` · ⚠️ 标品 entity_metadata md 不存在

### `hbss_rotationtype` · ⚠️ 标品 entity_metadata md 不存在

### `hbss_disptype` · ⚠️ 标品 entity_metadata md 不存在

### `hbss_perflevel` · ⚠️ 标品 entity_metadata md 不存在

### `hbss_depcytype` · ⚠️ 标品 entity_metadata md 不存在
