...
在开发平台给本 form（`lcs_coststru`）加 ISV 字段：
...
```java
@SdkPlugin
public class MyCoststruAfterOp extends HRDataBaseOp {
    @Override
...
- `beforeF7Select` (L203)
- `beforeDoOperation` (L117)
- `afterDoOperation` (L174)

ISV **并列挂**（PR-001 · 不要继承标品 Edit）：

```java
@SdkPlugin
public class MyCoststruIsvFormPlugin extends HRDataBaseEdit {
    @Override
