...
在开发平台给本 form（`hies_taskinfo`）加 ISV 字段：
...
标品 `TaskInfoListPlugin` 已实现：
- `registerListener` (L167)
- `beforeF7Select` (L173)
- `beforeDoOperation` (L353)

ISV **并列挂**（PR-001 · 不要继承标品 Edit）：

```java
@SdkPlugin
public class MyTaskinfoIsvFormPlugin extends HRDataBaseEdit {
    @Override
