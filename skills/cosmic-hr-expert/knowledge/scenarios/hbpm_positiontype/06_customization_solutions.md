...
| `kd.hrmp.hbpm.formplugin.web.basedata.PositionBasedataEdit` | `save` | afterBindData |
...
| `kd.hrmp.hbpm.opplugin.web.basedata.BaseDataBuOp` | `save` | onAddValidators |
...
- `PositionnBaseDataOrderPlugin` (FORM_PLUGIN) ← `kd.hrmp.hbpm.formplugin.web.basedata.PositionnBaseDataOrderPlugin`

**原因**：这些类带 `@SdkInternal` 或 `@SdkPlugin(role=internal)` 注解 · ISV 不得继承。

## ℹ️ 标品已接入插件（36 个 · 详见 07_ext_points.md）
