...
主表 `hcdm_salarystandard` 标品接入 **56** 个插件。
...
- `SWCNewHisBaseDataEdit` ← `kd.swc.hsbp.formplugin.web.newhismodel.SWCNewHisBaseDataEdit`
- `SalaryStandardGeneralInfoEdit` ← `kd.swc.hcdm.formplugin.salarystandard.SalaryStandardGeneralInfoEdit`
- `SalaryStandardDesignEdit` ← `kd.swc.hcdm.formplugin.salarystandard.SalaryStandardDesignEdit`
...
- `SWCNewHisBaseDataList` ← `kd.swc.hsbp.formplugin.web.newhismodel.SWCNewHisBaseDataList`
...
- `HRBaseDataOp` ← `kd.hr.hbp.opplugin.web.config.HRBaseDataOp`
- `SalaryStandardCommonOp` ← `kd.swc.hcdm.opplugin.salarystandard.SalaryStandardCommonOp`
...
## ✅ verified · 反编译实抓真处理类（21 个 · ISV 可继承 11 / 禁继承 10）
...
#### `HRBaseDataOp` · OP_PLUGIN @ `OPERATION` · opKey=`save`

- **完整 FQN**: `kd.hr.hbp.opplugin.web.config.HRBaseDataOp`
- **完整继承链**: `kd.bos.entity.plugin.AbstractOperationServicePlugIn`
...
- **ISV 扩展套路**：
  ...
  public class IsvCustomHRBaseDataOp extends HRBaseDataOp {
  }
  ```
- **详细 ISV 扩展指南**: [`<skills-dir>/cosmic-hr-expert/knowledge/_sdk_audit/_decompiled_deep/kd.hr.hbp.opplugin.web.config.HRBaseDataOp/extension_guide.md`](../../<skills-dir>/cosmic-hr-expert/knowledge/_sdk_audit/_decompiled_deep/kd.hr.hbp.opplugin.web.config.HRBaseDataOp/extension_guide.md)
...
- order **4**: `HRBaseDataStatusOp` · HR 基础资料单据状态（draft/audit/disable）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataStatusOp`
- order **5**: `HRBaseDataOp` · enabled=True
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataOp`
- order **6**: `SalaryStandardCommonOp` · enabled=False
  - 完整 FQN: `kd.swc.hcdm.opplugin.salarystandard.SalaryStandardCommonOp`
...
- order **5**: `HRBaseDataEnableOp` · HR 基础资料启用态管理 · 控制 enable 字段
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataEnableOp`
- order **6**: `HisUniqueValidateOp` · 时态模型唯一性校验（同一 boid 同一时间段唯一）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisUniqueValidateOp`
- order **7**: `HRBaseOriginalOp` · HR 基础资料原始值记录（对比变更前后）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp`
- order **8**: `SalaryStandardCommonOp` · enabled=True
  - 完整 FQN: `kd.swc.hcdm.opplugin.salarystandard.SalaryStandardCommonOp`
- order **9**: `HRBaseDataLogOp` · HR 基础资料操作日志
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp`
...
- order **2**: `HisUniqueValidateOp` · 时态模型唯一性校验（同一 boid 同一时间段唯一）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisUniqueValidateOp`
- order **3**: `SalaryStandardCommonOp` · enabled=True
  - 完整 FQN: `kd.swc.hcdm.opplugin.salarystandard.SalaryStandardCommonOp`
- order **4**: `HRBaseDataLogOp` · HR 基础资料操作日志
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp`
...
### opKey `allvalidator` · 全量保存校验

**执行链**（1 个 plugin 按 order 顺序跑）：

- order **1**: `SalaryStandardCommonOp` · enabled=True
  - 完整 FQN: `kd.swc.hcdm.opplugin.salarystandard.SalaryStandardCommonOp`
