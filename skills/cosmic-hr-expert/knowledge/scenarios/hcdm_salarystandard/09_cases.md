# ISV 实战 case · hcdm_salarystandard
...
### Case 6：扩展 `HRBaseDataOp` (opKey=`save`)

**业务背景**：标品 `HRBaseDataOp` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hr.hbp.opplugin.web.config` 包
2. 写 ISV 子类继承 `HRBaseDataOp`：
```java
...
### Case 7：扩展 `HRBaseDataEnableOp` (opKey=`save`)
