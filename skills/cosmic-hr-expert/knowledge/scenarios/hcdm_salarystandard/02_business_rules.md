...
## ✅ verified · 主表 `hcdm_salarystandard` 共 82 个 opKey

### 🔥 核心 opKey 执行链（11 个有真链）
...
| 5 | `HRBaseDataOp` | enabled=True |
| 6 | `SalaryStandardCommonOp` | enabled=False |
...
| 11 | `HRBaseDataLogOp` | HR 基础资料操作日志 |
...
| 6 | `HisUniqueValidateOp` | 时态模型唯一性校验（同一 boid 同一时间段唯一） |
| 7 | `HRBaseOriginalOp` | HR 基础资料原始值记录（对比变更前后） |
| 8 | `SalaryStandardCommonOp` | enabled=True |
...
| 3 | `SalaryStandardCommonOp` | enabled=True |
| 4 | `HRBaseDataLogOp` | HR 基础资料操作日志 |
...
#### `allvalidator` · 全量保存校验（opType=save）

**执行链（1 个 plugin · 按 order 排）**：

| order | className | 角色 |
|:---:|---|---|
| 1 | `SalaryStandardCommonOp` | enabled=True |
...
- `refresh` · `option` · `submitandnew` · `first` · `previous` · `next` · `last` · `importdata` · `importdetails` · `importtemplatelist` · `mobtoolbarselect` · `mobtoolbarcancel`
- `namehistory` · `namehistoryview` · `assign` · `unassign` · `bdctrlchange` · `individuation` · `assign_new` · `auto_assign` · `tbl_assign_import` · `logview` · `viewonelog` · `hisversioninfo`
- `hiscopy` · `orgpermchange` · `newhisversion` · `revise` · `reviserecord` · `versionchangecompare` · `showallversion` · `donothing_nextpage` · `donothing_lastpage` · `donothing_standardcompare` · `donothing_addsalaryitem` · `newentry`
- `donothing_addcontrastps` · `donothing_rcvstdgrid` · `donothing_rcvcontsetgrid` · `donothing_show_setgraderank` · `donothing_calcandsave` · `donothing_updatestdsetform` · `donothing_updatestdsetrow` · `donothing_delcontsetgrid` · `donothing_clrcontsetgrid` · `donothing_rcvallcontsetgrid` · `donothing_todesignedit` · `donothing_updatecalctype`

## ✅ verified · 反编译类画像（共 21 个真处理类）
...
| `HRBaseDataOp` | OP_PLUGIN | `save` | ✅ |  | [查看](../../<skills-dir>/cosmic-hr-expert/knowledge/_sdk_audit/_decompiled_deep/kd.hr.hbp.opplugin.web.config.HRBaseDataOp/extension_guide.md) |
...
（共 21 · 详见 `_deep_resolve_index.json`）
