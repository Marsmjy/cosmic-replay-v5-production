...
| `oldop` | 旧版按钮标识 | `TextField` | — |
| `newop` | 新版按钮标识 | `TextField` | — |
| `optype` | 操作类型 | `ComboField` | — |
...
- **继承**：`HRDataBaseEdit`
- **生命周期方法**：`registerListener`(L85), `propertyChanged`(L105), `beforeF7Select`(L150), `beforeDoOperation`(L167), `afterDoOperation`(L129), `click`(L92)
- **读字段**（4）：`entity`, `id`, `name`, `number`
- **调用的 Service**：`HRBaseServiceHelper`

### `SwitchRegistryTreeListPlugin`

- **FQN**：`kd.hr.hies.formplugin.SwitchRegistryTreeListPlugin`
- **继承**：`HRF7TreeListPlugin`
- **生命周期方法**：`beforeDoOperation`(L184), `itemClick`(L261)
- **读字段**（2）：`enablestatus`, `name`
- **写字段**（1）：`enablestatus`
- **调用的 Service**：`HRBaseServiceHelper`
...
- 数据源：`_sdk_audit/_decompiled/scenarios/hies_switchregistry/` 的 2 个反编译类 + `_shared/_standard_metadata/entity_metadata/hies_switchregistry.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hr.hies.formplugin.SwitchRegistryEdit -->

## chgaction 实证补充（SwitchRegistryEdit 跨类追踪聚合）
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hr.hies.formplugin.SwitchRegistryTreeListPlugin -->

## chgaction 实证补充（SwitchRegistryTreeListPlugin 跨类追踪聚合）
