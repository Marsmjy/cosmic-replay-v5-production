...
主表 `wtp_swshiftplan` 标品接入 **54** 个插件。
...
- `WTCHisModelBsedOp` ← `kd.wtc.wtbs.opplugin.web.his.WTCHisModelBsedOp`
...
- order **8**: `HRBaseOriginalOp` · HR 基础资料原始值记录（对比变更前后）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp`
- order **9**: `RuleEngingSaveOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.ruleenging.RuleEngingSaveOp`
- order **10**: `WTCHisModelBsedOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtbs.opplugin.web.his.WTCHisModelBsedOp`
...
- order **2**: `HisUniqueValidateOp` · 时态模型唯一性校验（同一 boid 同一时间段唯一）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisUniqueValidateOp`
- order **3**: `WTCHisModelBsedOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtbs.opplugin.web.his.WTCHisModelBsedOp`
- order **4**: `RuleEngingSaveOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.ruleenging.RuleEngingSaveOp`
