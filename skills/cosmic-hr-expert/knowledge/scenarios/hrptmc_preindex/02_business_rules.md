# hrptmc_preindex · 业务规则

> **form**：`hrptmc_preindex` · 预置指标
> **生成时间**：2026-04-29
...
| 字段 | 中文名 | 类型 | 引用 |
|---|---|---|---|
| `getway` | 获取方式 | `ComboField` | — |

> 这些字段在 standard 元数据里标 `required=true` · 标品 MustInputValidation 自动校验 · ISV 不可改其必填属性

## 三、FormPlugin 类业务规则（UI 联动）

### `ReportPresetIndexEdit`

- **FQN**：`kd.hr.hrptmc.formplugin.web.preindex.ReportPresetIndexEdit`
- **继承**：`HRDataBaseEdit`
- **生命周期方法**：`registerListener`(L108), `afterBindData`(L138), `propertyChanged`(L176), `beforeF7Select`(L118), `beforeDoOperation`(L205), `itemClick`(L163)
- **读字段**（13）：`anobj`, `colfrom`, `colnumber`, `entryentity`, `fieldpath`, `id`, `isrequired`, `number`
- **调用的 Service**：`HRBaseServiceHelper`

### `ReportPresetIndexListPlugin`

- **FQN**：`kd.hr.hrptmc.formplugin.web.preindex.ReportPresetIndexListPlugin`
- **继承**：`HRDataBaseList`
- **生命周期方法**：`afterDoOperation`(L44)
- **调用的 Service**：`PresetIndexServiceHelper`
...
- 数据源：`_sdk_audit/_decompiled/scenarios/hrptmc_preindex/` 的 2 个反编译类 + `_shared/_standard_metadata/entity_metadata/hrptmc_preindex.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hr.hrptmc.formplugin.web.preindex.ReportPresetIndexEdit -->

## chgaction 实证补充（ReportPresetIndexEdit 跨类追踪聚合）
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hr.hrptmc.formplugin.web.preindex.ReportPresetIndexListPlugin -->

## chgaction 实证补充（ReportPresetIndexListPlugin 跨类追踪聚合）
