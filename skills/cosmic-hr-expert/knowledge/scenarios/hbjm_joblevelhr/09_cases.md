...
### Case 3：扩展 `HisModelFormCommonPlugin` (opKey=`save`)

**业务背景**：标品 `HisModelFormCommonPlugin` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hr.hbp.formplugin.web.history.form` 包
2. 写 ISV 子类继承 `HisModelFormCommonPlugin`：
```java
...
### Case 4：扩展 `HisModelBuFormPlugin` (opKey=`save`)

**业务背景**：标品 `HisModelBuFormPlugin` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hr.hbp.formplugin.web.history.form` 包
2. 写 ISV 子类继承 `HisModelBuFormPlugin`：
```java
...
### Case 5：扩展 `JobHisBasedataEdit` (opKey=`save`)

**业务背景**：标品 `JobHisBasedataEdit` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hrmp.hbjm.formplugin.web.basedata` 包
2. 写 ISV 子类继承 `JobHisBasedataEdit`：
```java
...
### Case 6：扩展 `JobHisBasedataFiledChangeEdit` (opKey=`save`)

**业务背景**：标品 `JobHisBasedataFiledChangeEdit` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hrmp.hbjm.formplugin.web.basedata` 包
2. 写 ISV 子类继承 `JobHisBasedataFiledChangeEdit`：
```java
...
### Case 7：扩展 `HRCertCheckEdit` (opKey=`save`)
...
### Case 8：扩展 `JobLevelEditPlugin` (opKey=`save`)

**业务背景**：标品 `JobLevelEditPlugin` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hrmp.hbjm.formplugin.web.joblevel` 包
2. 写 ISV 子类继承 `JobLevelEditPlugin`：
```java
