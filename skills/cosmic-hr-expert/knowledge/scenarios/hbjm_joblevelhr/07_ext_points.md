...
主表 `hbjm_joblevelhr` 标品接入 **52** 个插件。
...
- `JobHisBasedataFiledChangeEdit` ← `kd.hrmp.hbjm.formplugin.web.basedata.JobHisBasedataFiledChangeEdit`
...
- `JobBaseBuListPlugin` ← `kd.hrmp.hbjm.formplugin.web.basedata.JobBaseBuListPlugin`
...
- `JobLevelSaveOp` ← `kd.hrmp.hbjm.opplugin.web.JobLevelSaveOp`
...
## ✅ verified · 反编译实抓真处理类（28 个 · ISV 可继承 14 / 禁继承 14）
...
#### `JobHisBasedataEdit` · FORM_PLUGIN @ `BILL_FORM` · opKey=`save`

- **完整 FQN**: `kd.hrmp.hbjm.formplugin.web.basedata.JobHisBasedataEdit`
- **完整继承链**: `kd.hr.hbp.formplugin.web.HRDataBaseEdit`
- **重写生命周期方法（6）**:
  - `preOpenForm` — ISV 子类可在此切入扩展逻辑
  - `beforeBindData` — ISV 子类可在此切入扩展逻辑
  ...
  - `afterDoOperation` — ISV 子类可在此切入扩展逻辑
  - `closedCallBack` — ISV 子类可在此切入扩展逻辑
- **静态指标**: 写表 0 · 发事件 0 · 抛异常 10 · 调核心 Service 0 · trace 访问 7 类
- **ISV 扩展套路**：
  ```java
  package <你的 ISV 包>;
  
  import kd.hrmp.hbjm.formplugin.web.basedata.JobHisBasedataEdit;
  
  public class IsvCustomJobHisBasedataEdit extends JobHisBasedataEdit {
      @Override
  ...
  ```
- **详细 ISV 扩展指南**: [`<skills-dir>/cosmic-hr-expert/knowledge/_sdk_audit/_decompiled_deep/kd.hrmp.hbjm.formplugin.web.basedata.JobHisBasedataEdit/extension_guide.md`](../../<skills-dir>/cosmic-hr-expert/knowledge/_sdk_audit/_decompiled_deep/kd.hrmp.hbjm.formplugin.web.basedata.JobHisBasedataEdit/extension_guide.md)

#### `JobHisBasedataFiledChangeEdit` · FORM_PLUGIN @ `BILL_FORM` · opKey=`save`

- **完整 FQN**: `kd.hrmp.hbjm.formplugin.web.basedata.JobHisBasedataFiledChangeEdit`
- **完整继承链**: `kd.hr.hbp.formplugin.web.HRDataBaseEdit`
- **重写生命周期方法（3）**:
  - `afterBindData` — ISV 子类可在此切入扩展逻辑
  - `beforeDoOperation` — ISV 子类可在此切入扩展逻辑
...
- **ISV 扩展套路**：
  ...
  import kd.hrmp.hbjm.formplugin.web.basedata.JobHisBasedataFiledChangeEdit;
  
  public class IsvCustomJobHisBasedataFiledChangeEdit extends JobHisBasedataFiledChangeEdit {
      @Override
      public void afterBindData(... args) {
          super.afterBindData(args);  // 必调 super · 保留标品行为
          // 你的 ISV 业务逻辑 ...
  ...
  ```
- **详细 ISV 扩展指南**: [`<skills-dir>/cosmic-hr-expert/knowledge/_sdk_audit/_decompiled_deep/kd.hrmp.hbjm.formplugin.web.basedata.JobHisBasedataFiledChangeEdit/extension_guide.md`](../../<skills-dir>/cosmic-hr-expert/knowledge/_sdk_audit/_decompiled_deep/kd.hrmp.hbjm.formplugin.web.basedata.JobHisBasedataFiledChangeEdit/extension_guide.md)
...
#### `JobLevelEditPlugin` · FORM_PLUGIN @ `BILL_FORM` · opKey=`save`

- **完整 FQN**: `kd.hrmp.hbjm.formplugin.web.joblevel.JobLevelEditPlugin`
- **完整继承链**: `kd.hr.hbp.formplugin.web.HRDataBaseEdit`
- **重写生命周期方法（5）**:
  - `registerListener` — ISV 子类可在此切入扩展逻辑
  - `afterBindData` — ISV 子类可在此切入扩展逻辑
  - `afterF7Select` — ISV 子类可在此切入扩展逻辑
  - `beforeF7Select` — ISV 子类可在此切入扩展逻辑
  - `beforeItemClick` — ISV 子类可在此切入扩展逻辑
- **静态指标**: 写表 0 · 发事件 0 · 抛异常 0 · 调核心 Service 0 · trace 访问 1 类
- **ISV 扩展套路**：
  ```java
  ...
  import kd.hrmp.hbjm.formplugin.web.joblevel.JobLevelEditPlugin;
  
  public class IsvCustomJobLevelEditPlugin extends JobLevelEditPlugin {
      @Override
      public void registerListener(... args) {
          super.registerListener(args);  // 必调 super · 保留标品行为
          // 你的 ISV 业务逻辑 ...
      }
  }
  ```
- **详细 ISV 扩展指南**: [`<skills-dir>/cosmic-hr-expert/knowledge/_sdk_audit/_decompiled_deep/kd.hrmp.hbjm.formplugin.web.joblevel.JobLevelEditPlugin/extension_guide.md`](../../<skills-dir>/cosmic-hr-expert/knowledge/_sdk_audit/_decompiled_deep/kd.hrmp.hbjm.formplugin.web.joblevel.JobLevelEditPlugin/extension_guide.md)
...
### ❌ 禁继承类（14 个 · @SdkInternal 内部类 · 详细说明）
...
#### `JobBaseDataListPlugin` · FORM_PLUGIN · opKey=`save`

- **FQN**: `kd.hrmp.hbjm.formplugin.web.common.JobBaseDataListPlugin`
- **继承链**: `kd.hr.hbp.formplugin.web.HRDataBaseList`
- **为什么禁**: 标识为内部实现 · ISV 继承会破升级兼容
- **该用什么**: 上一节「可继承类」中找对应 opKey 的类
...
#### `JobLevelListPlugin` · FORM_PLUGIN · opKey=`save`

- **FQN**: `kd.hrmp.hbjm.formplugin.web.joblevel.list.JobLevelListPlugin`
- **继承链**: `kd.hr.hbp.formplugin.web.HRDataBaseList`
- **为什么禁**: 标识为内部实现 · ISV 继承会破升级兼容
- **该用什么**: 上一节「可继承类」中找对应 opKey 的类
...
#### `JobLevelSaveOp` · OP_PLUGIN · opKey=`save`

- **FQN**: `kd.hrmp.hbjm.opplugin.web.JobLevelSaveOp`
...
- order **7**: `HRBaseOriginalOp` · HR 基础资料原始值记录（对比变更前后）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp`
- order **8**: `HisUniqueValidateOp` · 时态模型唯一性校验（同一 boid 同一时间段唯一）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisUniqueValidateOp`
- order **9**: `HisModelOPCommonPlugin` · 时态模型通用 · his_bsed/his_bsled/effectdate 的维护
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisModelOPCommonPlugin`
- order **10**: `JobLevelSaveOp` · enabled=True
  - 完整 FQN: `kd.hrmp.hbjm.opplugin.web.JobLevelSaveOp`
...
- order **2**: `HisUniqueValidateOp` · 时态模型唯一性校验（同一 boid 同一时间段唯一）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisUniqueValidateOp`
- order **3**: `JobLevelSaveOp` · enabled=True
  - 完整 FQN: `kd.hrmp.hbjm.opplugin.web.JobLevelSaveOp`
