...
## ✅ verified · 主表 `wtbd_cycset` 共 30 个 opKey
...
### 其他 opKey（22 个无独立规则·复用主链或反编译为空）

- `new` · `modify` · `view` · `close` · `returndata` · `copy` · `refresh` · `option` · `submitandnew` · `saveandnew` · `first` · `previous`
- `next` · `last` · `importtemplatelist` · `mobtoolbarselect` · `mobtoolbarcancel` · `namehistory` · `namehistoryview` · `logview` · `viewonelog` · `cul`
