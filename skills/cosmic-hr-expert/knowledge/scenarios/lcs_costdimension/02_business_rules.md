...
| `valuetype` | 值类型 | `ComboField` | — |
| `displayproperty` | 值显示属性 | `ComboField` | — |
| `importproperty` | 导入属性 | `ComboField` | — |

> 这些字段在 standard 元数据里标 `required=true` · 标品 MustInputValidation 自动校验 · ISV 不可改其必填属性

## 二、OP 类业务规则（反编译实证）

### `CostDimensionDeleteOp`

- **FQN**：`kd.hrmp.lcs.opplugin.web.CostDimensionDeleteOp`
- **继承**：`HRDataBaseOp`

**调用的 Service / Helper**（1 个）：`CostDimensionDeleteValidator`
...
- **继承**：`HRDataBaseEdit`
- **生命周期方法**：`registerListener`(L73), `afterBindData`(L88), `propertyChanged`(L95), `beforeF7Select`(L173), `beforeDoOperation`(L121)
- **读字段**（2）：`name`, `number`
...
- 数据源：`_sdk_audit/_decompiled/scenarios/lcs_costdimension/` 的 2 个反编译类 + `_shared/_standard_metadata/entity_metadata/lcs_costdimension.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.lcs.formplugin.web.basedata.CostDimensionEdit -->

## chgaction 实证补充（CostDimensionEdit 跨类追踪聚合）
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.lcs.opplugin.web.CostDimensionDeleteOp -->

## chgaction 实证补充（CostDimensionDeleteOp 跨类追踪聚合）
