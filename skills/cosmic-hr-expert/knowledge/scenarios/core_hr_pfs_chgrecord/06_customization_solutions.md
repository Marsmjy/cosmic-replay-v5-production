# core_hr_pfs_chgrecord · 定制方案（ISV 扩展真实模板）

> **场景**：变动记录流水持久化（chgaction 实际执行处）（3 个子实体）
> **生成时间**：2026-04-30

## CS-01 · 给子实体加 ISV 自定义字段

**适用**：`hpfs_chgrecord`, `hpfs_chgrecordentry`, `hpfs_changerecord`（任一子实体）
...
**真发证据 / 反编译示例（如有）**：
- `ChgRecordAfterEffectOp.java` (5743 bytes)
- `ChgRecordDiscardOp.java` (1231 bytes)
- `ChgRecordEditPlugin.java` (1292 bytes)
- `ChgRecordEffectOp.java` (34846 bytes)
- `ChgRollbackOp.java` (23845 bytes)
