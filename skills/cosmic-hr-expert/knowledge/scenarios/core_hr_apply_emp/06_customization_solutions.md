# core_hr_apply_emp · 定制方案（ISV 扩展真实模板）

> **form**：`hrom_applybill_emp`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`hrom_applybill_emp`）加 ISV 字段：
...
```java
@SdkPlugin
public class MyEmpAfterOp extends HRDataBaseOp {
    @Override
...
标品 `ApplyBaseFormPlugin` 已实现：
- `preOpenForm` (L134)
- `registerListener` (L125)
- `afterBindData` (L199)
- `afterCreateNewData` (L251)
- `propertyChanged` (L256)
- `beforeF7Select` (L395)
- `click` (L167)
...
```java
@SdkPlugin
public class MyEmpIsvFormPlugin extends HRDataBaseEdit {
    @Override
