...
主表 `wtp_attendconfig` 标品接入 **51** 个插件。
...
- `AttendConfigConditionOp` ← `kd.wtc.wtp.opplugin.web.attend.AttendConfigConditionOp`
...
- order **9**: `HisModelOPCommonPlugin` · 时态模型通用 · his_bsed/his_bsled/effectdate 的维护
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisModelOPCommonPlugin`
- order **10**: `AttendConfigConditionOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.attend.AttendConfigConditionOp`
...
- order **8**: `HRBaseOriginalOp` · HR 基础资料原始值记录（对比变更前后）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp`
- order **9**: `AttendConfigConditionOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.attend.AttendConfigConditionOp`
