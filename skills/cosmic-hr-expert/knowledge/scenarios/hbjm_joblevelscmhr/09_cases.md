# ISV 实战 case · hbjm_joblevelscmhr
...
### Case 3：扩展 `JobBasedataEdit` (opKey=`save`)

**业务背景**：标品 `JobBasedataEdit` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hrmp.hbjm.formplugin.web.basedata` 包
2. 写 ISV 子类继承 `JobBasedataEdit`：
```java
...
### Case 4：扩展 `JobLevelScmPlugin` (opKey=`save`)

**业务背景**：标品 `JobLevelScmPlugin` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hrmp.hbjm.formplugin.web.joblevelscm` 包
2. 写 ISV 子类继承 `JobLevelScmPlugin`：
```java
...
### Case 5：扩展 `HRBaseDataStatusOp` (opKey=`save`)
...
### Case 7：扩展 `HBJMDataBaseOp` (opKey=`save`)

**业务背景**：标品 `HBJMDataBaseOp` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hrmp.hbjm.opplugin.web` 包
2. 写 ISV 子类继承 `HBJMDataBaseOp`：
```java
