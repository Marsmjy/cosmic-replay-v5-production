...
主表 `homs_cardstyle_new` 标品接入 **20** 个插件。
...
- `NewCardStylePlugin` ← `kd.hr.homs.formplugin.web.newcardstyle.NewCardStylePlugin`
- `NewCardStyleColorSchemaPlugin` ← `kd.hr.homs.formplugin.web.newcardstyle.NewCardStyleColorSchemaPlugin`
...
- `NewCardStyleListPlugin` ← `kd.hr.homs.formplugin.web.newcardstyle.NewCardStyleListPlugin`
...
- `NewCardStyleDeleteOp` ← `kd.hr.homs.opplugin.web.newcardstyle.NewCardStyleDeleteOp`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.homs.formplugin.web.newcardstyle.NewCardStylePlugin -->

## ISV 扩展指引（基于 NewCardStylePlugin 真实证）
...
**QUERY_BUILDER** · StructProjectRepository L48
```java
  46   
  47       public List<Long> queryEnablingByIds(Set<Long> ids) {
  48 >         QFilter filter = new QFilter("id", "in", ids).and(new QFilter("enable", "=", (Object)"10"));
  49           return this.serviceHelper.queryTranPropList("id", new QFilter[]{filter}, "id", Long.class);
  50       }
```
...
详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.homs.formplugin.web.newcardstyle.NewCardStylePlugin/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.homs.formplugin.web.newcardstyle.NewCardStylePlugin/extension_guide.md)

<!-- /AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.homs.formplugin.web.newcardstyle.NewCardStylePlugin -->

<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.homs.formplugin.web.newcardstyle.NewCardStyleColorSchemaPlugin -->

## ISV 扩展指引（基于 NewCardStyleColorSchemaPlugin 真实证）
...
- 父类: `kd.bos.form.plugin.AbstractFormPlugin`
- ISV 可继承: ✅ 推荐继承本父类
- 推荐挂载方法: `afterBindData`, `beforeDoOperation`, `afterDoOperation`, `closedCallBack`
...
**CONTEXT_ACCESS** · NewCardStyleColorSchemaPlugin L504
```java
 502           QFilter filter = new QFilter("enable", "=", (Object)"1");
 503           filter.and(new QFilter("status", "=", (Object)"C"));
 504 >         long currUserId = RequestContext.get().getCurrUserId();
 505           String appId = this.getView().getFormShowParameter().getCheckRightAppId();
 506           switch (entityName) {
```

**QUERY_BUILDER** · OrgCardConfigRepository L28
```java
  26   
  27       public DynamicObject[] queryColorSchemaDimensions() {
  28 >         QFilter filter = new QFilter("enable", "=", (Object)"1");
  29           filter.and(new QFilter("fieldtype", "!=", (Object)"2"));
  30           filter.and("carddimension", "=", (Object)OrgChartConstants.ORG_CARDDISSION);
```

**READ_VIA_HELPER** · NewCardStyleColorSchemaPlugin L406
```java
 404                       if (HRStringUtils.equals((String)fieldDataType, (String)FieldDataTypeEnum.ENUM.getCode())) {
 405                           String fieldIdentifier = cardConfig.getString("fieldidentifier");
 406 >                         DynamicProperty property = ColorSchemaRuleServiceHelper.getDynamicProperty((DynamicObject)cardConfig, (String)fieldIdentifier);
 407                           if (property instanceof ComboProp) {
 408                               ComboProp comboProp = (ComboProp)property;
```

**CALL_CROSS_SERVICE** · NewCardStyleColorSchemaPlugin L513
```java
 511               case "haos_adminorgdetail": 
 512               case "haos_adminorghrf7": {
 513 >                 AuthorizedOrgResultWithSub authorizedOrgResultWithSub = (AuthorizedOrgResultWithSub)HRMServiceHelper.invokeHRMPService((String)"hrcs", (String)"IHRCSBizDataPermissionService", (String)"getUserAdminOrgsF7WithSub", (Object[])new Object[]{currUserId, appId, this.getView().getEntityId(), "47150e89000000ac", "adminorg", HAOSPermUtils.getStructProjectCustomInParam((String)"adminorg", (long)1010L)});
 514                   if (!authorizedOrgResultWithSub.isHasAllOrgPerm()) {
 515                       QFilter permFilter = TreeTemplateHelper.getOrgAndSubInPermFilter((AuthorizedOrgResultWithSub)authorizedOrgResultWithSub, (String)"id");
```

**THROW_BIZ_EXCEPTION** · NewCardStyleColorSchemaPlugin L249
```java
 247               LOGGER.error("customEvent error", (Throwable)e);
 248               ResultDataHelper.hideLoading((IFormView)formView);
 249 >             throw new KDBizException(new ErrorCode("customEvent", e.getMessage()), new Object[0]);
 250           }
 251           ResultDataHelper.hideLoading((IFormView)formView);
```

详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.homs.formplugin.web.newcardstyle.NewCardStyleColorSchemaPlugin/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.homs.formplugin.web.newcardstyle.NewCardStyleColorSchemaPlugin/extension_guide.md)
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.homs.formplugin.web.newcardstyle.NewCardStyleListPlugin -->

## ISV 扩展指引（基于 NewCardStyleListPlugin 真实证）
...
**QUERY_BUILDER** · NewCardStyleListPlugin L35
```java
  33           List filters = setFilterEvent.getQFilters();
  34           super.setFilter(setFilterEvent);
  35 >         filters.add(new QFilter("id", "!=", (Object)VIRTUAL_ORG_TYPE_ID));
  36           filters.add(new QFilter("cardstyleflag", "in", Arrays.asList("1", "2")));
  37           filters.add(new QFilter("carddimension", "=", (Object)Long.parseLong("1010")));
```

**READ_VIA_HELPER** · NewCardStyleListPlugin L53
```java
  51               }
  52               Set idSet = successPkIds.stream().map(pkId -> (Long)pkId).collect(Collectors.toSet());
  53 >             DynamicObject[] newChartStyleById = OrgChartNewServiceHelper.getNewChartStyleById(idSet);
  54               ArrayList<DynamicObject> needSaveObject = new ArrayList<DynamicObject>(newChartStyleById.length);
  55               for (DynamicObject dyn : newChartStyleById) {
```

详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.homs.formplugin.web.newcardstyle.NewCardStyleListPlugin/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.homs.formplugin.web.newcardstyle.NewCardStyleListPlugin/extension_guide.md)
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.homs.opplugin.web.newcardstyle.NewCardStyleDeleteOp -->

## ISV 扩展指引（基于 NewCardStyleDeleteOp 真实证）
...
详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.homs.opplugin.web.newcardstyle.NewCardStyleDeleteOp/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.homs.opplugin.web.newcardstyle.NewCardStyleDeleteOp/extension_guide.md)
