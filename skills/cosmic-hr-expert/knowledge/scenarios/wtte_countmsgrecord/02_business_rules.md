...
## ✅ verified · 主表 `wtte_countmsgrecord` 共 11 个 opKey

### 其他 opKey（11 个无独立规则·复用主链或反编译为空）

- `importdata_hr` · `show_import_record_hr` · `export_from_list_hr` · `export_from_impttpl_hr` · `export_from_expttpl_hr` · `show_export_record_hr` · `first` · `previous` · `next` · `last` · `save`

## ✅ verified · 反编译类画像（共 7 个真处理类）
...
| `HRBaseDataCreateOrgPluginExt` | OTHER | `save` | ❌ |  | [查看](../../<skills-dir>/cosmic-hr-expert/knowledge/_sdk_audit/_decompiled_deep/kd.hr.hbp.formplugin.web.bdctrl.HRBaseDataCreateOrgPluginExt/extension_guide.md) |
