# hrcs_warnscene · 定制方案（ISV 扩展真实模板）
...
在开发平台给本 form（`hrcs_warnscene`）加 ISV 字段：
...
```java
@SdkPlugin
public class MyWarnsceneAfterOp extends HRDataBaseOp {
    @Override
...
标品 `WarnAdFilterRightTreeEditPlugin` 已实现：
- `registerListener` (L77)

ISV **并列挂**（PR-001 · 不要继承标品 Edit）：

```java
@SdkPlugin
public class MyWarnsceneIsvFormPlugin extends HRDataBaseEdit {
    @Override
...
- `HRBaseServiceHelper`
- `ReportQueryService`
- `WarnCalcFieldService`
- `WarnSceneCommonConditionValidator`
- `WarnSceneValidator`
- `WarningSceneService`
