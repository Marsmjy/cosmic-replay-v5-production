...
> 本 form（`wtte_recordsumreport`，所属 other）引用了其他云的 **11** 个底座实体：

### ⬆️ HR 基础服务云（`hr_hrmp`）2 个引用

| 字段 | 字段名 | 类型 | 引用实体 | 上游场景 |
|---|---|---|---|---|
| `agreedworkplace` | 协议工作地 | BasedataField | `hbss_workplace` | [hbss_supplier](../hbss_supplier/) |
...
| `jobvid` | 职位 | BasedataField | `hbjm_jobhr` | [hjm_jobhr_maintenance](../hjm_jobhr_maintenance/) |
| `affiliateadminorgvid` | 挂靠行政组织 | HRAdminOrgField | `haos_adminorghrf7` | — |

### ⬆️ unknown（`unknown`）4 个引用
