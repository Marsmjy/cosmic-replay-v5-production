...
| `taxpayertype` | 纳税人类型 | `ComboField` | — |
| `emptype` | 任职受雇从业类型 | `ComboField` | — |
| `taxstatus` | 个税状态 | `ComboField` | — |
...
**调用的 Service / Helper**（1 个）：`EmployeeTaxCNSaveValidator`

**实现的生命周期方法**：`onAddValidators`(L68), `beforeExecuteOperationTransaction`(L73)
...
- 数据源：`_sdk_audit/_decompiled/scenarios/core_hr_employeetaxcn/` 的 12 个反编译类 + `_shared/_standard_metadata/entity_metadata/hrpi_employeetaxcn.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.hrpi.opplugin.web.employee.EmployeeTaxCNSaveOp -->

## chgaction 实证补充（EmployeeTaxCNSaveOp 跨类追踪聚合）
