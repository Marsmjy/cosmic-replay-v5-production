...
## ✅ verified · 主表 `wtte_persumscheme` 共 56 个 opKey
...
| 7 | `DisplaySchemeSaveOp` | enabled=True |
...
| 5 | `DisplaySchemeSaveOp` | enabled=True |
| 6 | `HRBaseDataEnableOp` | HR 基础资料启用态管理 · 控制 enable 字段 |
...
- `exportlist_expt` · `exportdetails` · `importexport_userset` · `mobtoolbarselect` · `mobtoolbarcancel` · `namehistory` · `namehistoryview` · `assign` · `unassign` · `bdctrlchange` · `assign_new` · `auto_assign`
- `tbl_assign_import` · `logview` · `viewonelog` · `donothing` · `displaydeleteentry` · `displaymoveentryup` · `displaymoveentrydown` · `sortdeleteentry` · `sortmoveentryup` · `sortmoveentrydown` · `individuation`
