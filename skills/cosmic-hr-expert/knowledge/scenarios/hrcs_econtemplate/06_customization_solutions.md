# hrcs_econtemplate · 定制方案（ISV 扩展真实模板）

> **form**：`hrcs_econtemplate`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`hrcs_econtemplate`）加 ISV 字段：
...
标品 `EContractTemplateSaveOp` 在 `onAddValidators` 注入了：
...
@SdkPlugin
public class MyEcontemplateIsvOp extends HRDataBaseOp {
    @Override
...
```java
@SdkPlugin
public class MyEcontemplateAfterOp extends HRDataBaseOp {
    @Override
...
标品 `EContractTemplateEditPlugin` 已实现：
- `registerListener` (L46)
- `afterBindData` (L72)
- `propertyChanged` (L60)
- `beforeDoOperation` (L51)

ISV **并列挂**（PR-001 · 不要继承标品 Edit）：

```java
@SdkPlugin
public class MyEcontemplateIsvFormPlugin extends HRDataBaseEdit {
    @Override
...
本场景反编译类调用的 Service / Helper（1 个 · 仅前 10）：
