...
| 字段 | 中文名 | 类型 | 引用 |
|---|---|---|---|
| `startdate` | 外派开始日期 | `DateField` | — |
| `enddate` | 外派结束日期 | `DateField` | — |
...
- **FQN**：`kd.hrmp.hrpi.opplugin.web.assignment.attach.DispatchSaveOp`
- **继承**：`HRDataBaseOp`

**调用的 Service / Helper**（4 个）：`BelongCompanyValidator`, `DispatchDateValidator`, `PositionAndJobMatchValidator`, `PositionValidator`

**实现的生命周期方法**：`onAddValidators`(L34)
...
- 数据源：`_sdk_audit/_decompiled/scenarios/core_hr_dispatchinfo/` 的 12 个反编译类 + `_shared/_standard_metadata/entity_metadata/hrpi_dispatchinfo.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.hrpi.opplugin.web.assignment.attach.DispatchSaveOp -->

## chgaction 实证补充（DispatchSaveOp 跨类追踪聚合）

> FQN: `kd.hrmp.hrpi.opplugin.web.assignment.attach.DispatchSaveOp`
> 跨类追踪: 1 个类访问
> 反编译产物: `_sdk_audit/_decompiled_deep/kd.hrmp.hrpi.opplugin.web.assignment.attach.DispatchSaveOp/`
