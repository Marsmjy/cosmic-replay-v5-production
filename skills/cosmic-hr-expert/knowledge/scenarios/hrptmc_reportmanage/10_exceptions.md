...
- `V_ReportManageOp_1` (opKey=`save`) · (errorMessage 未实抓 · 待 jar 反查)

## ✅ verified · 反编译 throw 语句（OP 类业务异常）

反编译共抓到 **3** 条 `throw new *Exception(...)` 调用：

- `KDBizException` 在 `ReportManageEditPlugin` · `(动态消息 · 变量=args)`
- `KDBizException` 在 `ReportManageOp` · `(动态消息 · 变量=ResManager)`
