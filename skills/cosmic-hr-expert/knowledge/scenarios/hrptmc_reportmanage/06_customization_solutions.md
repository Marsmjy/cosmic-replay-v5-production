# hrptmc_reportmanage · 定制方案（ISV 扩展真实模板）

> **form**：`hrptmc_reportmanage`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`hrptmc_reportmanage`）加 ISV 字段：
...
标品 `ReportManageOp` 在 `onAddValidators` 注入了：
- `ReportValidator`
...
@SdkPlugin
public class MyReportmanageIsvOp extends HRDataBaseOp {
    @Override
...
```java
@SdkPlugin
public class MyReportmanageAfterOp extends HRDataBaseOp {
    @Override
...
标品 `ReportManageEditPlugin` 已实现：
- `afterBindData` (L235)
- `afterCreateNewData` (L155)
- `beforeDoOperation` (L256)
- `afterDoOperation` (L318)

ISV **并列挂**（PR-001 · 不要继承标品 Edit）：

```java
@SdkPlugin
public class MyReportmanageIsvFormPlugin extends HRDataBaseEdit {
    @Override
...
本场景反编译类调用的 Service / Helper（6 个 · 仅前 10）：

- `HRBaseServiceHelper`
- `HRReportMetaDataHelper`
- `HRReportPreSQLHelper`
- `PresetIndexServiceHelper`
- `ReportConfigService`
- `ReportValidator`
