# haos_adminorgdetail · 定制方案（ISV 扩展真实模板）

> **form**：`haos_adminorgdetail`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`haos_adminorgdetail`）加 ISV 字段：
...
### 引用关系
成建制划转任务 A/B 通过 `OrgTransferHelper.queryOrgHisDataByIdAndEffectDate` 查询本场景的历史版本（`iscurrentversion!=1`）

### 部署后的影响
当客户部署成建制划转资产 (`_assets/org_unit_transfer/`)·调度任务每日跑会**消费本场景的标品数据** (查询行政组织历史版本·刷新任职经历的 `adminorgvid` 字段)·然后批量装配标品调动单 `hdm_transferbatch` + `SUBMITEFFECT` 触发跨云协同。
