...
- **继承**：`HRDataBaseEdit`
- **生命周期方法**：`preOpenForm`(L183), `registerListener`(L113), `propertyChanged`(L139), `beforeF7Select`(L191), `beforeDoOperation`(L157), `afterDoOperation`(L173), `click`(L120), `itemClick`(L133)
- **读字段**（3）：`entity`, `id`, `number`

### `EntityEncryptTreeListEdit`

- **FQN**：`kd.hr.expt.formplugin.EntityEncryptTreeListEdit`
- **继承**：`HRTreeListBizAppsPlugin`
- **生命周期方法**：`preOpenForm`(L148), `registerListener`(L363), `afterBindData`(L340), `beforeF7Select`(L348), `afterDoOperation`(L323), `click`(L156), `itemClick`(L160)
- **读字段**（4）：`bizappid_id`, `cloud.id`, `id`, `number`
...
- 数据源：`_sdk_audit/_decompiled/scenarios/hies_entityencryptconf/` 的 2 个反编译类 + `_shared/_standard_metadata/entity_metadata/hies_entityencryptconf.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hr.expt.formplugin.EntityEncryptEdit -->

## chgaction 实证补充（EntityEncryptEdit 跨类追踪聚合）
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hr.expt.formplugin.EntityEncryptTreeListEdit -->

## chgaction 实证补充（EntityEncryptTreeListEdit 跨类追踪聚合）
