# hies_diaetplconf · 定制方案（ISV 扩展真实模板）

> **form**：`hies_diaetplconf`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`hies_diaetplconf`）加 ISV 字段：
...
```java
@SdkPlugin
public class MyDiaetplconfAfterOp extends HRDataBaseOp {
    @Override
...
标品 `TemplateAddSubEntityPlugin` 已实现：
- `registerListener` (L189)
- `afterBindData` (L215)
- `afterCreateNewData` (L532)
- `propertyChanged` (L593)
- `beforeF7Select` (L536)
- `afterDoOperation` (L406)
- `click` (L431)
- `itemClick` (L392)
...
```java
@SdkPlugin
public class MyDiaetplconfIsvFormPlugin extends HRDataBaseEdit {
    @Override
...
- `HRBaseServiceHelper`
- `InteService`
- `TemplateDeleteValidator`
- `TemplateEnableValidator`
