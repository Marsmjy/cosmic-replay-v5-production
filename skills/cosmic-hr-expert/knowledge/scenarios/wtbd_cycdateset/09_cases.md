...
### Case 4：扩展 `HRBaseDataEnableOp` (opKey=`save`)
...
### Case 6：扩展 `HRBaseDataLogOp` (opKey=`delete`)

**业务背景**：标品 `HRBaseDataLogOp` 处理 `delete` 操作 · ISV 想加自定义校验/写表/通知
...
```
3. 在 BOS 设计器注册 ISV 插件 · targetType=OPERATION
4. 单测：触发 opKey `delete` · 验证 ISV 逻辑 + super 行为都跑
