...
> 本 form（`hrpi_rotationinfo`，所属 核心人力云）引用了其他云的 **5** 个底座实体：

### ⬆️ HR 基础服务云（`hr_hrmp`）1 个引用

| 字段 | 字段名 | 类型 | 引用实体 | 上游场景 |
|---|---|---|---|---|
| `rottype` | 轮岗类型 | BasedataField | `hbss_rotationtype` | [hbss_position_dict](../hbss_position_dict/) |

### ⬆️ 组织发展云（`org_dev`）4 个引用

| 字段 | 字段名 | 类型 | 引用实体 | 上游场景 |
|---|---|---|---|---|
| `adminorg` | 轮岗部门 | HRAdminOrgField | `haos_adminorghrf7` | — |
| `job` | 担任职位 | BasedataField | `hbjm_jobhr` | [hjm_jobhr_maintenance](../hjm_jobhr_maintenance/) |
| `position` | 担任岗位 | HRPositionField | `hbpm_positionhrf7` | — |
| `company` | 轮岗公司 | HRAdminOrgField | `haos_adminorghrf7` | — |
