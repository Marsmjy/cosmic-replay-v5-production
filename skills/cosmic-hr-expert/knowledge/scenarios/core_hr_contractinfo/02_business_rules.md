# core_hr_contractinfo · 业务规则

> **form**：`hrpi_contractinfo` · 合同信息
> **生成时间**：2026-04-29
...
| 字段 | 中文名 | 类型 | 引用 |
|---|---|---|---|
| `startdate` | 合同开始日期 | `DateField` | — |
| `enddate` | 实际结束日期 | `DateField` | — |
| `assignment` | 组织分配 | `BasedataField` | `hrpi_assignment` |
| `employee` | 员工 | `EmployeeField` | `hrpi_employeenewf7query` |
| `number` | 合同编号 | `TextField` | — |
| `periodunit` | 合同期限单位 | `ComboField` | — |
| `signcompany` | 聘用单位 | `BasedataField` | `hbss_signcompany` |
| `contracttype` | 合同类型 | `BasedataField` | `hbss_contracttypes` |
| `planenddate` | 计划结束日期 | `DateField` | — |
| `periodtype` | 期限类型 | `BasedataField` | `hbss_timelimittype` |
...
**实现的生命周期方法**：`beginOperationTransaction`(L35)
...
- 数据源：`_sdk_audit/_decompiled/scenarios/core_hr_contractinfo/` 的 12 个反编译类 + `_shared/_standard_metadata/entity_metadata/hrpi_contractinfo.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.hrpi.opplugin.web.assignment.attach.ContractInfoSaveOp -->

## chgaction 实证补充（ContractInfoSaveOp 跨类追踪聚合）
