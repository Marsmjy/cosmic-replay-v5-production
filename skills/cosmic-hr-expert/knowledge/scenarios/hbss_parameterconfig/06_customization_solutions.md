# hbss_parameterconfig · 定制方案（ISV 扩展真实模板）

> **form**：`hbss_parameterconfig`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`hbss_parameterconfig`）加 ISV 字段：
...
标品 `BasedataConfigOp` 在 `onAddValidators` 注入了：
...
@SdkPlugin
public class MyParameterconfigIsvOp extends HRDataBaseOp {
    @Override
...
```java
@SdkPlugin
public class MyParameterconfigAfterOp extends HRDataBaseOp {
    @Override
...
标品 `AppConfigEditPlugin` 已实现：
- `registerListener` (L90)
- `propertyChanged` (L63)
- `beforeF7Select` (L122)
...
```java
@SdkPlugin
public class MyParameterconfigIsvFormPlugin extends HRDataBaseEdit {
    @Override
...
本场景反编译类调用的 Service / Helper（3 个 · 仅前 10）：

- `BasedataConfigOpValidator`
- `HRBaseServiceHelper`
- `HRParamConfigTreeServiceHelper`
