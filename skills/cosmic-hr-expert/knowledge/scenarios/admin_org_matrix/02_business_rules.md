...
- `org` (创建组织) · `OrgField` → ref bos_org
- `rootorg` (根组织) · `HRAdminOrgField` → ref haos_adminorghrf7
- `roottype` (根组织类型) · `ComboField`
...
- `org` (创建组织) → `bos_org` · `OrgField`
- `rootorg` (根组织) → `haos_adminorghrf7` · `HRAdminOrgField`
- `relyonstructproject` (依赖架构方案) → `haos_structproject` · `BasedataField`
- `otclassify` (组织团队分类) → `haos_otclassify` · `BasedataField`
- `orgorg` (组织体系管理组织) → `bos_org` · `OrgField`
