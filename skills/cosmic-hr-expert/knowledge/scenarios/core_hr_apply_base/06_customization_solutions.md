# core_hr_apply_base · 定制方案（ISV 扩展真实模板）
...
在开发平台给本 form（`hrom_applybill`）加 ISV 字段：
...
```java
@SdkPlugin
public class MyBaseAfterOp extends HRDataBaseOp {
    @Override
...
```java
@SdkPlugin
public class MyBaseIsvFormPlugin extends HRDataBaseEdit {
    @Override
