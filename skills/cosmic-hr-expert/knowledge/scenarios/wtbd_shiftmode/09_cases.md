...
### Case 3：扩展 `HRBaseDataStatusOp` (opKey=`save`)
...
### Case 4：扩展 `HRBaseDataLogOp` (opKey=`save`)
...
### Case 5：扩展 `HRBaseOriginalOp` (opKey=`save`)
...
### Case 6：扩展 `HRBaseDataEnableOp` (opKey=`submit`)

**业务背景**：标品 `HRBaseDataEnableOp` 处理 `submit` 操作 · ISV 想加自定义校验/写表/通知
...
```
3. 在 BOS 设计器注册 ISV 插件 · targetType=OPERATION
4. 单测：触发 opKey `submit` · 验证 ISV 逻辑 + super 行为都跑
