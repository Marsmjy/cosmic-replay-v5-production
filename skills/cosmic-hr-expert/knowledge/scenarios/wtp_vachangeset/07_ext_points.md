...
主表 `wtp_vachangeset` 标品接入 **52** 个插件。
...
- `VacationChangeSetSaveOp` ← `kd.wtc.wtp.opplugin.web.vacation.VacationChangeSetSaveOp`
...
- order **9**: `HisModelOPCommonPlugin` · 时态模型通用 · his_bsed/his_bsled/effectdate 的维护
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisModelOPCommonPlugin`
- order **10**: `VacationChangeSetSaveOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.vacation.VacationChangeSetSaveOp`
...
- order **5**: `HRBaseDataLogOp` · HR 基础资料操作日志
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp`
- order **6**: `VacationChangeSetSaveOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.vacation.VacationChangeSetSaveOp`
...
- order **2**: `HisUniqueValidateOp` · 时态模型唯一性校验（同一 boid 同一时间段唯一）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisUniqueValidateOp`
- order **3**: `VacationChangeSetSaveOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.vacation.VacationChangeSetSaveOp`
