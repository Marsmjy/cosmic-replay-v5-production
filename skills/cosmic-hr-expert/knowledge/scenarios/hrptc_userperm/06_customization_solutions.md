# hrptc_userperm · 定制方案（ISV 扩展真实模板）
...
在开发平台给本 form（`hrptc_userperm`）加 ISV 字段：
...
标品 `ReportUserPermOp` 在 `onAddValidators` 注入了：
...
@SdkPlugin
public class MyUserpermIsvOp extends HRDataBaseOp {
    @Override
...
```java
@SdkPlugin
public class MyUserpermAfterOp extends HRDataBaseOp {
    @Override
...
标品 `ReportUserPermEdit` 已实现：
- `propertyChanged` (L243)
- `beforeDoOperation` (L228)
- `afterDoOperation` (L360)

ISV **并列挂**（PR-001 · 不要继承标品 Edit）：

```java
@SdkPlugin
public class MyUserpermIsvFormPlugin extends HRDataBaseEdit {
    @Override
