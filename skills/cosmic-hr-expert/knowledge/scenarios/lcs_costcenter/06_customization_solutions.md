# lcs_costcenter · 定制方案（ISV 扩展真实模板）
...
在开发平台给本 form（`lcs_costcenter`）加 ISV 字段：
...
标品 `CostCenterEdit` 已实现：
- `registerListener` (L29)
- `afterBindData` (L35)
- `beforeF7Select` (L52)
...
```java
@SdkPlugin
public class MyCostcenterIsvFormPlugin extends HRDataBaseEdit {
    @Override
