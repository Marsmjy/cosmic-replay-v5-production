...
- **FQN**：`kd.hrmp.lcs.opplugin.web.CostCenterAuditOp`
- **继承**：`HRDataBaseOp`

### `CostCenterEnableOp`

- **FQN**：`kd.hrmp.lcs.opplugin.web.CostCenterEnableOp`
- **继承**：`HRDataBaseOp`
...
- **继承**：`HRDataBaseEdit`
- **生命周期方法**：`registerListener`(L29), `afterBindData`(L35), `beforeF7Select`(L52)
- **读字段**（2）：`id`, `status`
...
- 数据源：`_sdk_audit/_decompiled/scenarios/lcs_costcenter/` 的 4 个反编译类 + `_shared/_standard_metadata/entity_metadata/lcs_costcenter.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.lcs.formplugin.web.costcenter.CostCenterEdit -->

## chgaction 实证补充（CostCenterEdit 跨类追踪聚合）
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.lcs.formplugin.web.costcenter.CostCenterList -->

## chgaction 实证补充（CostCenterList 跨类追踪聚合）
