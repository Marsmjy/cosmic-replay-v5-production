...
## ✅ verified · 主表 `wtom_otbillself` 共 49 个 opKey
...
| 3 | `WTCBillApplyTypeOp` | enabled=True |
| 4 | `BillTplOp` | enabled=True |
| 5 | `WTCBillUpdateAttfileOp` | enabled=True |
| 6 | `WTCBillChangeShowOp` | enabled=True |
| 7 | `WTCBillChangeStatusOp` | enabled=True |
| 8 | `WTCBillReplenishOriginalOp` | enabled=True |
| 9 | `OtApplyOp` | enabled=True |
| 10 | `OtApplyArchiveControlValidateOp` | enabled=True |
...
| 4 | `WTCBillApplyTypeOp` | enabled=True |
| 5 | `OtApplyOp` | enabled=True |
| 6 | `WTCBillUpdateAttfileOp` | enabled=True |
| 7 | `WTCBillChangeShowOp` | enabled=True |
| 8 | `WTCBillReplenishOriginalOp` | enabled=True |
| 9 | `OtApplyArchiveControlValidateOp` | enabled=True |
...
| 3 | `BillTplOp` | enabled=True |
| 4 | `OtApplyOp` | enabled=True |
| 5 | `WTCBillSelfApplyOp` | enabled=True |
| 6 | `WTCBillUpdateAttfileOp` | enabled=True |
| 7 | `WTCBillReplenishOriginalOp` | enabled=True |
| 8 | `OtApplyArchiveControlValidateOp` | enabled=True |
...
| 2 | `WTCBillSelfApplyOp` | enabled=True |
| 3 | `WTCBillApplyTypeOp` | enabled=True |
| 4 | `WTCBillAttFileOp` | enabled=True |
...
| 2 | `WTCBillApplyTypeOp` | enabled=True |
| 3 | `WTCBillSelfApplyOp` | enabled=True |
...
| 2 | `OtChangeOp` | enabled=True |
| 3 | `OtApplyArchiveControlValidateOp` | enabled=True |
...
| 2 | `OtApplyArchiveControlValidateOp` | enabled=True |
...
| order | className | 角色 |
|:---:|---|---|
| 1 | `OtApplyArchiveControlValidateOp` | enabled=True |
...
#### `donothing0` · 空操作（opType=donothing）

**执行链（1 个 plugin · 按 order 排）**：

| order | className | 角色 |
|:---:|---|---|
| 1 | `wtom_otbilltest` | enabled=True |
...
### 其他 opKey（38 个无独立规则·复用主链或反编译为空）

- `export_from_list_hr` · `show_export_record_hr` · `new` · `close` · `refresh` · `copy` · `option` · `submitandnew` · `view` · `first` · `previous` · `next`
- `last` · `importtemplate` · `importtemplatelist` · `returndata` · `mobtoolbarselect` · `mobtoolbarcancel` · `viewtrash` · `trash` · `restoredata` · `modify` · `wfauditing` · `wfrejecttosubmit`
- `wfauditnotpass` · `wfauditpass` · `viewflowchart` · `unaudit` · `deleteentry` · `deletesdentry` · `openselectperson` · `newscentry` · `deletescentry` · `newsdentry` · `singletype` · `multitype`
- `returnlist` · `showhisbill`
