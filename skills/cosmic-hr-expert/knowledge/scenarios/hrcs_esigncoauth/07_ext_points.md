...
主表 `hrcs_esigncoauth` 标品接入 **18** 个插件。
...
- `ESignCompanyAuthFormPlugin` ← `kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthFormPlugin`
...
- `ESignCompanyAuthListPlugin` ← `kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthListPlugin`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthFormPlugin -->

## ISV 扩展指引（基于 ESignCompanyAuthFormPlugin 真实证）

> FQN: `kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthFormPlugin`
> 反编译产物: `_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthFormPlugin/`

### 类型与继承
- 插件类型：**FORM_PLUGIN**
- 父类: `kd.hr.hbp.formplugin.web.HRDataBaseEdit`
- ISV 可继承: ✅ 推荐继承本父类
- 推荐挂载方法: `registerListener`, `propertyChanged`, `beforeDoOperation`, `beforeF7Select`
...
**QUERY_BUILDER** · ESignCompanyAuthFormPlugin L141
```java
 139               if (!CollectionUtils.isEmpty((Collection)allEnableAppInfos)) {
 140                   List collect = allEnableAppInfos.stream().map(info -> info.getLong("bdesignappcfg.id")).collect(Collectors.toList());
 141 >                 showParameter.getListFilterParameter().setFilter(new QFilter("id", "not in", collect));
 142               }
 143           }
```

**READ_VIA_HELPER** · ESignCompanyAuthFormPlugin L84
```java
  82                   return;
  83               }
  84 >             ESignAppInfo eSignSPAppInfo = HRCSESignAppCfgServiceHelper.getESignAppInfo((Object)authapp.getPkValue());
  85               this.getModel().setValue("esignspmgr", (Object)eSignSPAppInfo.getSpId());
  86           }
```

详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthFormPlugin/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthFormPlugin/extension_guide.md)
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthListPlugin -->

## ISV 扩展指引（基于 ESignCompanyAuthListPlugin 真实证）

> FQN: `kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthListPlugin`
> 反编译产物: `_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthListPlugin/`

### 类型与继承
- 插件类型：**FORM_PLUGIN**
- 父类: `kd.hr.hbp.formplugin.web.HRDataBaseList`
- ISV 可继承: ⚠️ 标品 @SdkInternal·改继承基类
- 推荐挂载方法: `filterContainerInit`, `beforeDoOperation`
...
**READ_VIA_HELPER** · ESignCompanyAuthListPlugin L48
```java
  46   extends HRDataBaseList {
  47       public void filterContainerInit(FilterContainerInitArgs args) {
  48 >         DynamicObject onlineESignSP = HRCSESignSPMgrServiceHelper.getOnlineESignSP();
  49           if (onlineESignSP != null) {
  50               args.getFilterColumn("esignspmgr.name").setDefaultValue(onlineESignSP.getPkValue().toString());
```

详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthListPlugin/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthListPlugin/extension_guide.md)
