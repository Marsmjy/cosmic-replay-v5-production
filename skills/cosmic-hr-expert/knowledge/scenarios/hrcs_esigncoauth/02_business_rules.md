...
### `ESignCompanyAuthFormPlugin`

- **FQN**：`kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthFormPlugin`
- **继承**：`HRDataBaseEdit`
- **生命周期方法**：`registerListener`(L70), `propertyChanged`(L76), `beforeF7Select`(L134), `beforeDoOperation`(L89)
- **读字段**（3）：`bdesignappcfg.id`, `id`, `name`

### `ESignCompanyAuthListPlugin`

- **FQN**：`kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthListPlugin`
- **继承**：`HRDataBaseList`
- **生命周期方法**：`beforeDoOperation`(L54)
- **读字段**（9）：`authapp`, `authstatus`, `esignspmgr.id`, `firmname`, `lawentity`, `name`, `phone`, `representative`
- **写字段**（2）：`authstatus`, `thirdcompanyid`
...
- 数据源：`_sdk_audit/_decompiled/scenarios/hrcs_esigncoauth/` 的 2 个反编译类 + `_shared/_standard_metadata/entity_metadata/hrcs_esigncoauth.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthFormPlugin -->

## chgaction 实证补充（ESignCompanyAuthFormPlugin 跨类追踪聚合）
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hr.hrcs.formplugin.web.econtract.ESignCompanyAuthListPlugin -->

## chgaction 实证补充（ESignCompanyAuthListPlugin 跨类追踪聚合）
...
| 实体 | 操作 | 入口类 | 来源链 |
|---|---|---|---|
| `hrcs_esigncoauth` | updateOne | HRBaseServiceHelper | <self> (depth=0) |
