...
- `WTCNonTimeHisNewEdit` ← `kd.wtc.wtbs.formplugin.web.history.WTCNonTimeHisNewEdit`
- `ShiftGroupEdit` ← `kd.wtc.wts.formplugin.web.shiftgroup.ShiftGroupEdit`
...
- `ShiftGroupList` ← `kd.wtc.wts.formplugin.web.shiftgroup.ShiftGroupList`
- `ShiftGroupTipsList` ← `kd.wtc.wts.formplugin.web.shiftgroup.ShiftGroupTipsList`
...
- `ShiftGroupSaveOp` ← `kd.wtc.wts.opplugin.web.shiftgroup.ShiftGroupSaveOp`
...
- `ShiftGroupCancelAuthOp` ← `kd.wtc.wts.opplugin.web.shiftgroup.ShiftGroupCancelAuthOp`
...
## ✅ verified · 反编译实抓真处理类（18 个 · ISV 可继承 9 / 禁继承 9）
...
- order **2**: `BdVersionSaveServicePlugin` · 基础资料版本管理 · 主表 name + 版本子表 name 的写历史
  - 完整 FQN: `kd.bos.base.bdversion.BdVersionSaveServicePlugin`
- order **3**: `HRBaseDataStatusOp` · HR 基础资料单据状态（draft/audit/disable）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataStatusOp`
- order **4**: `ShiftGroupSaveOp` · enabled=True
  - 完整 FQN: `kd.wtc.wts.opplugin.web.shiftgroup.ShiftGroupSaveOp`
...
- order **7**: `HRBaseOriginalOp` · HR 基础资料原始值记录（对比变更前后）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp`
- order **8**: `HisModelOPCommonPlugin` · 时态模型通用 · his_bsed/his_bsled/effectdate 的维护
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisModelOPCommonPlugin`
...
- order **3**: `HRBaseDataLogOp` · HR 基础资料操作日志
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp`
- order **4**: `HRBaseDataEnableOp` · HR 基础资料启用态管理 · 控制 enable 字段
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataEnableOp`
- order **5**: `HisUniqueValidateOp` · 时态模型唯一性校验（同一 boid 同一时间段唯一）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisUniqueValidateOp`
- order **6**: `HRBaseOriginalOp` · HR 基础资料原始值记录（对比变更前后）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp`
- order **7**: `HisModelOPCommonPlugin` · 时态模型通用 · his_bsed/his_bsled/effectdate 的维护
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisModelOPCommonPlugin`
...
- order **2**: `HRBaseDataStatusOp` · HR 基础资料单据状态（draft/audit/disable）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataStatusOp`
- order **3**: `HRBaseDataLogOp` · HR 基础资料操作日志
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataLogOp`
...
### opKey `cancelauth` · 取消分配

**执行链**（1 个 plugin 按 order 顺序跑）：

- order **1**: `ShiftGroupCancelAuthOp` · enabled=True
  - 完整 FQN: `kd.wtc.wts.opplugin.web.shiftgroup.ShiftGroupCancelAuthOp`
