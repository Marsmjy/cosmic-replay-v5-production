# core_hr_pfs_mapping · 定制方案（ISV 扩展真实模板）

> **场景**：⭐⭐⭐ pfs 字段映射引擎（ISV 零代码扩展利器）（4 个子实体）
> **生成时间**：2026-04-30

## CS-01 · 给子实体加 ISV 自定义字段

**适用**：`hpfs_filemapmanager`, `hpfs_fieldrelation`, `hpfs_fieldrelationc`, `hpfs_hrbm_config`（任一子实体）
...
**真发证据 / 反编译示例（如有）**：
- `FieldRelationEdit.java` (30518 bytes)
- `FileMapManagerEdit.java` (47741 bytes)
- `FileMapManagerNewAddEdit.java` (7549 bytes)
- `FileMapManagerSaveOp.java` (1307 bytes)
