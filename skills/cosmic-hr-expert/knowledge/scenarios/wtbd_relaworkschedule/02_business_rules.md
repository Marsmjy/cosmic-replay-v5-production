...
## ✅ verified · 主表 `wtbd_relaworkschedule` 共 63 个 opKey
...
#### `batchadj` · 更新班次和日期类型（opType=donothing）

**执行链（1 个 plugin · 按 order 排）**：

| order | className | 角色 |
|:---:|---|---|
| 1 | `WorkScheduleBatchAdjOp` | enabled=True |
...
- `auto_assign` · `tbl_assign_import` · `logview` · `viewonelog` · `gen` · `addduty` · `sync` · `change` · `records` · `premonth` · `nextmonth` · `editgencondition`
- `batchnew` · `reviseconflicthd` · `donothing` · `syncplanroster` · `tobatchadj`

## ✅ verified · 反编译类画像（共 10 个真处理类）
