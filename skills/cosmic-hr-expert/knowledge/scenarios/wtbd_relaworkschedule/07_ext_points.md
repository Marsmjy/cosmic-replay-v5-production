...
主表 `wtbd_relaworkschedule` 标品接入 **45** 个插件。
...
- `WorkScheduleEdit` ← `kd.wtc.wtbd.fromplugin.web.workschedule.WorkScheduleEdit`
- `WorkScheduleHpConflictEdit` ← `kd.wtc.wtbd.fromplugin.web.workschedule.WorkScheduleHpConflictEdit`
...
- `WorkScheduleList` ← `kd.wtc.wtbd.fromplugin.web.workschedule.WorkScheduleList`
- `WTCPresetList` ← `kd.wtc.wtbs.formplugin.web.model.WTCPresetList`
- `RelWorkScheduleList` ← `kd.wtc.wtbd.fromplugin.web.workschedule.RelWorkScheduleList`
...
- `WorkScheduleBatchAdjOp` ← `kd.wtc.wtbd.opplugin.web.workschedule.update.WorkScheduleBatchAdjOp`
...
## ✅ verified · 反编译实抓真处理类（10 个 · ISV 可继承 6 / 禁继承 4）
...
### opKey `batchadj` · 更新班次和日期类型

**执行链**（1 个 plugin 按 order 顺序跑）：

- order **1**: `WorkScheduleBatchAdjOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtbd.opplugin.web.workschedule.update.WorkScheduleBatchAdjOp`
