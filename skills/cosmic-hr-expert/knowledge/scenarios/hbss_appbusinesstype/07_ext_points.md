...
主表 `hbss_appbusinesstype` 标品接入 **20** 个插件。
...
- `HRAppBusinessTypeEdit` ← `kd.hr.hbss.formplugin.web.hrbu.HRAppBusinessTypeEdit`
- `HRBaseDataCommonFieldLockEdit` ← `kd.hr.hbp.formplugin.web.HRBaseDataCommonFieldLockEdit`
...
- `HRAppBusinessTypeList` ← `kd.hr.hbss.formplugin.web.hrbu.HRAppBusinessTypeList`
...
- `HRAppBusinessSaveOp` ← `kd.hr.hbss.opplugin.web.hrbu.HRAppBusinessSaveOp`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.hbss.formplugin.web.hrbu.HRAppBusinessTypeEdit -->

## ISV 扩展指引（基于 HRAppBusinessTypeEdit 真实证）
...
- 父类: `kd.hr.hbp.formplugin.web.HRDataBaseEdit`
- ISV 可继承: ✅ 推荐继承本父类
- 推荐挂载方法: `registerListener`, `afterBindData`, `beforeF7Select`, `afterCreateNewData`
...
**QUERY_BUILDER** · HRAppBusinessTypeEdit L72
```java
  70                   DynamicObject[] dynamicObjects = serviceHelper.queryOriginalArray("cloud", null);
  71                   ListShowParameter showParameter = (ListShowParameter)beforeF7SelectEvent.getFormShowParameter();
  72 >                 QFilter cloud = new QFilter("bizcloud", "in", Arrays.stream(dynamicObjects).map(el -> el.getString("cloud")).collect(Collectors.toSet()));
  73                   showParameter.getListFilterParameter().getQFilters().add(cloud);
  74                   showParameter.getListFilterParameter().getQFilters().add(new QFilter("id", "not in", (Object)appIdSet.toArray(new String[0])));
```

**READ_VIA_HELPER** · HRAppBusinessTypeEdit L62
```java
  60           DynamicObjectCollection docs;
  61           String fieldName = beforeF7SelectEvent.getProperty().getName();
  62 >         if ("app".equals(fieldName) && Objects.nonNull(docs = QueryServiceHelper.query((String)"hbss_appbusinesstype", (String)"app", null)) && docs.size() > 0) {
  63               HashSet<String> appIdSet = new HashSet<String>();
  64               for (DynamicObject doc : docs) {
```

详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.hbss.formplugin.web.hrbu.HRAppBusinessTypeEdit/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.hbss.formplugin.web.hrbu.HRAppBusinessTypeEdit/extension_guide.md)
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.hbss.formplugin.web.hrbu.HRAppBusinessTypeList -->

## ISV 扩展指引（基于 HRAppBusinessTypeList 真实证）
...
**QUERY_BUILDER** · HRAppBusinessTypeList L117
```java
 115               List<TreeNode> appNodes = this.getAppNodes(nodeId);
 116               appNodes.forEach(node -> nodeIds.add(node.getId()));
 117 >             QFilter qFilter = new QFilter("app", "in", (Object)nodeIds);
 118               nodeEvent.addQFilter(qFilter);
 119           }
```

**READ_VIA_HELPER** · HRAppBusinessTypeList L155
```java
 153           if (pageType != null && "extend".equals(pageType)) {
 154               String refAppId = (String)formShowParameter.getCustomParam("refappid");
 155 >             AppMetadata refAppMeta = AppMetaServiceHelper.loadAppMetadataFromCacheById((String)refAppId, (boolean)false);
 156               String refAppName = refAppMeta.getName().getLocaleValue();
 157               TreeNode node = new TreeNode(cloudId, refAppId, refAppName);
```

详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.hbss.formplugin.web.hrbu.HRAppBusinessTypeList/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.hbss.formplugin.web.hrbu.HRAppBusinessTypeList/extension_guide.md)
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.hbss.opplugin.web.hrbu.HRAppBusinessSaveOp -->

## ISV 扩展指引（基于 HRAppBusinessSaveOp 真实证）
...
详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.hbss.opplugin.web.hrbu.HRAppBusinessSaveOp/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.hbss.opplugin.web.hrbu.HRAppBusinessSaveOp/extension_guide.md)
