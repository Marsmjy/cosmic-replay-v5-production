# hbss_appbusinesstype · 定制方案（ISV 扩展真实模板）

> **form**：`hbss_appbusinesstype`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`hbss_appbusinesstype`）加 ISV 字段：
...
标品 `HRAppBusinessSaveOp` 在 `onAddValidators` 注入了：
...
@SdkPlugin
public class MyAppbusinesstypeIsvOp extends HRDataBaseOp {
    @Override
...
```java
@SdkPlugin
public class MyAppbusinesstypeAfterOp extends HRDataBaseOp {
    @Override
...
标品 `HRAppBusinessTypeEdit` 已实现：
- `registerListener` (L45)
- `afterBindData` (L51)
- `afterCreateNewData` (L87)
- `beforeF7Select` (L59)
...
```java
@SdkPlugin
public class MyAppbusinesstypeIsvFormPlugin extends HRDataBaseEdit {
    @Override
