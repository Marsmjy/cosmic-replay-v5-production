# 扩展点全图 · 岗位模板类型（hbpm_positiontpltype）
...
### 1.1 PositionTplTypeEditPlugin — 编辑视图核心插件

| 属性 | 值 |
|---|---|
| **全限定类名** | `kd.hrmp.hbpm.formplugin.web.position.PositionTplTypeEditPlugin` |
| **父类** | `HRDataBaseEdit`（白名单合规） |
| **Jar** | `hrmp-hbpm-formplugin-1.0.jar` |
| **注册顺序** | #9（在 HRHiesButtonSwitchPlugin #8 之后）|
| **生命周期方法** | `afterBindData` + `afterDoOperation` |
| **ISV 继承** | ❌ **禁继承**（PR-001）|

**afterBindData 职责（反编译实证）**：
1. 检查 `enable="0"` → VIEW 模式 + 隐藏 btnsave/btncancel + 显示 btnclose（INV-TT-01）
2. 检查 `index=0/空` → 查 maxIndex → 填 maxIndex+10 → setDataChanged(false)（INV-TT-02）

**afterDoOperation 职责（反编译实证）**：
- 操作失败时，解析 nameError/indexError 选项变量
- 调 `PositionTplUtil.isNameEnableRe` 高亮对应字段（INV-TT-03）

**ISV 扩展替代方案**：
- 禁止继承 `PositionTplTypeEditPlugin`（PR-001）
- 改用 `HRDataBaseEdit` 并列挂新 FormPlugin，RowKey 排在 #9 之后
- afterBindData 必须先判断 enable="0"，禁用态跳过 ISV 自定义联动

---

### 1.2 PositionTplTypeSaveOp — 保存校验 OP

| 属性 | 值 |
|---|---|
| **全限定类名** | `kd.hrmp.hbpm.opplugin.web.position.PositionTplTypeSaveOp` |
| **父类** | `HRDataBaseOp`（白名单合规）|
| **Jar** | `hrmp-hbpm-opplugin-1.0.jar` |
| **注册顺序** | #25（在 HRBaseOriginalOp #24 之后）|
| **生命周期方法** | `onAddValidators` + `beginOperationTransaction` |
| **ISV 继承** | ❌ **禁继承**（PR-001）|

**onAddValidators 职责（反编译实证）**：
```java
evt.addValidator(new PositionTplCommonValidator());         // 名称唯一
evt.addValidator(new PositionTplTypeIndexUniqueValidator()); // index 唯一
evt.addValidator(new CtrlStrategyValidator());               // 控制策略
```

**beginOperationTransaction 职责（反编译实证）**：
- 兜底填充 index 为空的数据行（API/Import 绕过 UI 时）

**ISV 扩展替代方案**：
- 禁止继承 `PositionTplTypeSaveOp`（PR-001）
- 改用 `HRDataBaseOp` 并列挂新 OP，RowKey 排在 #25 之后
...
| 类名 | 包路径 | 禁止原因 |
|---|---|---|
| `PositionTplTypeEditPlugin` | `kd.hrmp.hbpm.formplugin.web.position.PositionTplTypeEditPlugin` | 场景专属 FormPlugin · PR-001 |
| `PositionTplTypeSaveOp` | `kd.hrmp.hbpm.opplugin.web.position.PositionTplTypeSaveOp` | 场景专属 OP · PR-001 |
| `AbsOrgBaseOp` | `kd.hr.haos.opplugin.web.AbsOrgBaseOp` | 不在 HR SDK 白名单（forbidden）|

以上 3 个类在全文各出现 2+ 次（本文 + 06_customization_solutions.md），确保 ISV 不会误继承。
...
│    ⚠ 禁继承 PositionTplTypeSaveOp / PositionTplTypeEditPlugin / AbsOrgBaseOp（PR-001）
│    ⚠ BaseFormModel · 不加 iscurrentversion 过滤
├─ 字段联动 → CS-03（HRDataBaseEdit 并列挂 · propertyChanged）
│    ⚠ 必须 beginInit/endInit（PR-004）
│    ⚠ 禁继承 PositionTplTypeEditPlugin（PR-001）
│    ⚠ afterBindData 联动需先判断 enable="0"
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hrmp.hbpm.formplugin.web.position.PositionTplTypeEditPlugin -->

## ISV 扩展指引（基于 PositionTplTypeEditPlugin 真实证）
...
详细参考：[`_sdk_audit/_decompiled_deep/kd.hrmp.hbpm.formplugin.web.position.PositionTplTypeEditPlugin/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hrmp.hbpm.formplugin.web.position.PositionTplTypeEditPlugin/extension_guide.md)
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hrmp.hbpm.opplugin.web.position.PositionTplTypeSaveOp -->

## ISV 扩展指引（基于 PositionTplTypeSaveOp 真实证）
...
详细参考：[`_sdk_audit/_decompiled_deep/kd.hrmp.hbpm.opplugin.web.position.PositionTplTypeSaveOp/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hrmp.hbpm.opplugin.web.position.PositionTplTypeSaveOp/extension_guide.md)
