...
### `hsbs_statisticstag`（3 条）

| ruleId | 类型 | preCondition | 描述 |
|---|---|---|---|
| `/K28N+RW3Z2P` | bizRule | `number <> null OR parent.number <> null` | 长编码=上级长编码.编码 |
| `/K28N+RW4/3G` | bizRule | `name <> null OR parent.name <> null` | 长名称=上级长名称.名称 |
| `/K28N+RW3Z2Q` | bizRule | `parent.level <> null OR number <> null` | 级次=上级级次+1 |
