...
## ✅ verified · 主表 `wtbd_scenerulecfg` 共 43 个 opKey
...
| 4 | `HRBaseDataLogOp` | HR 基础资料操作日志 |
| 5 | `RetrievalRuleOp` | enabled=True |
...
| 3 | `RetrievalRuleOp` | enabled=True |
...
- `new` · `modify` · `view` · `close` · `returndata` · `refresh` · `option` · `submitandnew` · `first` · `previous` · `next` · `last`
- `importtemplatelist` · `mobtoolbarselect` · `mobtoolbarcancel` · `namehistory` · `namehistoryview` · `assign` · `unassign` · `bdctrlchange` · `individuation` · `assign_new` · `auto_assign` · `tbl_assign_import`
- `logview` · `viewonelog` · `moveentryup` · `moveentrydown` · `deleteentry` · `addparam` · `preview` · `addscope` · `extend` · `deletescope`
