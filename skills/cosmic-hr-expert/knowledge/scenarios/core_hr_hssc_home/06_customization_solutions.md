# core_hr_hssc_home · 定制方案（ISV 扩展真实模板）
...
在开发平台给本 form（`hssc_apphome`）加 ISV 字段：
...
标品 `BizAppHomePlugin` 已实现：
- `registerListener` (L70)
- `afterDoOperation` (L192)
- `click` (L87)
...
```java
@SdkPlugin
public class MyHomeIsvFormPlugin extends HRDataBaseEdit {
    @Override
