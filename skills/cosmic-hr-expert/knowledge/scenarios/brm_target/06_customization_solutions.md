...
在开发平台给本 form（`brm_target`）加 ISV 字段：
...
标品 `TargetLogOp` 在 `onAddValidators` 注入了：
- `TargetValidator`
...
@SdkPlugin
public class MyTargetIsvOp extends HRDataBaseOp {
    @Override
...
```java
@SdkPlugin
public class MyTargetAfterOp extends HRDataBaseOp {
    @Override
...
标品 `FunctionPlugin` 已实现：
- `registerListener` (L97)
- `propertyChanged` (L111)

ISV **并列挂**（PR-001 · 不要继承标品 Edit）：

```java
@SdkPlugin
public class MyTargetIsvFormPlugin extends HRDataBaseEdit {
    @Override
