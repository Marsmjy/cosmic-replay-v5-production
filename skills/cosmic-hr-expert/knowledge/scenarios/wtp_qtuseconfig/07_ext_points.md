...
主表 `wtp_qtuseconfig` 标品接入 **52** 个插件。
...
- `QTUseConfigOp` ← `kd.wtc.wtp.opplugin.web.quota.QTUseConfigOp`
...
- order **9**: `HisUniqueValidateOp` · 时态模型唯一性校验（同一 boid 同一时间段唯一）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisUniqueValidateOp`
- order **10**: `QTUseConfigOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.quota.QTUseConfigOp`
...
- order **8**: `HisUniqueValidateOp` · 时态模型唯一性校验（同一 boid 同一时间段唯一）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisUniqueValidateOp`
- order **9**: `QTUseConfigOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.quota.QTUseConfigOp`
...
- order **3**: `HisModelOPCommonPlugin` · 时态模型通用 · his_bsed/his_bsled/effectdate 的维护
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisModelOPCommonPlugin`
- order **4**: `QTUseConfigOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtp.opplugin.web.quota.QTUseConfigOp`
