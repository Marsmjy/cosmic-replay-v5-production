...
### Case 7：扩展 `HRBaseOriginalOp` (opKey=`save`)
...
### Case 8：扩展 `HisModelOPCommonPlugin` (opKey=`save`)

**业务背景**：标品 `HisModelOPCommonPlugin` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hr.hbp.opplugin.web.history` 包
2. 写 ISV 子类继承 `HisModelOPCommonPlugin`：
```java
