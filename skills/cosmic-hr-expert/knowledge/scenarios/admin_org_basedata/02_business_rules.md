...
**必填字段（1）**：
- `adminorgtypestd` (类型归属) · `BasedataField` → ref haos_adminorgtypestd
...
- `adminorgtypestd` (类型归属) → `haos_adminorgtypestd` · `BasedataField`
- `orgpattern` (形态) → `bos_org_pattern` · `BasedataField`
...
**必填字段（1）**：
- `orgchangetype` (变动类型) · `BasedataField` → ref haos_orgchangetype
...
- `srccreateorg` (原创建组织) → `bos_org` · `OrgField`
- `disabler` (禁用人) → `bos_user` · `UserField`
- `orgchangetype` (变动类型) → `haos_orgchangetype` · `BasedataField`
- `otclassify` (组织团队分类) → `haos_otclassify` · `BasedataField`
