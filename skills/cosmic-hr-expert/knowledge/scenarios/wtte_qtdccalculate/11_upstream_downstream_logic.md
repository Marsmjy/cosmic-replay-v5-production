...
> 本 form（`wtte_qtdccalculate`，所属 other）引用了其他云的 **2** 个底座实体：

### ⬆️ unknown（`unknown`）2 个引用

| 字段 | 字段名 | 类型 | 引用实体 | 上游场景 |
|---|---|---|---|---|
| `planid` | 核算方案 | BasedataField | `wtp_accountplan` | — |
| `tieplanid` | 单据核算方案 | BasedataField | `wtp_accountplan` | — |
