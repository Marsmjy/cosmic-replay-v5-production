...
### `AssignmentMagDeleteOp`

- **FQN**：`kd.hrmp.hrpi.opplugin.web.assignment.attach.AssignmentMagDeleteOp`
- **继承**：`HRDataBaseOp`

**调用的 Service / Helper**（2 个）：`AssignmentMagDeleteKeepOneValidator`, `OperationService`

**实现的生命周期方法**：`onAddValidators`(L74), `beginOperationTransaction`(L79)
...
**调用的 Service / Helper**（2 个）：`EmployeeDateOverLapValidator`, `OperationService`

**实现的生命周期方法**：`onAddValidators`(L134), `beginOperationTransaction`(L141)
...
- 数据源：`_sdk_audit/_decompiled/scenarios/core_hr_assignmentmag/` 的 13 个反编译类 + `_shared/_standard_metadata/entity_metadata/hrpi_assignmentmag.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.hrpi.opplugin.web.assignment.attach.AssignmentMagSaveOp -->

## chgaction 实证补充（AssignmentMagSaveOp 跨类追踪聚合）
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.hrpi.opplugin.web.assignment.attach.AssignmentMagDeleteOp -->

## chgaction 实证补充（AssignmentMagDeleteOp 跨类追踪聚合）
