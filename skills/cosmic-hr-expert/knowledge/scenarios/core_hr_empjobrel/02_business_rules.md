...
- **FQN**：`kd.hrmp.hrpi.opplugin.web.assignment.attach.EmpJobRelSaveOp`
- **继承**：`HRDataBaseOp`

**触发的 Validator**（save/delete 前置校验）：
- `BeforeAdjEmpJobRelEndDateValidator`
- `BeforeAdjEmpJobRelValidator`
- `EmpJobRelJobValidator`

**调用的 Service / Helper**（4 个）：`BeforeAdjEmpJobRelEndDateValidator`, `BeforeAdjEmpJobRelValidator`, `EmpJobRelJobValidator`, `HRBaseServiceHelper`

**实现的生命周期方法**：`onAddValidators`(L100), `beginOperationTransaction`(L133)
...
- 数据源：`_sdk_audit/_decompiled/scenarios/core_hr_empjobrel/` 的 12 个反编译类 + `_shared/_standard_metadata/entity_metadata/hrpi_empjobrel.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.hrpi.opplugin.web.assignment.attach.EmpJobRelSaveOp -->

## chgaction 实证补充（EmpJobRelSaveOp 跨类追踪聚合）

> FQN: `kd.hrmp.hrpi.opplugin.web.assignment.attach.EmpJobRelSaveOp`
> 跨类追踪: 19 个类访问
> 反编译产物: `_sdk_audit/_decompiled_deep/kd.hrmp.hrpi.opplugin.web.assignment.attach.EmpJobRelSaveOp/`
