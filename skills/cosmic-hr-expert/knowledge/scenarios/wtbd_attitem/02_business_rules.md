...
## ✅ verified · 主表 `wtbd_attitem` 共 68 个 opKey
...
| 2 | `HRBaseDataLogOp` | HR 基础资料操作日志 |
| 3 | `BaseDataAuditPlugin` | enabled=True |
...
#### `donothing` · 空操作（opType=donothing）

**执行链（1 个 plugin · 按 order 排）**：

| order | className | 角色 |
|:---:|---|---|
| 1 | `AttItemOp` | enabled=True |
...
- `auto_assign` · `tbl_assign_import` · `logview` · `viewonelog` · `hisversioninfo` · `change` · `hiscopy` · `newhisversion` · `revise` · `reviserecord` · `versionchangecompare` · `showallversion`
- `newentry` · `deleteentry` · `moveentryup` · `moveentrydown` · `donothing_viewhislist` · `viewhis` · `openform` · `delete_donothing` · `orgpermchange`
