# ISV 实战 case · haos_adminorgstruct
...
### Case 1：扩展 `HRHiesButtonSwitchPlugin` (opKey=`save`)
...
### Case 2：扩展 `HRBaseDataTplEdit` (opKey=`save`)
...
### Case 3：扩展 `HRCertCheckList` (opKey=`save`)
...
### Case 7：扩展 `HisUniqueValidateOp` (opKey=`save`)
...
### Case 8：扩展 `HRBaseDataLogOp` (opKey=`unsubmit`)

**业务背景**：标品 `HRBaseDataLogOp` 处理 `unsubmit` 操作 · ISV 想加自定义校验/写表/通知
...
```
3. 在 BOS 设计器注册 ISV 插件 · targetType=OPERATION
4. 单测：触发 opKey `unsubmit` · 验证 ISV 逻辑 + super 行为都跑
