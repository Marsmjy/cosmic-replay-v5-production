...
> 本 form（`wtte_exrecord`，所属 other）引用了其他云的 **6** 个底座实体：

### ⬆️ 核心人力云（`core_hr`）3 个引用

| 字段 | 字段名 | 类型 | 引用实体 | 上游场景 |
|---|---|---|---|---|
| `employee` | 员工 | BasedataField | `hrpi_employee` | [core_hr_employee](../core_hr_employee/) |
| `assignment` | 组织分配 | BasedataField | `hrpi_assignment` | [core_hr_assignment](../core_hr_assignment/) |
| `empposorgrel` | 任职经历 | BasedataField | `hrpi_empposorgrel` | [core_hr_empposorgrel](../core_hr_empposorgrel/) |

### ⬆️ unknown（`unknown`）3 个引用

| 字段 | 字段名 | 类型 | 引用实体 | 上游场景 |
|---|---|---|---|---|
| `exprocessid` | 异常处理方式 | BasedataField | `wtp_exprocess` | — |
| `attfilevid` | 考勤档案版本 | BasedataField | `wtp_attfilebase` | — |
| `attfileid` | 考勤档案 | BasedataField | `wtp_attfilebase` | — |
