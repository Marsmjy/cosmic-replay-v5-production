...
### OpPlugin（2）
- `ExRecordConfirmOp` ← `kd.wtc.wtte.opplugin.web.ex.ExRecordConfirmOp`
- `ExRecordCancelConfirmOp` ← `kd.wtc.wtte.opplugin.web.ex.ExRecordCancelConfirmOp`

### 其他（3）
- `templatebaseedit` ← `dev.tpl.base.kd.bos.form.plugin.templatebaseedit`
- `HRBaseDataCreateOrgPluginExt` ← `kd.hr.hbp.formplugin.web.bdctrl.HRBaseDataCreateOrgPluginExt`
- `BasedataFilterController` ← `kd.bos.form.plugin.bdctrl.BasedataFilterController`

## ✅ verified · 反编译实抓真处理类（7 个 · ISV 可继承 5 / 禁继承 2）
...
#### `HRBaseDataCreateOrgPluginExt` · OTHER · opKey=`save`

- **FQN**: `kd.hr.hbp.formplugin.web.bdctrl.HRBaseDataCreateOrgPluginExt`
- **继承链**: `kd.bos.form.plugin.bdctrl.BaseDataCreateOrgPlugin`
- **为什么禁**: 标识为内部实现 · ISV 继承会破升级兼容
- **该用什么**: 上一节「可继承类」中找对应 opKey 的类
...
- order **1**: `ExRecordConfirmOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtte.opplugin.web.ex.ExRecordConfirmOp`
...
### opKey `cancelconfirm` · 取消确认

**执行链**（1 个 plugin 按 order 顺序跑）：

- order **1**: `ExRecordCancelConfirmOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtte.opplugin.web.ex.ExRecordCancelConfirmOp`
