# core_hr_pfs_chgevent · 定制方案（ISV 扩展真实模板）

> **场景**：⭐⭐⭐ 变动操作元配置（单据头引用焦点）（5 个子实体）
> **生成时间**：2026-04-30

## CS-01 · 给子实体加 ISV 自定义字段

**适用**：`hpfs_chgevent`, `hpfs_chgcategory`, `hpfs_chgaction`, `hpfs_chgreason`, `hpfs_chgactionlist`（任一子实体）
...
**真发证据 / 反编译示例（如有）**：
- `ChgActionEditPlugin.java` (11843 bytes)
