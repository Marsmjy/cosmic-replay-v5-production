...
在开发平台给本 form（`hrcs_label`）加 ISV 字段：
...
```java
@SdkPlugin
public class MyLabelAfterOp extends HRDataBaseOp {
    @Override
...
标品 `LabelListPlugin` 已实现：
- `registerListener` (L72)
- `afterBindData` (L119)
- `beforeDoOperation` (L78)

ISV **并列挂**（PR-001 · 不要继承标品 Edit）：

```java
@SdkPlugin
public class MyLabelIsvFormPlugin extends HRDataBaseEdit {
    @Override
