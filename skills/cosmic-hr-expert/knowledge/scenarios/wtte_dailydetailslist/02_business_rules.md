...
## ✅ verified · 主表 `wtte_dailydetailslist` 共 17 个 opKey

### 其他 opKey（17 个无独立规则·复用主链或反编译为空）

- `reportquery` · `refresh` · `close` · `syncexportexcel` · `exportexcel` · `exportreport` · `evaall` · `evaselect` · `notevaperson` · `modifyroster` · `modifyattfile` · `vacationbill`
- `travelbill` · `otbill` · `reissuebill` · `displayscheme` · `viewdetails`
