...
### Case 5：扩展 `FormulaFuncTreePlugin` (opKey=`save`)

**业务背景**：标品 `FormulaFuncTreePlugin` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hr.hbp.formplugin.web.formula` 包
2. 写 ISV 子类继承 `FormulaFuncTreePlugin`：
```java
...
### Case 6：扩展 `FormulaItemTreePlugin` (opKey=`save`)

**业务背景**：标品 `FormulaItemTreePlugin` 处理 `save` 操作 · ISV 想加自定义校验/写表/通知

**实现路径**：
1. ISV 工程 `pom.xml` 引 `kd.hr.hbp.formplugin.web.formula` 包
2. 写 ISV 子类继承 `FormulaItemTreePlugin`：
```java
...
### Case 7：扩展 `HRBaseDataStatusOp` (opKey=`save`)
...
### Case 8：扩展 `HRBaseDataEnableOp` (opKey=`save`)
