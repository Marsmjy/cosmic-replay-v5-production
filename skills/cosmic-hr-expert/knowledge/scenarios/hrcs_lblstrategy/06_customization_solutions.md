# hrcs_lblstrategy · 定制方案（ISV 扩展真实模板）

> **form**：`hrcs_lblstrategy`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`hrcs_lblstrategy`）加 ISV 字段：
...
```java
@SdkPlugin
public class MyLblstrategyAfterOp extends HRDataBaseOp {
    @Override
...
标品 `LabelStrategyPlugin` 已实现：
- `registerListener` (L214)
- `afterCreateNewData` (L226)
- `propertyChanged` (L329)
- `beforeF7Select` (L440)
- `beforeDoOperation` (L458)
- `afterDoOperation` (L513)
- `click` (L286)
- `itemClick` (L321)
...
```java
@SdkPlugin
public class MyLblstrategyIsvFormPlugin extends HRDataBaseEdit {
    @Override
...
- `HRBaseServiceHelper`
- `LabelDataService`
- `LabelPolicyServiceHelper`
- `LabelService`
- `LabelStrategyValidator`
- `LabelTaskService`
- `LabelTaskStorageService`
