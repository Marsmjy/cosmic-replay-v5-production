...
主表 `hrcs_lblstrategy` 标品接入 **19** 个插件。
...
- `LabelStrategyPlugin` ← `kd.hr.hrcs.formplugin.web.label.LabelStrategyPlugin`
...
- `LblStrategyTreeListPlugin` ← `kd.hr.hrcs.formplugin.web.label.LblStrategyTreeListPlugin`
...
- `LabelStrategyOp` ← `kd.hr.hrcs.opplugin.web.label.LabelStrategyOp`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.hrcs.formplugin.web.label.LabelStrategyPlugin -->

## ISV 扩展指引（基于 LabelStrategyPlugin 真实证）
...
- ISV 可继承: ⚠️ 标品 @SdkInternal·改继承基类
- 推荐挂载方法: `registerListener`, `afterCreateNewData`, `afterLoadData`, `beforeBindData`, `click`, `closedCallBack`, `itemClick`, `propertyChanged`, `beforeF7Select`, `beforeDoOperation`, `afterDoOperation`
...
详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.label.LabelStrategyPlugin/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.label.LabelStrategyPlugin/extension_guide.md)
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.hrcs.formplugin.web.label.LblStrategyTreeListPlugin -->

## ISV 扩展指引（基于 LblStrategyTreeListPlugin 真实证）
...
- ISV 可继承: ⚠️ 标品 @SdkInternal·改继承基类
- 推荐挂载方法: `afterCreateNewData`, `setFilter`, `afterDoOperation`
...
**QUERY_BUILDER** · LblStrategyTreeListPlugin L133
```java
 131           if (StringUtils.isNotEmpty((CharSequence)(labelObjIds = (String)this.getView().getFormShowParameter().getCustomParam("labelObjectIds")))) {
 132               List idList = SerializationUtils.fromJsonStringToList((String)labelObjIds, Long.class);
 133 >             evt.getQFilters().add(new QFilter("labelobject", "in", (Object)idList));
 134               evt.getQFilters().add(new QFilter("enable", "=", (Object)"1"));
 135           }
```

**READ_VIA_HELPER** · LblStrategyTreeListPlugin L207
```java
 205   
 206       private List<TreeNode> getLblObjNodeList(String parentId, String text) {
 207 >         DynamicObject[] labelObjects = HRStringUtils.isEmpty((String)text) ? LblStrategyServiceHelper.getLabelObjects() : LblStrategyServiceHelper.getLabelObjects((String)text);
 208           ArrayList treeNodeList = Lists.newArrayListWithExpectedSize((int)labelObjects.length);
 209           for (DynamicObject labelObject : labelObjects) {
```

详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.label.LblStrategyTreeListPlugin/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.hrcs.formplugin.web.label.LblStrategyTreeListPlugin/extension_guide.md)
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · extension_points · kd.hr.hrcs.opplugin.web.label.LabelStrategyOp -->

## ISV 扩展指引（基于 LabelStrategyOp 真实证）
...
- ISV 可继承: ✅ 推荐继承本父类
- 推荐挂载方法: `onPreparePropertys`, `onAddValidators`, `beforeExecuteOperationTransaction`, `beginOperationTransaction`, `endOperationTransaction`
...
**QUERY_BUILDER** · LabelStrategyOp L176
```java
 174               List idList = Arrays.stream(e.getDataEntities()).map(dataEntity -> dataEntity.getLong("id")).collect(Collectors.toList());
 175               HRBaseServiceHelper filterServiceHelper = new HRBaseServiceHelper("hrcs_lblstrategyfilter");
 176 >             filterServiceHelper.deleteByFilter(new QFilter[]{new QFilter("lblstrategy.id", "in", idList)});
 177               this.deleteRule(e.getDataEntities());
 178               for (DynamicObject dataEntity3 : e.getDataEntities()) {
```
...
**WRITE_VIA_HELPER** · LabelStrategyOp L339
```java
 337                   saveCol.add((Object)dynamicObject);
 338               }
 339 >             OperationServiceHelper.executeOperate((String)"save", (String)"hrcs_lblstrategyfilter", (DynamicObject[])((DynamicObject[])saveCol.toArray((Object[])new DynamicObject[0])), (OperateOption)OperateOption.create());
 340           }
 341       }
```

**CALL_CROSS_SERVICE** · LabelStrategyOp L215
```java
 213           this.serviceHelper.deleteByFilter(new QFilter[]{new QFilter("labelpolicy.id", "in", idList)});
 214           List brmPolicyIdList = Arrays.stream(dataEntities).map(dyn -> dyn.getLong("brmpolicyid")).collect(Collectors.toList());
 215 >         HRMServiceHelper.invokeHRMPService((String)"brm", (String)"IBRMPolicyService", (String)"deletePolicy", (Object[])new Object[]{brmPolicyIdList});
 216           for (DynamicObject lblStrategy : dataEntities) {
 217               long lblObjId = lblStrategy.getLong("labelobject.id");
```
...
详细参考：[`_sdk_audit/_decompiled_deep/kd.hr.hrcs.opplugin.web.label.LabelStrategyOp/extension_guide.md`](../../_sdk_audit/_decompiled_deep/kd.hr.hrcs.opplugin.web.label.LabelStrategyOp/extension_guide.md)
