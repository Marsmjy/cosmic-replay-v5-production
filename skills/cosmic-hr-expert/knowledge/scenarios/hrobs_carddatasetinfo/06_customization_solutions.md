# hrobs_carddatasetinfo · 定制方案（ISV 扩展真实模板）

> **form**：`hrobs_carddatasetinfo`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`hrobs_carddatasetinfo`）加 ISV 字段：
...
```java
@SdkPlugin
public class MyCarddatasetinfoAfterOp extends HRDataBaseOp {
    @Override
...
标品 `CardDataSetInfoFormPlugin` 已实现：
- `registerListener` (L93)
- `propertyChanged` (L152)
- `beforeF7Select` (L143)
- `beforeDoOperation` (L256)
- `afterDoOperation` (L265)
- `click` (L191)
...
```java
@SdkPlugin
public class MyCarddatasetinfoIsvFormPlugin extends HRDataBaseEdit {
    @Override
