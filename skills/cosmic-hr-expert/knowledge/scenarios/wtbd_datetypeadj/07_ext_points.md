...
主表 `wtbd_datetypeadj` 标品接入 **55** 个插件。
...
- `DateTypeAdjSaveOp` ← `kd.wtc.wtbd.opplugin.web.workschedule.datetypeadj.DateTypeAdjSaveOp`
...
- order **9**: `HisModelOPCommonPlugin` · 时态模型通用 · his_bsed/his_bsled/effectdate 的维护
  - 完整 FQN: `kd.hr.hbp.opplugin.web.history.HisModelOPCommonPlugin`
- order **10**: `WTCNumberCheckOp` · enabled=True
...
- order **8**: `HRBaseOriginalOp` · HR 基础资料原始值记录（对比变更前后）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp`
- order **9**: `DateTypeAdjSaveOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtbd.opplugin.web.workschedule.datetypeadj.DateTypeAdjSaveOp`
