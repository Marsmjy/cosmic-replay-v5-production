# core_hr_perserlen · 业务规则

> **form**：`hrpi_perserlen` · 服务年限
> **生成时间**：2026-04-29
...
| 字段 | 中文名 | 类型 | 引用 |
|---|---|---|---|
| `startdate` | 司龄开始日期 | `DateField` | — |
| `enddate` | 司龄结束日期 | `DateField` | — |
| `assignment` | 组织分配 | `BasedataField` | `hrpi_assignment` |
| `employee` | 员工 | `EmployeeField` | `hrpi_employeenewf7query` |
| `firstjoincomdate` | 首次加入集团日期 | `DateField` | — |
| `joincomdate` | 本次加入集团日期 | `DateField` | — |
...
- **FQN**：`kd.hrmp.hrpi.opplugin.web.assignment.attach.PerSerLenSaveOp`
- **继承**：`HRDataBaseOp`

**触发的 Validator**（save/delete 前置校验）：
- `PerSerLenValidator`

**调用的 Service / Helper**（1 个）：`PerSerLenValidator`

**实现的生命周期方法**：`onAddValidators`(L57), `beginOperationTransaction`(L62)
...
- 数据源：`_sdk_audit/_decompiled/scenarios/core_hr_perserlen/` 的 12 个反编译类 + `_shared/_standard_metadata/entity_metadata/hrpi_perserlen.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.hrpi.opplugin.web.assignment.attach.PerSerLenSaveOp -->

## chgaction 实证补充（PerSerLenSaveOp 跨类追踪聚合）

> FQN: `kd.hrmp.hrpi.opplugin.web.assignment.attach.PerSerLenSaveOp`
> 跨类追踪: 21 个类访问
> 反编译产物: `_sdk_audit/_decompiled_deep/kd.hrmp.hrpi.opplugin.web.assignment.attach.PerSerLenSaveOp/`
...
### 调用的核心 Service（Top 10）
- `PerSerLenApplicationServiceImpl.class`
