...
- `NumberCheckPlugin` ← `kd.wtc.wtbs.formplugin.web.NumberCheckPlugin`
- `BreakTimeEdit` ← `kd.wtc.wtbd.fromplugin.web.shift.breaktime.BreakTimeEdit`
...
- `BreakTimeList` ← `kd.wtc.wtbd.fromplugin.web.shift.breaktime.BreakTimeList`

### OpPlugin（6）
- `CodeRuleOp` ← `kd.bos.business.plugin.CodeRuleOp`
- `BreakTimeOp` ← `kd.wtc.wtbd.opplugin.web.shift.breaktime.breaktimeop.BreakTimeOp`
...
- `BreakTimeOP` ← `kd.hr.wtc.wtbs.opplugin.web.shift.breaktime.breaktimeop.BreakTimeOP`
...
#### `HRBaseDataStatusOp` · OP_PLUGIN @ `OPERATION` · opKey=`save1`
...
#### `HRBaseDataLogOp` · OP_PLUGIN @ `OPERATION` · opKey=`save1`
...
#### `HRBaseDataEnableOp` · OP_PLUGIN @ `OPERATION` · opKey=`save1`
...
#### `HRBaseOriginalOp` · OP_PLUGIN @ `OPERATION` · opKey=`save1`
...
- order **3**: `BreakTimeOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtbd.opplugin.web.shift.breaktime.breaktimeop.BreakTimeOp`
...
- order **4**: `HRBaseDataEnableOp` · HR 基础资料启用态管理 · 控制 enable 字段
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseDataEnableOp`
- order **5**: `HRBaseOriginalOp` · HR 基础资料原始值记录（对比变更前后）
  - 完整 FQN: `kd.hr.hbp.opplugin.web.config.HRBaseOriginalOp`
...
- order **1**: `CodeRuleOp` · 编码规则（未启用时跳过）· onAddValidators 注册 numberValidator
  - 完整 FQN: `kd.bos.business.plugin.CodeRuleOp`
- order **2**: `BreakTimeOp` · enabled=True
  - 完整 FQN: `kd.wtc.wtbd.opplugin.web.shift.breaktime.breaktimeop.BreakTimeOp`
...
### opKey `wtbs_save` · 休息时间保存

**执行链**（1 个 plugin 按 order 顺序跑）：

- order **1**: `BreakTimeOP` · enabled=True
  - 完整 FQN: `kd.hr.wtc.wtbs.opplugin.web.shift.breaktime.breaktimeop.BreakTimeOP`
