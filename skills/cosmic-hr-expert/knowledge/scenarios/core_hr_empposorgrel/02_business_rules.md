...
### `EmpPosOrgRelDeleteOp`

- **FQN**：`kd.hrmp.hrpi.opplugin.web.assignment.attach.EmpPosOrgRelDeleteOp`
- **继承**：`HRDataBaseOp`

**调用的 Service / Helper**（1 个）：`EmpPosOrgRelDeleteKeepOneValidator`

**实现的生命周期方法**：`onAddValidators`(L58), `beginOperationTransaction`(L74)
...
- `EmpPosOrgRelIsPrimaryValidator`
- `EmpPosOrgRelPositionValidator`
- `EmpPosOrgRelPostTypeCategoryValidator`
- `EmpPosOrgRelSeqValidator`

**调用的 Service / Helper**（7 个）：`EmpPosOrgRelIsPrimaryValidator`, `EmpPosOrgRelPositionValidator`, `EmpPosOrgRelPostTypeCategoryValidator`, `EmpPosOrgRelSeqValidator`, `HRBaseServiceHelper`, `HRBuHelper`, `OperationService`

**实现的生命周期方法**：`onAddValidators`(L146), `beginOperationTransaction`(L214)
...
- 数据源：`_sdk_audit/_decompiled/scenarios/core_hr_empposorgrel/` 的 13 个反编译类 + `_shared/_standard_metadata/entity_metadata/hrpi_empposorgrel.md`
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.hrpi.opplugin.web.assignment.attach.EmpPosOrgRelSaveOp -->

## chgaction 实证补充（EmpPosOrgRelSaveOp 跨类追踪聚合）
...
| 异常码 | 业务消息 |
|---|---|
| `EmpPosOrgRelError_1` | 保存失败。 |
| `EmpPosOrgRelSaveOp_1` | 保存%1$s数据失败：%2$s |
| `` | errorMsg |
| `DynamicTransformUtil_1` | 获取属性失败。 |
| `DynamicTransformUtil_2` | 返回结果失败。 |
| `DynamicTransformUtil_3` | 转换失败! |
...
<!-- AUTO-INJECTED-BY-CLASS-RESOLVER · business_rules · kd.hrmp.hrpi.opplugin.web.assignment.attach.EmpPosOrgRelDeleteOp -->

## chgaction 实证补充（EmpPosOrgRelDeleteOp 跨类追踪聚合）
