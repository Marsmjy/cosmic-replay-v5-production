# core_hr_empposorgrel · 定制方案（ISV 扩展真实模板）

> **form**：`hrpi_empposorgrel`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`hrpi_empposorgrel`）加 ISV 字段：
...
```java
@SdkPlugin
public class MyEmpposorgrelAfterOp extends HRDataBaseOp {
    @Override
...
标品 `EmployeeAuditCommonOP` 已实现：
- `onAddValidators` (L79)
- `beforeExecuteOperationTransaction` (L86)
- `beginOperationTransaction` (L102)

ISV **并列挂**（PR-001 · 不要继承标品 Edit）：

```java
@SdkPlugin
public class MyEmpposorgrelIsvFormPlugin extends HRDataBaseEdit {
    @Override
...
本场景反编译类调用的 Service / Helper（14 个 · 仅前 10）：

- `EmpPosOrgRelDeleteKeepOneValidator`
- `EmpPosOrgRelIsPrimaryValidator`
- `EmpPosOrgRelPositionValidator`
- `EmpPosOrgRelPostTypeCategoryValidator`
- `EmpPosOrgRelSeqValidator`
- `EmployeeAuditRejectDataValidator`
- `EmployeeAuditValidator`
