...
## ✅ verified · 主表 `wtp_attperiod` 共 57 个 opKey
...
| 4 | `AttPeriodSaveOp` | enabled=True |
...
#### `entrysave` · 保存（opType=save）

**执行链（5 个 plugin · 按 order 排）**：

| order | className | 角色 |
|:---:|---|---|
| 1 | `AttPeriodSaveOp` | enabled=True |
| 2 | `BaseDataSavePlugin` | enabled=True |
| 3 | `HRBaseDataLogOp` | HR 基础资料操作日志 |
| 4 | `HRBaseDataEnableOp` | HR 基础资料启用态管理 · 控制 enable 字段 |
| 5 | `CodeRuleOp` | 编码规则（未启用时跳过）· onAddValidators 注册 numberValidator |
...
| 3 | `AttPeriodSaveOp` | enabled=True |
| 4 | `WTCSnapSaveOp` | enabled=False |
...
#### `submiteffect` · 提交并生效（opType=save）
...
- `importdata_hr` · `show_import_record_hr` · `export_from_list_hr` · `export_from_impttpl_hr` · `export_from_expttpl_hr` · `show_export_record_hr` · `new` · `modify` · `view` · `close` · `returndata` · `refresh`
- `option` · `submitandnew` · `first` · `previous` · `next` · `last` · `importdata` · `importdetails` · `importtemplatelist` · `exportlist` · `exportlistbyselectfields` · `exportlist_expt`
- `exportdetails` · `importexport_userset` · `mobtoolbarselect` · `mobtoolbarcancel` · `namehistory` · `namehistoryview` · `assign` · `unassign` · `bdctrlchange` · `individuation` · `assign_new` · `auto_assign`
- `tbl_assign_import` · `logview` · `viewonelog` · `donothing_assgin` · `donothing_manage` · `copy` · `newentry` · `deleteentry` · `donothing` · `manage`
