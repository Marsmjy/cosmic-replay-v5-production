...
## ✅ verified · 主表 `wtabm_vaapplyself` 共 67 个 opKey
...
| 2 | `BillTplOp` | enabled=True |
| 3 | `VaArchiveControlCheckOp` | enabled=True |
| 4 | `WTCBillSelfApplyOp` | enabled=True |
| 5 | `WTCBillApplyTypeOp` | enabled=True |
| 6 | `VaApplyOp` | enabled=True |
| 7 | `WTCBillUpdateAttfileOp` | enabled=True |
...
| 5 | `VaArchiveControlCheckOp` | enabled=True |
| 6 | `VaApplyOp` | enabled=True |
| 7 | `WTCBillUpdateAttfileOp` | enabled=True |
| 8 | `WTCBillChangeShowOp` | enabled=True |
| 9 | `WTCBillReplenishOriginalOp` | enabled=True |
...
| 5 | `WTCBillAttFileOp` | enabled=True |
...
| 5 | `WTCBillVerifyStatusOp` | enabled=True |
...
#### `update` · 变更（opType=donothing）

**执行链（1 个 plugin · 按 order 排）**：

| order | className | 角色 |
|:---:|---|---|
| 1 | `VaApplyChangeOp` | enabled=True |
...
- `wfauditing` · `wfauditpass` · `viewflowchart` · `unaudit` · `deleteentry` · `editrow` · `vadetail` · `quotainfodetail` · `applydetail` · `calculatedetail` · `opaddnewpage` · `mob_deleteentry`
- `openselectperson` · `openselecttype` · `newentry` · `spread` · `singletype` · `multitype` · `returnlist`
