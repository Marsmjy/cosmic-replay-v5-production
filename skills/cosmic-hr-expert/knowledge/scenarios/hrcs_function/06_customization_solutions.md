...
在开发平台给本 form（`hrcs_function`）加 ISV 字段：
...
```java
@SdkPlugin
public class MyFunctionAfterOp extends HRDataBaseOp {
    @Override
...
标品 `FunctionEdit` 已实现：
- `registerListener` (L70)
- `afterBindData` (L80)
- `propertyChanged` (L130)
- `beforeDoOperation` (L88)
- `afterDoOperation` (L115)

ISV **并列挂**（PR-001 · 不要继承标品 Edit）：

```java
@SdkPlugin
public class MyFunctionIsvFormPlugin extends HRDataBaseEdit {
    @Override
