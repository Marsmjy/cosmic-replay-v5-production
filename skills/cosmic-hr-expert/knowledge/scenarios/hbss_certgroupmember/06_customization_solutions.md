# hbss_certgroupmember · 定制方案（ISV 扩展真实模板）

> **form**：`hbss_certgroupmember`
> **生成时间**：2026-04-29
> **方法**：基于反编译实物的标品类，给出 ISV 真扩展模板（不是空壳骨架）

## CS-01 · 加 ISV 自定义字段

在开发平台给本 form（`hbss_certgroupmember`）加 ISV 字段：
...
标品 `CertGroupMemberPlugin` 已实现：
- `registerListener` (L78)
- `afterCreateNewData` (L146)
- `beforeDoOperation` (L102)
- `click` (L90)
...
```java
@SdkPlugin
public class MyCertgroupmemberIsvFormPlugin extends HRDataBaseEdit {
    @Override
