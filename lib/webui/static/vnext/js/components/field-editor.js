...
// 该字段是否被变量化：命中 detected_vars 且变量启用、含模板（即"变化量"规则）
function variableInfo(field, preview) {
  const vars = new Map((preview.detected_vars || []).map((item) => [item.name, item]));
  const variable = (field.vars || []).map((name) => vars.get(name)).find(Boolean);
  if (!variable || variable.enabled === false || !variable.template) return null;
  return variable;
}
...
function renderFieldRow(field, state) {
  const preview = state.preview || {};
  const variable = variableInfo(field, preview);
  const template = variable ? String(variable.template) : "";
  const recorded = recordedValue(field, preview);
  const draft = state.fieldDrafts[field.field_id] ?? "";
  const env = resolution(field, preview, state.resolvedFields);
  const finalValue = draft || env.value || recorded || (template || "运行时产生");
  const badgeClass = ["ambiguous", "not_found", "error"].includes(env.status)
    ? "is-danger" : env.status === "resolved" ? "is-success" : "is-warning";
  const placeholder = template ? `沿用变量：${template}` : "沿用录制值";
  const varHint = template
    ? `<small class="field-var-hint" title="该字段已变量化，每次运行按模板重新生成">变化量 <code>${escapeHtml(template)}</code></small>`
    : "";
  return `
    <tr>
      <td class="field-name">
      ...
      <td>
        <input class="input field-input" data-field-id="${escapeHtml(field.field_id)}"
          value="${escapeHtml(draft)}" placeholder="${escapeHtml(placeholder)}">
        ${varHint}
      </td>
