...
import { bindCaseDetail, renderCaseDetail } from "./workspaces/case-detail.js";
import {
  bindReportDialog,
  renderReportDialog,
  renderReportCenter,
  bindReportCenter,
  openReportCenter,
} from "./workspaces/report-workspace.js";
import { initTheme, renderThemeToggle, bindThemeToggle } from "./components/theme.js";
import { renderOnboarding, bindOnboarding, shouldShowGuide } from "./components/onboarding.js";
import { renderSettings, bindSettings, openSettings } from "./workspaces/settings-workspace.js";
import { renderLogs, bindLogs, openLogs } from "./workspaces/logs-workspace.js";
...
const stageMeta = {
  ...
  detail: ["用例详情", "查看与维护单个用例", "运行结果、变量、校验点、YAML 与执行步骤统一可追溯。"],
  settings: ["设置", "环境与凭证管理", "维护目标环境的地址、凭证与基础资料映射，保存即时生效。"],
  logs: ["执行历史", "可追溯的运行记录", "按执行历史回看步骤、变量、环境字段与请求响应证据。"],
  reports: ["报告中心", "集中查看执行质量趋势", "按天查看通过率趋势，随时打开最近一次批量执行报告。"],
};
...
async function bootstrap() {
  initTheme(store);
  if (shouldShowGuide()) store.update({ showGuide: true });
  render(store.get());
  try {
...
function render(state) {
  ...
  if (tools) {
    tools.innerHTML =
      `<nav class="topnav" aria-label="全局导航">` +
        navItem("open-reports", "reports", state.stage, "报告中心", "M4 13h3v6H4zM10.5 7h3v12h-3zM17 10h3v9h-3z") +
        navItem("open-logs", "logs", state.stage, "执行历史", "M12 8v5l3 2M12 3a9 9 0 1 0 9 9 9 9 0 0 0-9-9z") +
        navItem("open-settings", "settings", state.stage, "设置", "M12 9a3 3 0 1 0 3 3 3 3 0 0 0-3-3zM4 12h2m12 0h2M12 4v2m0 12v2") +
      `</nav>` +
  ...
  const stageHtml = state.stage === "readiness"
  ...
  content.innerHTML = stageHtml + renderReportDialog(state) + renderOnboarding(state);
  ...
  bindCaseDetail(store);
  bindReportDialog(store);
  bindSettings(store);
  bindLogs(store);
  bindReportCenter(store);
  bindThemeToggle(store);
  bindOnboarding(store);
  const logsBtn = document.querySelector("#open-logs");
  if (logsBtn) logsBtn.onclick = () => openLogs(store);
  const settingsBtn = document.querySelector("#open-settings");
  if (settingsBtn) settingsBtn.onclick = () => openSettings(store);
  const reportsBtn = document.querySelector("#open-reports");
  if (reportsBtn) reportsBtn.onclick = () => openReportCenter(store);
}

function navItem(id, stage, activeStage, label, iconPath) {
  return `<button class="topnav-item ${activeStage === stage ? "is-active" : ""}" id="${id}" title="${label}">` +
    `<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="${iconPath}"/></svg>` +
