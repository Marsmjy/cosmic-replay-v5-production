...
export const api = {
  ...
  config: (mask = true) => request(`/api/config?mask=${mask ? "true" : "false"}`),
  saveWebui: (payload) => jsonRequest("/api/config/webui", "PUT", payload),
  envs: (mask = true) => request(`/api/envs?mask=${mask ? "true" : "false"}`),
  saveEnv: (envId, payload) =>
    jsonRequest(`/api/envs/${encodeURIComponent(envId)}`, "PUT", payload),
  deleteEnv: (envId) =>
    request(`/api/envs/${encodeURIComponent(envId)}`, { method: "DELETE" }),
