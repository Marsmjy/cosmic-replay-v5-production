...
function visibleCases(state) {
  ...
  return sortCases(list, state);
}

// 按选定列排序（名称/步骤/最近结果/稳定性）
function sortCases(list, state) {
  const by = state.caseSortBy || "name";
  const dir = state.caseSortDir === "desc" ? -1 : 1;
  const resultRank = { PASS: 2, FAIL: 1 };
  const arr = [...list];
  arr.sort((a, b) => {
    if (by === "step") {
      return ((a.step_count ?? -1) - (b.step_count ?? -1)) * dir;
    }
    if (by === "result") {
      return ((resultRank[a.last_result] || 0) - (resultRank[b.last_result] || 0)) * dir;
    }
    if (by === "stability") {
      const av = state.stability?.[a.name]?.pass_rate ?? -1;
      const bv = state.stability?.[b.name]?.pass_rate ?? -1;
      return (av - bv) * dir;
    }
...
function sortIndicator(state, key) {
  if ((state.caseSortBy || "name") !== key) return '<span class="sort-ind">⇅</span>';
  return `<span class="sort-ind is-active">${state.caseSortDir === "desc" ? "↓" : "↑"}</span>`;
}
          ...
          <div class="table-wrap">
            <table class="field-table case-table">
              <thead><tr>
                ...
                <th class="sortable" data-sort="name">用例 ${sortIndicator(state, "name")}</th>
                <th style="width:90px" class="sortable" data-sort="step">步骤 ${sortIndicator(state, "step")}</th>
                <th style="width:110px" class="sortable" data-sort="result">最近结果 ${sortIndicator(state, "result")}</th>
...
export function bindRunWorkspace(store) {
  ...
  document.querySelectorAll("[data-sort]").forEach((el) => {
    el.addEventListener("click", () => {
      ...
      store.mutate((draft) => {
        if (draft.caseSortBy === key) {
          draft.caseSortDir = draft.caseSortDir === "asc" ? "desc" : "asc";
        } else {
          draft.caseSortBy = key;
          draft.caseSortDir = "asc";
        }
      });
    });
  });
...
function applyDetailEvent(state, type, data) {
  if (type === "step_start" || type === "step_ok" || type === "step_fail") {
    ...
    if (type === "step_start") {
      ...
      if (data.resolved_request !== undefined) phase.resolved_request = data.resolved_request;
