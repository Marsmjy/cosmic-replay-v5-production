...
function renderRow(item, checked, state) {
  ...
  return `
    <tr class="${selected}">
      ...
      <td>${resultBadge}</td>
      <td><button class="btn" data-run-one="${escapeHtml(item.name)}" ${state.busy ? "disabled" : ""}>运行</button></td>
    </tr>`;
}
...
export function bindRunWorkspace(store) {
  ...
  document.querySelector("#batch-run")?.addEventListener("click", () => runCases(store, store.get().checkedCases));
...
function subscribeRun(runId, store, onDone) {
