...
export function parsePickFields(yamlText) {
  ...
  return Object.keys(result).map((key) => {
    ...
    return {
      ...
      required_context: e.required_context === true || e.required_context === "true",
      source: e.source || "",
      reason: e.reason || "",
...
function renderVarsTab(state) {
  ...
  // 方案A：软必填上下文字段（系统按业务规则注入，可留空）单独分区，避免与普通环境字段混淆
  const softRequired = picks.filter((pf) => pf.required_context);
  const normal = picks.filter((pf) => !pf.required_context);
  return `
    ${normal.length ? renderPickTable("业务字段（环境敏感）", "编辑后写入版本化 YAML", normal, false) : ""}
    ${softRequired.length ? renderPickTable(
      "业务规则要求维护（可留空）",
      "系统按目标环境业务规则提示维护，通常无需修改，留空将沿用运行时解析",
      softRequired, true) : ""}`;
}

function renderPickTable(title, hint, picks, isSoftRequired) {
  return `
    <section class="field-group">
      <div class="field-group-title">
        <strong>${escapeHtml(title)}</strong>
        <span class="cell-muted">${escapeHtml(hint)}</span>
      </div>
        ...
        <table class="field-table">
          ...
          <tbody>
            ${picks.map((pf) => {
              ...
              const reasonHint = isSoftRequired && pf.reason
                ? `<small class="cell-muted" title="${escapeHtml(pf.reason)}">ⓘ ${escapeHtml(pf.reason)}</small>`
                : "";
              const tagHtml = isSoftRequired
                ? `<small class="cell-muted">业务规则上下文 · 可留空</small>`
                : `<small>${escapeHtml(pf.env_sensitive)} · ${escapeHtml(pf.resolve_status || "—")}</small>`;
              const placeholder = isSoftRequired ? "可留空，或填业务编码覆盖" : "业务编码 / value_id";
              return `
                <tr>
                  <td class="field-name">
                  ...
                  <td>
                    <input class="input detail-pick-input" data-pick-step="${escapeHtml(pf.step_id)}"
                      value="${escapeHtml(pf.value_code || pf.value_id || "")}" placeholder="${escapeHtml(placeholder)}">
                  </td>
