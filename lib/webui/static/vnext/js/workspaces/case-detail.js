...
function renderStepsTab(state) {
  const phases = state.phases || [];
  if (!phases.length) return '<div class="empty-state">运行后按业务阶段展示步骤导航。</div>';
  let activeStage = "";
  return `<div class="step-nav">${phases.map((p) => {
    ...
    return `${heading}
      <div class="step-nav-item">
        <span class="badge ${tone}">${escapeHtml(p.status || "—")}</span>
        <div>
          <strong>${escapeHtml(p.label || p.id)}</strong>
          ${p.detail ? `<small class="cell-muted">${escapeHtml(p.detail)}</small>` : ""}
          ${(p.errors || []).length ? `<small class="cell-danger">${escapeHtml((p.errors || []).join("; "))}</small>` : ""}
        </div>
      </div>`;
  }).join("")}</div>`;
}
...
export function bindCaseDetail(store) {
  ...
  document.querySelectorAll("[data-step-id]").forEach((el) => {
    el.addEventListener("click", () => store.update({ selectedStep: el.dataset.stepId }));
  });
